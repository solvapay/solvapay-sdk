...
**New — `@solvapay/core` pricing-option readers (Rust core, every language
surface).** `charges`, `headlineCharges`, `perUnitCharge`, `billingCycle`,
`trialDays`, `includedUnits`, `peggedCreditsPerUnit`, and
`creditsPerUnitFromBalance` read a plan or a frozen plan snapshot from
`options[]`. TypeScript, Go, Python, and Ruby all dispatch to the same
native helpers so MCP text narration and the MCP UI panel agree.
