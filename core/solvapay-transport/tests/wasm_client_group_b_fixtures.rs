//! Typed Group B client fixtures through [`FetchTransport`] under wasm32.
...
#[wasm_bindgen_test]
async fn group_b_typed_methods_round_trip_through_fetch() {
    ...
    for (index, (path, fixture)) in all.iter().enumerate() {
        if is_group_b_fixture(fixture) && fixture.wire.is_some() {
    ...
    assert_eq!(
        ...
        "expected {GROUP_B_FIXTURE_COUNT} Group B wire fixtures from {base}/__fixtures, found {}",
        wire_cases.len()
    );
...
fn fixture_server_base() -> String {
    ...
    let base = value.as_string().expect(
        "SOLVAPAY_FIXTURE_SERVER must be set (run via tools/conformance/test-wasm-transport.sh)",
    );
