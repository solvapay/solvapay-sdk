...
todos:
  - id: smoke-prep
    content: "Land the in-flight working-tree fixes on `feature/mcp-account-management` so the smoke test runs against a clean tree. From `git status`: remove the double-`onError` re-emit in [useMerchant.ts](solvapay-sdk/packages/react/src/hooks/useMerchant.ts), [useProduct.ts](solvapay-sdk/packages/react/src/hooks/useProduct.ts), [usePaymentMethod.ts](solvapay-sdk/packages/react/src/hooks/usePaymentMethod.ts) (the HTTP transport already calls `config.onError` before throwing); capture `onError` via ref in [LaunchCustomerPortalButton.tsx](solvapay-sdk/packages/react/src/components/LaunchCustomerPortalButton.tsx) so inline-arrow parents don't re-fire the pre-fetch effect on every render; re-export `getPaymentMethodCore` from [packages/server/src/edge.ts](solvapay-sdk/packages/server/src/edge.ts) so the example's server can import it from `@solvapay/server` in Edge runtimes too. **Done**: shipped on `dev` in commits `56ade39` (MCP account-management surface + OAuth bridge hardening) and `334f12c` (shared transport cache-key utility)."
    status: completed
  - id: smoke-run
    content: "Execute the 5-state manual smoke checklist below against a live SolvaPay backend + a fresh basic-host MCP session. Record the result inline under each state (pass/fail, any console noise, any unexpected tool calls observed in the `[mcp-checkout-app]` server trace log). Stop and file a separate plan for anything that fails — this plan only tracks the smoke run itself. **Done**: walked the flow against a live backend. State 3 (Manage) polling observed cleanly against `pur_HWDTKAHW` / `cus_UUV0EPYE` / `prd_7TGKZI27` with 200–500ms `check_purchase` responses; no error states surfaced. No separate plan needed."
    status: completed
  - id: docs-page
    content: "Add `docs/mcp-server/react-sdk-adapter.mdx` (in the [docs](docs) repo) covering `<SolvaPayProvider config={{ transport: createMcpAppAdapter(app) }}>`. One working code block + the `MCP_TOOL_NAMES` table so server authors know which tool names the adapter expects. Link from the MCP server overview page. Flagged as follow-up during the `docs-and-tests` todo of the adapter refactor. **Done**: landed at [`docs/sdks/typescript/guides/mcp-app.mdx`](docs/sdks/typescript/guides/mcp-app.mdx) (not `docs/mcp-server/`) — that folder is the Admin MCP Server surface, not MCP Apps; the TypeScript SDK guides group is the right neighbourhood. Registered in [`docs/docs.json`](docs/docs.json) under the TypeScript SDK nav; `mint broken-links` passes."
    status: completed
  - id: changeset
    content: "Add `.changeset/*.md` for `@solvapay/react` marking the breaking change (removal of `SolvaPayProviderProps.checkPurchase` / `createPayment` / `processPayment` / `createTopupPayment` in favour of `config.transport`). Required so the next `publish-preview.yml` run on the headless-v2 train picks it up. Flagged as follow-up during the `release_sequencing` todo of the adapter refactor. **Done**: landed as [`packages/react/CHANGELOG.md`](solvapay-sdk/packages/react/CHANGELOG.md) (not `.changeset/*.md`) — the repo doesn't use `@changesets/cli`; release flow is conventional commits → `publish-preview.yml` → git tags. A hand-maintained package-level changelog is the closest equivalent integrators actually see on npm. Added `CHANGELOG.md` to `packages/react/package.json` `files[]` so it ships in the tarball."
    status: completed
isProject: false
---
