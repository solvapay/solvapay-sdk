...
#   2. wasm-opt -Oz (if available) → sdks/go/solvapay_core.wasm
...
go_dir="$(cd "${script_dir}/.." && pwd)"          # sdks/go
repo_root="$(cd "${go_dir}/../.." && pwd)"        # cargo workspace root
...
( cd "${repo_root}" && cargo build -p "${crate}" --target "${target}" --profile "${profile}" )
