...
func TestReplaysDispatchAndOAuthThroughHTTP(t *testing.T) {
	for _, rel := range mcpAuthoringFixtures {
		if !(strings.HasPrefix(rel, "dispatch/") || strings.HasPrefix(rel, "oauth-proxy/")) {
			continue
		}
		if strings.HasSuffix(rel, "invoke-handler.json") {
			continue
		}
		t.Run(rel, func(t *testing.T) {
			env := loadFixtureJSON(t, rel)
			...
			client := newTestClient(t, base)
			cfg := asMap(args["config"])
			engine := NewEngine(client, HTTPConfig{
				ProductRef:    stringOr(cfg["productRef"], "prd_demo"),
				PublicBaseURL: stringOr(cfg["publicBaseUrl"], "https://app.example.com"),
				ResourceURI:   stringOr(cfg["resourceUri"], "ui://test/view.html"),
				MCPPath:       stringOr(cfg["mcpPath"], "/mcp"),
				OauthPaths:    asMap(cfg["oauthPaths"]),
			})
			var req *http.Request
			if fn == "mcpDispatch" {
				body, _ := json.Marshal(args["rpc"])
				req = httptest.NewRequest(http.MethodPost, "/mcp", bytes.NewReader(body))
				if auth, ok := args["authHeader"].(string); ok {
					req.Header.Set("Authorization", auth)
				}
			} else {
				body, _ := args["body"].(string)
				req = httptest.NewRequest(stringOr(args["method"], "GET"), stringOr(args["path"], "/"), strings.NewReader(body))
			}
			...
			if fn == "mcpOauthRequest" {
				...
				assertAsyncResult(t, rel, fn, map[string]any{
					"status":  float64(rec.Code),
					"headers": headers,
					"body":    body,
				}, want, srv)
				return
			}
			if want["kind"] == "challenge" {
				if rec.Code != int(want["status"].(float64)) {
					t.Fatalf("status %d want %v", rec.Code, want["status"])
				}
				var body any
				if err := json.Unmarshal(rec.Body.Bytes(), &body); err != nil {
					t.Fatal(err)
			...
			if !reflect.DeepEqual(body, want["rpc"]) {
				g, _ := json.MarshalIndent(body, "", "  ")
				w, _ := json.MarshalIndent(want["rpc"], "", "  ")
				t.Fatalf("mismatch\ngot:\n%s\nwant:\n%s", g, w)
			}
		})
	}
...
func TestEngineLoopInvokeHandlerThenResume(t *testing.T) {
	ctx := context.Background()
	...
	if err := engine.RegisterPayable("echo_paid", Options{
		Client:  client,
		Product: "prd_demo",
		Handler: func(_ context.Context, args map[string]any, rc *ResponseContext) (Response, error) {
			return rc.Respond(map[string]any{"echo": args}, nil)
		},
	}); err != nil {
		t.Fatal(err)
	}
	body, _ := json.Marshal(map[string]any{
		...
		"id":      3,
		"method":  "tools/call",
		"params":  map[string]any{"name": "echo_paid", "arguments": map[string]any{"n": 1}},
	})
	req := httptest.NewRequest(http.MethodPost, "/mcp", bytes.NewReader(body))
	req.Header.Set("Authorization", "Bearer eyJhbGciOiJub25lIn0.eyJzdWIiOiJjdXNfMSJ9.")
	rec := httptest.NewRecorder()
	...
	result := asMap(parsed["result"])
	echo := asMap(asMap(result["structuredContent"])["echo"])
	if echo["n"] != float64(1) || echo["customer_ref"] != "cus_1" {
