 ...
 *   node scripts/prepare-clean-install-packages.mjs --out-dir <dir> --targets darwin-arm64
...
function assertArtifactsPresent(selected) {
  ...
  for (const t of selected) {
    ...
    if (!existsSync(binaryPath) || !statSync(binaryPath).isFile()) {
      ...
      const files = readdirSync(dirPath).filter(f => f.endsWith('.node'))
      if (files.length !== 1) {
...
function main() {
  ...
  if (requireAll && selected.length !== ALL_TARGETS.length) {
    throw new Error('prepare-clean-install-packages: --require-all needs all 8 targets')
  }
  if (requireAll) {
