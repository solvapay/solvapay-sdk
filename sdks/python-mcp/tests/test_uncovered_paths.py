...
from solvapay_mcp.register import _PayableTool, _invoke_payable
...
@pytest.mark.asyncio
async def test_unresolvable_customer_ref_falls_back_to_anonymous() -> None:
    backend = MockBackend({"withinLimits": True, "remaining": 5, "meterName": "requests"})
    solvapay = create_solvapay(api_client=backend)
    ...
    assert backend.track_usage_calls[0]["customerRef"] == "anonymous"
