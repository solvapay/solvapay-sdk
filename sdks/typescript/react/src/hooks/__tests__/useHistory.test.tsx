...
describe('useHistory', () => {
  it('fetches charges and credit activity when enabled', async () => {
    ...
    const { result } = renderHook(() => useHistory({ productRef: 'prd_widget', enabled: true }))
  ...
  it('does not fetch when enabled is false', async () => {
    ...
    const { result } = renderHook(() => useHistory({ productRef: 'prd_widget', enabled: false }))
  ...
  it('surfaces a failed fetch distinctly from an empty list', async () => {
    ...
    expect(result.current.error).toEqual(
      expect.objectContaining({ message: 'history unavailable' }),
    )
  })
