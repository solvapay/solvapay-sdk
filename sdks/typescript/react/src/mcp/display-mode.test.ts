...
describe('resolveHostedRail', () => {
  ...
  // Host-reported width is not an input at all any more: the rail is a
  // pure function of `displayMode`, and the 1000px gate lives in the
  // `@container mcp-hosted` query. The signature only accepts
  // `displayMode`, so a stale `containerDimensions` cannot reach it.
  it('uses the hosted rail in fullscreen regardless of any host-reported width', () => {
