...
describe('Paywall Unit Tests - Mocked Backend', () => {
  ...
  describe('ProtectHandlerContext — ctx.respond plumbing', () => {
    it('passes { customerRef, limits } to the handler on cache-miss', async () => {
      ...
      await mcpHandler({ input: 'test' }, { authInfo: { extra: { customer_ref: 'ctx_user' } } })
    ...
    it('keeps `limits` populated on cache-hit within the TTL window', async () => {
      ...
      await mcpHandler({ a: 1 }, { authInfo: { extra: { customer_ref: 'cache_user' } } })
      await mcpHandler({ a: 2 }, { authInfo: { extra: { customer_ref: 'cache_user' } } })
    ...
    it('forwards `extra` bag as `handlerContext.extra`', async () => {
      ...
      const [, handlerContext] = handler.mock.calls[0]
      expect(handlerContext.extra).toEqual(
  ...
  describe('Action Name Resolution', () => {
    ...
    it('sends an idempotencyKey derived from the decide() requestId', async () => {
      const handler = vi.fn().mockResolvedValue({ success: true })
      const payable = solvaPay.payable({ product: 'idempotency-test' })
      const protectedHandler = await payable.function(handler)

      await protectedHandler({ auth: { customer_ref: 'cus_idem' } })

      expect(mockApiClient.trackUsageCalls).toHaveLength(1)
      const call = mockApiClient.trackUsageCalls[0]
      expect(call.idempotencyKey).toBe(`${call.metadata.requestId}:success`)
    })

    it('records toolName on usage events when payable metadata carries it', async () => {
      const handler = vi.fn().mockResolvedValue({ success: true })
      const payable = solvaPay.payable({ product: 'tool-name-test', toolName: 'search_knowledge' })
      const protectedHandler = await payable.function(handler)

      await protectedHandler({ auth: { customer_ref: 'cus_tool' } })

      expect(mockApiClient.trackUsageCalls[0].metadata.toolName).toBe('search_knowledge')
    })
  ...
  describe('Plan simplification - no plan/planRef in payable flow', () => {
    ...
    it('opts into checkout-session creation so the 402 carries a hosted URL', async () => {
      // `paywall.decide()` embeds `checkoutUrl` in the 402
      // `PaywallStructuredContent` (consumed by
      // `<PaywallNotice.HostedCheckoutLink>`), so it must opt in to
      // the backend's `includeCheckoutSession` flag. Other callers of
      // `apiClient.checkLimits` (notably `checkLimitsCore` powering
      // `useLimits`) deliberately do NOT opt in — they discard the
      // URL and were causing orphan `CheckoutSession` documents +
      // spurious `checkout_session.created` webhooks on every
      // refetch. Pinning this here guards against accidental drift.
      const checkLimitsSpy = vi.spyOn(mockApiClient, 'checkLimits')
      const handler = vi.fn().mockResolvedValue({ ok: true })
      const payable = solvaPay.payable({ product: 'prd_with_url' })
      const protectedHandler = await payable.function(handler)

      await protectedHandler({ auth: { customer_ref: 'cus_with_url' } })

      const callArgs = checkLimitsSpy.mock.calls[0][0]
      expect(callArgs).toMatchObject({ includeCheckoutSession: true })
    })
  })
  ...
  describe('checkLimits Cache', () => {
    ...
    it('coalesces concurrent checkLimits and consumes one unit per caller', async () => {
      ...
      const handler = vi.fn().mockResolvedValue({ success: true })
      const payable = solvaPay.payable({ product: 'cache-concurrent' })
      const protectedHandler = await payable.function(handler)

      const args = { auth: { customer_ref: 'cus_concurrent' } }
      const first = protectedHandler(args)
      const second = protectedHandler(args)
      const third = protectedHandler(args)
      const fourth = protectedHandler(args)

      resolveLimits({ withinLimits: true, remaining: 3, plan: 'free' })

      await expect(first).resolves.toEqual({ success: true })
      await expect(second).resolves.toEqual({ success: true })
      await expect(third).resolves.toEqual({ success: true })
      await expect(fourth).rejects.toBeInstanceOf(PaywallError)
      expect(checkLimitsSpy).toHaveBeenCalledTimes(1)
  ...
  describe('SolvaPayPaywall.decide()', () => {
    ...
    it('returns an allow decision with limits + customerRef when within limits', async () => {
      ...
      expect(decision.requestId).toMatch(/^solvapay_/)
