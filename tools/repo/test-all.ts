/**
 * Aggregate tester: core tier by default, `--native` adds bindings.
 */

import path from 'node:path'
import { fileURLToPath } from 'node:url'
import { parseSelectFlags, selectTestTasks } from '../shared/surfaces.js'
...
function printUsage(): string {
  return `Usage:
  pnpm test:all [--native | --native-only] [--only <surface>] [--json] [--bail]
`
}

export async function runCli(argv: string[], deps?: Partial<RunnerDeps>): Promise<CliResult> {
  const selected = selectTestTasks(argv)
  ...
  const summary = await runTasks(
    selected,
    { command: 'test:all', json: flags.json, bail: flags.bail },
    deps,
  )
