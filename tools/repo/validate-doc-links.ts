...
import { DOCS_DIR, REPO_ROOT } from '../shared/paths.js'
...
function checkLink(
  ...
  if (stat.isDirectory()) {
    ...
    const hasIndex = indexFiles.some(file => existsSync(join(resolvedPath, file)))
...
function validateLinks(skipApiValidation: boolean = false): LinkIssue[] {
  ...
  if (skipApiValidation) {
    console.log(
      'ℹ️  Skipping validation for generated API doc links (validated after docs build)\n',
    )
  }
