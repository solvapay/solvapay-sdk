...
fn public_client_helpers_and_rbs_match_committed() {
    ...
    assert_eq!(
        emitted.client_rb,
        fs::read_to_string(support::paths().generated_path("rbClient").expect("rbClient")).expect("client")
    );
    assert_eq!(
        emitted.helpers_rb,
        fs::read_to_string(support::paths().generated_path("rubyHelpers").expect("rubyHelpers"))
            .expect("helpers")
    );
