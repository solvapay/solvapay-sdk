...
use solvapay_core::{
    ...
    derive_icons, mcp_tool_names_json, validate_public_base_url,
...
pub(crate) fn invoke_mcp_tool_names(_input: &FixtureInput) -> Result<Value, BindingError> {
    Ok(mcp_tool_names_json())
}

/// Binding for `deriveIcons` — absent branding → JSON `null`.
pub(crate) fn invoke_derive_icons(input: &FixtureInput) -> Result<Value, BindingError> {
...
pub(crate) fn invoke_build_tool_descriptor_metadata(
    input: &FixtureInput,
...
pub(crate) fn invoke_build_prompt_descriptor_metadata(
    input: &FixtureInput,
) -> Result<Value, BindingError> {
    let views = optional_views_arg(input)?;
...
pub(crate) fn invoke_build_prompt_user_message(
    input: &FixtureInput,
) -> Result<Value, BindingError> {
