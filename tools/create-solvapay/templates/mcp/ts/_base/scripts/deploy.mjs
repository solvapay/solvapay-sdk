...
function readWranglerConfig() {
  ...
  const hasCustomDomainRoute = /"routes"\s*:\s*\[[\s\S]*?"custom_domain"\s*:\s*true/i.test(contents)
...
function looksLikePlaceholderUpstreamKey(value) {
  ...
  if (/^(your|placeholder|example|sample|test|demo|todo|fake|dummy)-api-key$/i.test(value))
    return true
...
function ensureMcpPublicBaseUrl(localEnv, preflight, { dryRun = false } = {}) {
  ...
  if (!name || name.includes('__')) {
    console.warn(
      '⚠  Could not read worker name from wrangler.jsonc — skipping MCP_PUBLIC_BASE_URL auto-resolution.',
    )
