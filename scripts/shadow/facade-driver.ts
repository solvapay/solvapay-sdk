/**
 * Facade-side shadow driver: camelCase dispatch over the published
 * `@solvapay/server` npm facade (`createSolvaPayClient`) with a fetch recorder.
 *
 * Step 53: the facade client is Rust-only. There is no TypeScript `fetch` body
 * to record anymore, so this driver injects the real `@solvapay/server-wasm`
 * `WasmClient` (via the exported test seam) and forces the WASM `FetchTransport`
 * path under Node. `FetchTransport` calls `globalThis.fetch(request)`, which the
 * recorder intercepts — so the shadow comparison is now
 * "npm facade binding (WASM) vs Rust CLI (shadow-invoker)".
 */
...
export type ShadowOpName = (typeof FACADE_SHADOW_OPERATION_NAMES)[number]

export type FacadeDriverOptions = {
  apiKey: string
  apiBaseUrl: string
...
/**
 * Loads the real `@solvapay/server-wasm` binding and installs its `WasmClient`
 * as the facade override for `config`. Clearing the cache first ensures a fresh
...
/**
 * Create a facade driver that records fetch exchanges per invoke.
 */
export function createFacadeShadowDriver(options: FacadeDriverOptions): FacadeDriver {
  ...
  // The WASM `FetchTransport` uses global fetch — install it for invokes.
  ...
  // Warm + install the WASM facade override lazily (async) on first invoke so
  // the synchronous operation-name registration path stays dependency-free.
  let facadeReady: Promise<void> | undefined
  const ensureFacadeReady = (): Promise<void> => {
    if (!facadeReady) {
      facadeReady = installFacadeClient({
        apiKey: options.apiKey,
        apiBaseUrl: options.apiBaseUrl,
      })
    }
  ...
  return {
    ...
    async invoke(fn, args) {
      await ensureFacadeReady()
      globalThis.fetch = recordingFetch
      wireBucket.length = 0
      const dispatcher = dispatchers[fn]
      if (!dispatcher) {
        throw new Error(`No shadow facade dispatcher for fn: ${fn}`)
      }
      try {
...
// Ensure we can restore the original after createFacadeShadowDriver patches it.
