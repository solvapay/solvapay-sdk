...
| `customerCountry` | `string` | Buyer billing country. Always sent. Copied onto `country` as well when `isBusiness` is `true` |
| `customerName` | `string` | Consumer name |
| `customerState` | `string` | Billing state or province. Required for US, CA, and IN |
| `customerPostalCode` | `string` | Billing postal or ZIP code. Required for US, CA, and GB |
...
For businesses, `country` is required and must be supported; the tax ID format is validated per
country. `customerCountry` is always sent — copied from `country` when the buyer did not set it
separately. For consumers, `customerCountry` must be a supported country.
...
`isBusiness`, and the optional `businessName` / `country` / `customerCountry` / `customerState` /
`customerPostalCode` / `taxId` / `taxIdType` fields, resolves the authenticated customer, and
returns the `taxBreakdown`. The embedded checkout views in `@solvapay/react/mcp` call it
automatically — no extra wiring is needed.
