...
A usage event is one recorded data point against a meter — an API request, token spend, or
document generation. The HTTP path stays `POST /v1/sdk/meter-events`. See
[Plans and billing glossary](/plans/glossary) for the meter noun and credit peg.
