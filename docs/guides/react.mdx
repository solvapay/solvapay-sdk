...
allowed per UTC calendar month. It is **API-only** — there's no field in the
`<AutoRecharge>` form — so set it through
`useAutoRecharge().save({ ..., maxMonthlySpendMajor })` (or the route directly).
When the cap is hit mid-month, the config stays `active` and charges resume
automatically next UTC month. Use `config.monthlySpendMinor` and
`config.monthlySpendPeriod` to show current-period spend.
