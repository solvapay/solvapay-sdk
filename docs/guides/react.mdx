...
balance **threshold** and a **fixed top-up amount**. When enabled, SolvaPay
charges the saved card off-session and mints credits whenever the balance falls
below the threshold — no manual checkout.
...
and the 3DS redirect return internally. If a series of off-session charges keeps
...
- `save(input)` — set or update the config. Accepts `SaveAutoRechargeInput` (see the [top-up section](#credit-top-ups--auto-recharge) for the `maxRecharges` cap and `deferSetupIntent`).
- `disable()` — turn auto-recharge off.

`config.status` is `active`, `disabled`, `pending_setup`, `failed`, or
`completed` (the last is returned once a `maxRecharges` cap is reached). Read
...
An optional `maxRecharges` cap limits the total number of automatic recharges.
It is **API-only** — there's no field in the `<AutoRecharge>` form — so set it
through `useAutoRecharge().save({ ..., maxRecharges })` (or the route directly).
Once the cap is reached, the config's status becomes `completed` and no further
recharges occur.
