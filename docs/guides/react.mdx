...
and the 3DS redirect return internally. Expand **Advanced** to set an optional
`maxMonthlySpendMajor` cap (blank = unlimited). The summary card shows current-period
...
`AutoRecharge.AdvancedToggle`, `AutoRecharge.MaxMonthlySpendField`,
