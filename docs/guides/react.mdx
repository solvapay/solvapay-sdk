...
are never used for top-ups. The provider's **minimum top-up** (`minTopUpMinor` in settings) is
enforced on every top-up and auto-recharge — amounts below it are rejected. `useCheckoutFlow`
exposes `topupCurrencies`, `topupCurrency`, and `setTopupCurrency` for building a currency picker
on the amount step; pass the chosen currency to `TopupForm.Root` / `useTopup`.
...
`10,000` major units. The top-up amount must also clear the higher of Stripe's
per-currency minimum (for example `$0.50` for USD/EUR/CHF/CAD/AUD, `£0.30` for GBP,
`30 kr` for SEK/NOK, `2.50 kr` for DKK) and the provider's **minimum top-up**.
The currency must be one of the provider's supported top-up currencies.
