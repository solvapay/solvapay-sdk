...
Returns `{ status, purchaseRef?, checkoutUrl?, creditBalance?, ... }`. See [Purchase lifecycle management](./purchase-management) for all status values.
...
Returns `{ plans, productRef }`. Pass `?productRef=prd_...` as a query parameter.
