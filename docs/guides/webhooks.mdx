...
const seen = new Set<string>() // replace with Redis / your DB
...
Signature verification is not replay protection. Dedupe on `event.id` (or pass
`seenEventId` to `verifyWebhook` / `solvapayWebhook`). Return HTTP 2xx for a
duplicate so SolvaPay stops retrying. `SV-Delivery` identifies the attempt, not
the event.
...
| **Credits** | `customer.credit.topped_up`, `customer.credit.debited`, `customer.credit.low_balance`, `customer.credit.exhausted`, `customer.credit.granted`, `customer.credit.adjusted`, `customer.credit.auto_topup_failed` |
