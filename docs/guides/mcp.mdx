...
Full contract: [Add a free allowance to MCP tools](/sdks/typescript/guides/free-allowance). The [mcp-checkout-app](https://github.com/solvapay/solvapay-sdk/tree/main/examples/typescript/mcp-checkout-app) example ships the two preview tools above.
...
- **Text-only hosts** — pass `hideToolsByAudience: ['ui']` to keep the LLM-facing `tools/list` narrow to the two intent tools (`account`, `activate_plan`) plus your own data tools, while leaving the UI transport tools callable from the SolvaPay iframe.
...
- [Add a free allowance to MCP tools](/sdks/typescript/guides/free-allowance) — `registerFree` for capped free tools
