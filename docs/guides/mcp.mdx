...
```bash
npm install @solvapay/server @solvapay/mcp-core @solvapay/mcp @solvapay/mcp-express \
  @modelcontextprotocol/sdk @modelcontextprotocol/ext-apps
# or
pnpm add @solvapay/server @solvapay/mcp-core @solvapay/mcp @solvapay/mcp-express \
  @modelcontextprotocol/sdk @modelcontextprotocol/ext-apps
# or
yarn add @solvapay/server @solvapay/mcp-core @solvapay/mcp @solvapay/mcp-express \
  @modelcontextprotocol/sdk @modelcontextprotocol/ext-apps
```
...
```bash
pnpm add @solvapay/server @solvapay/mcp-core @solvapay/mcp @solvapay/mcp-fetch \
  @modelcontextprotocol/sdk @modelcontextprotocol/ext-apps
```
...
- **`@solvapay/mcp`** — official `@modelcontextprotocol/sdk` + `ext-apps` adapter. Exports `createSolvaPayMcpServer` and `registerPayableTool`. Only package that imports `@modelcontextprotocol/*`.
- **`@solvapay/mcp-express`** — Node `(req, res, next)` OAuth bridge middleware (`createMcpOAuthBridge`).
- **`@solvapay/mcp-fetch`** — Web-standards `(Request) => Promise<Response>` OAuth bridge + turnkey MCP handler (`createSolvaPayMcpFetchHandler`) for Deno / Supabase Edge / Cloudflare Workers / Bun / Next edge / Vercel Functions.
