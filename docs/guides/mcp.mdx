...
```bash
npm install @solvapay/server @solvapay/mcp @solvapay/mcp-sdk \
  @modelcontextprotocol/sdk @modelcontextprotocol/ext-apps
# or
pnpm add @solvapay/server @solvapay/mcp @solvapay/mcp-sdk \
  @modelcontextprotocol/sdk @modelcontextprotocol/ext-apps
# or
yarn add @solvapay/server @solvapay/mcp @solvapay/mcp-sdk \
  @modelcontextprotocol/sdk @modelcontextprotocol/ext-apps
```
...
- **`@solvapay/mcp`** — framework-neutral MCP contracts: `MCP_TOOL_NAMES`, `buildSolvaPayDescriptors`, `buildPayableHandler`, `createMcpOAuthBridge`, JWT bearer helpers. No `@modelcontextprotocol/*` dependency.
- **`@solvapay/mcp-sdk`** — official `@modelcontextprotocol/sdk` + `ext-apps` adapter. Exports `createSolvaPayMcpServer` and `registerPayableTool`. Only package that imports `@modelcontextprotocol/*`.

Skip `@solvapay/mcp-sdk` + `@modelcontextprotocol/*` if you only need the OAuth bridge / bearer helpers, or if you're adapting SolvaPay onto a non-official MCP server (mcp-lite, fastmcp-node, raw JSON-RPC).
