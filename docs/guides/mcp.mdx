...
Install the required packages. Pick the runtime-specific OAuth bridge based on where you're deploying.

### Node.js (Express, Fastify, etc.)

```bash
npm install @solvapay/server @solvapay/mcp-core @solvapay/mcp \
  @modelcontextprotocol/sdk @modelcontextprotocol/ext-apps
# or
pnpm add @solvapay/server @solvapay/mcp-core @solvapay/mcp \
  @modelcontextprotocol/sdk @modelcontextprotocol/ext-apps
# or
yarn add @solvapay/server @solvapay/mcp-core @solvapay/mcp \
  @modelcontextprotocol/sdk @modelcontextprotocol/ext-apps
```

Import the Node OAuth bridge from the `@solvapay/mcp/express` subpath.

### Web-standards runtimes (Deno, Supabase Edge, Cloudflare Workers, Bun, Next edge, Vercel Functions)
...
Import the fetch-first OAuth bridge + turnkey handler from the `@solvapay/mcp/fetch` subpath.

### Package split

- **`@solvapay/server`** — paywall core, `createSolvaPay`, `PaywallError`, webhook verification.
- **`@solvapay/mcp-core`** — framework-neutral MCP contracts: `MCP_TOOL_NAMES`, `buildSolvaPayDescriptors`, `buildPayableHandler`, OAuth discovery JSON builders, JWT bearer helpers. No `@modelcontextprotocol/*` dependency.
- **`@solvapay/mcp`** — official `@modelcontextprotocol/sdk` + `ext-apps` adapter. The only package that imports `@modelcontextprotocol/*`. Three subpath exports:
  - `@solvapay/mcp` (root) — `createSolvaPayMcpServer` + `registerPayableTool`.
  - `@solvapay/mcp/express` — Node `(req, res, next)` OAuth bridge middleware (`createMcpOAuthBridge`).
  - `@solvapay/mcp/fetch` — Web-standards `(Request) => Promise<Response>` OAuth bridge + turnkey MCP handler (`createSolvaPayMcpFetch`, `createSolvaPayMcpFetchHandler`) for Deno / Supabase Edge / Cloudflare Workers / Bun / Next edge / Vercel Functions.

Skip `@solvapay/mcp` + `@modelcontextprotocol/*` if you only need the OAuth bridge / bearer helpers, or if you're adapting SolvaPay onto a non-official MCP server (`fastmcp`, raw JSON-RPC).
...
If your MCP server runs on localhost/custom domain (not SolvaPay hosted MCP proxy), the
`createMcpOAuthBridge` helper turns your MCP origin into a fully self-consistent OAuth
authorization server (RFC 8414 §3.3). All OAuth endpoints are served from your MCP origin
and transparently proxied to SolvaPay:

| Endpoint (on MCP origin)                         | Proxied to                                         |
| ------------------------------------------------ | -------------------------------------------------- |
| `GET /.well-known/oauth-protected-resource`      | local (`authorization_servers: [mcpOrigin]`)       |
| `GET /.well-known/oauth-authorization-server`    | local (`issuer` = MCP origin, endpoints on origin) |
...
`product_ref` is injected automatically at Dynamic Client Registration, so your MCP clients
do not need to know about it. `/mcp` is still protected with bearer auth and returns an
RFC9728 `WWW-Authenticate` challenge when the token is missing or invalid.
...
// /oauth/token uses application/x-www-form-urlencoded
...
- **`GET /mcp` must return 405, not 400.** Cursor (and other Streamable HTTP clients) opens
  a server-initiated SSE back-channel with `GET /mcp` + `Accept: text/event-stream` right
  after OAuth completes. Stateless MCP servers can't serve it; respond `405 Method Not
  Allowed` with `Allow: POST, OPTIONS` and the client stays connected. `createMcpOAuthBridge`
  does this for you — if your router forwards GETs to a different transport, make sure the
  fallback returns 405, not 400, or the client will transition to `failed` on
  "Failed to open SSE stream: Bad Request".
- **`/.well-known/openid-configuration` must return 404.** Cursor attempts OIDC Discovery
  before RFC 8414. `createMcpOAuthBridge` returns 404 on this path for you. If you don't
  use the bridge, add a handler that explicitly returns 404 — a JSON response missing
  `jwks_uri`, `subject_types_supported`, or `id_token_signing_alg_values_supported` fails
  Cursor's OIDC validator and logs a noisy Zod error. We return 404 (rather than a full
  OIDC doc) because SolvaPay is an OAuth 2.0 authorization server (RFC 8414), not an
  OpenID Provider: we don't issue `id_token`s or expose a JWKS endpoint, so advertising
  OIDC would be a false capability claim that could break strict OIDC clients later in
  the flow. Only serve `/.well-known/oauth-authorization-server` and
  `/.well-known/oauth-protected-resource` from your MCP origin.
