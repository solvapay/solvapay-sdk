...
- [Free tools with a usage cap](#free-tools-with-a-usage-cap)
...
## Free tools with a usage cap

`registerFree` is the SDK-only path for an otherwise-free tool with a per-customer cap declared in code. Exhaustion emits the same paywall gate as `registerPayable`. Managed MCP public tools stay unlimited.
...
additionalTools: ({ registerPayable, registerFree }) => {
  ...
  registerFree('preview_company_profile', {
    description: 'Name and sector preview. Shares the free-previews allowance.',
    schema: { symbol: z.string() },
    limit: PREVIEW_ALLOWANCE,
  ...
  registerPayable('get_market_quote', {
    description: 'Full market quote.',
    schema: { symbol: z.string() },
    handler: async ({ symbol }, ctx) => ctx.respond(await getQuote(symbol)),
  })
}
```

Naming the same `limit.meter` is what shares the allowance — five calls total across both preview tools, not five each. Omitting `meter` defaults to `free-requests`. Unidentified callers fail with `401`; there is no anonymous bucket. Reserve **included** for a plan’s included amount; a free allowance is the **Free** series on **Usage**.

Full contract: [Add a free allowance to MCP tools](/sdks/typescript/guides/free-allowance). The [mcp-checkout-app](https://github.com/solvapay/solvapay-sdk/tree/main/examples/mcp-checkout-app) example ships the two preview tools above.
  ...
  handler: async ({ range }, ctx) => {
    ...
    return ctx.respond(
      { range, results },
      {
        ...
        nudge: {
          kind: 'low-balance',
          message: 'Running low on credits — call the `account` tool with view: "topup" to add more.',
        },
      },
    )
  },
...
- `text` — override `content[0].text`. The default is a short `"Success"` narration; merchant data rides on `structuredContent`.
- `nudge` — the inline upsell suffix shown above.
- `dataInText` — append the serialized payload as a trailing text block. Silent `ctx.respond(data)` does **not** dump the same object into `content[0].text` (hosts that feed both fields to the model would see duplicate JSON). When you pass `text` or `nudge`, a trailing JSON block is still appended unless this is `false`. Set `true` to opt in even without narration.
...
registerPayable('search_knowledge', {
  description: 'Search the knowledge base.',
  schema: { query: z.string().min(1) },
  outputSchema: z.object({ query: z.string(), results: z.array(z.string()) }),
