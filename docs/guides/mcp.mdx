...
A compact, copy-paste-safe Express server mirroring [`examples/typescript/mcp-checkout-app`](https://github.com/solvapay/solvapay-sdk/tree/main/examples/typescript/mcp-checkout-app): the OAuth bridge, one `registerPayable` tool, and the streamable HTTP transport.
