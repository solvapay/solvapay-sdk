...
- A SolvaPay account with a product and at least one plan configured in the [SolvaPay Console](https://app.solvapay.com)
...
| `get-history` | GET | `getHistory` | Past charges for a product plus account-wide credit activity (`?productRef=…&limit=…`) |
...
The full reference project is available at [`examples/typescript/supabase-edge`](https://github.com/solvapay/solvapay-sdk/tree/main/examples/typescript/supabase-edge) in the SDK repository.
