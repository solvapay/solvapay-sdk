...
See the runnable reference: [express-provider-linkage example](https://github.com/solvapay/solvapay-sdk/tree/main/examples/typescript/express-provider-linkage).
...
- Example: [examples/typescript/nextjs-auth0](https://github.com/solvapay/solvapay-sdk/tree/main/examples/typescript/nextjs-auth0)
...
- Do **not** assume SolvaPay will host your customer login — use [customer OAuth](/no-code-mcp/authentication) only when **your** OAuth clients need SolvaPay as the authorization server (e.g. MCP / third-party apps), which is separate from provider-owned CLI linkage.
