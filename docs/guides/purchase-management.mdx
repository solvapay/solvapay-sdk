...
| `maxRecharges` | `number` | No | Optional lifetime cap on automatic recharges |
...
- **`maxRecharges` completion.** When the cap is reached, the config's status becomes `completed` and no further recharges occur.
