...
Validation enforces both amounts `<= 10,000` major units and a floor of the higher of Stripe's
per-currency minimum (for example `$0.50` for USD/EUR/CHF/CAD/AUD, `£0.30` for GBP) and the
provider's **minimum top-up** (`minTopUpMinor` in SolvaPay Console settings). Amounts below that
floor are rejected.
