...
| [mcp-time-app](https://github.com/solvapay/solvapay-sdk/tree/dev/examples/mcp-time-app)                                         | MCP App    | N/A (Paywall only) | OAuth + Bearer   | MCP app UI + tools  | Intermediate   |
| [mcp-oauth-bridge](https://github.com/solvapay/solvapay-sdk/tree/dev/examples/mcp-oauth-bridge)                                 | MCP Server | N/A (Paywall only) | OAuth + Bearer   | Non-hosted MCP OAuth bridge | Advanced |
...
### 4. MCP Time App (`mcp-time-app`)

**Purpose**: Demonstrates an MCP App with an interactive UI resource and paywall-protected tool calls.

**Key Features**:

- MCP app integration with `@modelcontextprotocol/ext-apps`
- Tool protection with paywall
- Interactive app resource (`mcp-app://` UI)
- OAuth discovery bridge integration
- Bearer token protected MCP endpoint

**Best For**:

- MCP app developers
- Teams building UI-rich MCP experiences
- Tool access control with OAuth identity
- Usage-based pricing models

**Documentation**: [mcp-time-app README](https://github.com/solvapay/solvapay-sdk/tree/dev/examples/mcp-time-app/README.md)

---

### 5. MCP OAuth Bridge (`mcp-oauth-bridge`)

**Purpose**: Demonstrates a non-hosted MCP server that serves local discovery metadata and delegates OAuth/DCR to SolvaPay backend.

**Key Features**:

- Local `/.well-known/oauth-protected-resource`
- Local `/.well-known/oauth-authorization-server`
- Bearer-protected `/mcp` with RFC9728 challenge
- Dynamic client registration with required `mcp_server_id`
- `payable.mcp()` with OAuth-derived customer identity

**Best For**:

- Non-hosted MCP deployments
- MCP OAuth discovery bridging patterns
- Teams that want backend-owned OAuth with local MCP runtime

**Documentation**: [mcp-oauth-bridge README](https://github.com/solvapay/solvapay-sdk/tree/dev/examples/mcp-oauth-bridge/README.md)
...
- **Building an MCP App with UI?** → Use `mcp-time-app`
- **Building non-hosted MCP + OAuth bridge?** → Use `mcp-oauth-bridge`
...
**Location**: `examples/mcp-time-app/`
...
- MCP app server + UI resource integration
- Tool protection with `payable.mcp()`
- OAuth discovery bridge setup
- Interactive app resource rendering

**Requires**: SolvaPay backend OAuth endpoints and MCP server reference (`MCP_SERVER_ID`)

---

### MCP OAuth Bridge

**Location**: `examples/mcp-oauth-bridge/`
...
- OAuth + dynamic client registration via SolvaPay backend
- Bearer-token protected MCP endpoint
- `payable.mcp()` with custom customer extraction
...
Begin with `express-basic` to understand core paywall concepts, then move to `mcp-time-app` or `mcp-oauth-bridge` for MCP-specific patterns.
