...
- Publishes seven packages: `@solvapay/core`, `@solvapay/react`, `@solvapay/react-supabase`, `@solvapay/server`, `@solvapay/auth`, `@solvapay/next`, and `solvapay`
...
The CLI npm package is `solvapay`.
The CLI command is `solvapay`.
...
- Published packages are limited to the six SDK packages plus the `solvapay` CLI
