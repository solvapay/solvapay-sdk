...
- Publishes the SDK packages on a shared version line: `@solvapay/core`, `@solvapay/react`, `@solvapay/react-supabase`, `@solvapay/server`, `@solvapay/auth`, `@solvapay/next`, and `solvapay`
- Also publishes two CLI packages on **independent semver lines** (no fixed versioning): `@solvapay/cli-core` (the shared lib consumed by `solvapay init` and `create-paid-mcp-app`) and `create-paid-mcp-app` (the `npm create paid-mcp-app` scaffolder)
- Uses **fixed versioning** for the SDK packages - they share the same version number
...
- When you want `package.json` to reflect the current published state
- After publishing multiple versions without syncing
...
- If you're working on `dev` branch (sync on `main` only)

**Note**: The public documentation is built in a separate docs repository that syncs content from this SDK repository using a checkout-and-copy workflow. Version syncing in this repo is only needed if you want `package.json` files to match published versions for consistency.
...
3. **Tags** all published packages as "latest"
...
| npm package | binary | invocation |
| --- | --- | --- |
| `solvapay` | `solvapay` | `npx solvapay init` — auth + env + product picker in an existing project |
| `create-paid-mcp-app` | `create-paid-mcp-app` | `npm create paid-mcp-app <name>` — scaffold a fresh paid MCP server |
| `@solvapay/cli-core` | _(library, no bin)_ | internal — shared lib consumed by the two CLIs above |

### CLI release lines

`solvapay`, `@solvapay/cli-core`, and `create-paid-mcp-app` evolve on **independent semver lines** (no `linked` entry in `.changeset/config.json`). The SDK fixed-version workflow does not apply to them — bump them via changesets only when the corresponding `packages/cli*` or `packages/create-paid-mcp-app/` directory changes.

## SDK internals (maintainers)

This section is maintainer-focused. It documents repository internals that are useful when
working on the SDK itself, not when integrating the SDK in an app.
...
│  ├─ mcp
│  ├─ mcp-core
│  ├─ cli                  (published as `solvapay`)
│  ├─ cli-core             (published as `@solvapay/cli-core`)
│  ├─ create-paid-mcp-app  (published as `create-paid-mcp-app`)
│  ├─ demo-services
│  ├─ test-utils
│  └─ tsconfig
├─ docs/
...
- `turbo` orchestrates build, test, lint, and dev commands across packages
- `tsup` builds ESM, CJS, and types for publishable packages
- Node engine target is `>=18.17`
- TypeDoc output is validated in CI (`pnpm docs:check`)

### Internal package boundaries

- `@solvapay/demo-services`, `@solvapay/test-utils`, and `@solvapay/tsconfig` are private helper
  packages used for examples, testing, and shared config
- Published packages are the six SDK packages plus three CLI-side packages (`solvapay`, `@solvapay/cli-core`, `create-paid-mcp-app`)

### Documentation sync model

Public docs are built in a separate docs repo.
TypeScript SDK pages are synced from this repo (`solvapay-sdk/docs`) into that docs repo with a
checkout-and-copy workflow.
