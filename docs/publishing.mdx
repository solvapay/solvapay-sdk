...
- Also publishes two CLI packages on **independent semver lines** (no fixed versioning): `@solvapay/init` (the shared lib consumed by `solvapay init` and `create-solvapay`) and `create-solvapay` (the `npm create solvapay` scaffolder)
...
| `create-solvapay` | `create-solvapay` | `npm create solvapay <name> -- --type mcp` — scaffold a fresh paid MCP server |
| `@solvapay/init` | _(library, no bin)_ | internal — shared lib consumed by the two CLIs above |

### CLI release lines

`solvapay`, `@solvapay/init`, and `create-solvapay` evolve on **independent semver lines** (no `linked` entry in `.changeset/config.json`). The SDK fixed-version workflow does not apply to them — bump them via changesets only when the corresponding `tools/cli/`, `tools/init/`, or `tools/create-solvapay/` directory changes.
...
```text
solvapay-sdk/
├─ sdks/typescript/        # published @solvapay/* TypeScript packages
│  ├─ core
│  ├─ server
│  ├─ react
│  ├─ react-supabase
...
│  ├─ init                 (published as `@solvapay/init`)
│  └─ create-solvapay      (published as `create-solvapay`)
├─ internal/
│  ├─ demo-services
│  ├─ test-utils
│  └─ tsconfig
...
- Published packages are the six SDK packages plus three CLI-side packages (`solvapay`, `@solvapay/init`, `create-solvapay`)
