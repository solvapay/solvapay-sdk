...
`solvapay`, `@solvapay/init`, and `create-solvapay` evolve on **independent semver lines** (no `linked` entry in `.changeset/config.json`). The SDK fixed-version workflow does not apply to them — bump them via changesets only when the corresponding `packages/cli/`, `packages/init/`, or `packages/create-solvapay/` directory changes.
