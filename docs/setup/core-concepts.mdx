...
- **Usage-based (PAYG)** — topup-first: returns `topup_required` at zero balance and only `activated` once the customer's credit balance covers at least one unit. Route the customer through a top-up (`createTopupPaymentIntent`) first, then re-run `activatePlan` to create the active purchase.
- **Recurring / hybrid** — return `payment_required` when the customer has no card on file. Route them to checkout before granting access.
