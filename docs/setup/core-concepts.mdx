...
All three are exposed as route-wrapper helpers in `@solvapay/next` (`getMerchant`, `getProduct`, `getPaymentMethod`) and as fetch handlers in `@solvapay/server/fetch`. On MCP they are **not** tools: `createSolvaPayMcpServer` folds this data into the bootstrap payload that each intent tool returns, so the MCP App UI reads it without a round-trip. See the [MCP App guide](../guides/mcp-app).
...
- [Customer linkage (bring your own client and IdP)](/sdks/typescript/guides/customer-linkage) — `ensureCustomer(customerRef, externalRef?, options?)` from any backend
