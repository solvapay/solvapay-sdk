...
A plan is an `options[]` list. These names are **derived labels**, not a `type` you send:

| Label | Derived when | Best for |
| --- | --- | --- |
| Recurring | Billing cycle, no meter | SaaS subscriptions |
| Usage-based | Metered, no billing cycle | APIs and token services |
| Hybrid | Billing cycle and a meter | Mixed workloads |
| One-time | Flat charge, no cycle | Digital goods and lifetime access |

See [Plans](/plans/overview) to compose those options.

## Meters

Meters define what usage to measure.

- Seeded default: `requests` (`name` / `displayName` / `unit`)
- MCP tool meters: `tool:{toolName}`
- Custom meters for business-specific usage

Plans can enforce limits using meter data. Copy uses the meter's `unit` noun — see [Plans and billing glossary](/plans/glossary).
...
Three read-only endpoints power customer-facing UI without bloating `checkPurchase`:
...
- **`GET /v1/sdk/payment-method?customerRef=…`** — returns `{ kind: 'card', brand, last4, expMonth, expYear, reusable }` or `{ kind: 'none' }`. The card brand and last4 are mirrored onto the Customer by the `payment_intent.succeeded` webhook, so this endpoint reads from SolvaPay's database with no Stripe round-trip. `reusable` distinguishes a chargeable saved card from a card that is only on file. Safe to poll alongside `checkPurchase`.

All three are exposed as route-wrapper helpers in `@solvapay/next` (`getMerchant`, `getProduct`, `getPaymentMethod`) and as fetch handlers in `@solvapay/server/fetch`. In an MCP server they are not separate tools — `createSolvaPayMcpServer` delivers the same data on the `account` viewer's bootstrap payload, which seeds the client caches at mount.
...
- [Customer linkage (bring your own client and IdP)](/sdks/typescript/guides/customer-linkage) — `ensureCustomer(externalRef)` from any backend
