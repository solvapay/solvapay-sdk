...
**Current progress (2026-07-24):** Steps 1–38 **Done**; Step **37R Done** (a–e); Step **38R Done** (a–g); Step 39 **In progress** (local host-native + WASI clean-install green; awaiting full CI matrix); Step **39G-a/b/c Done** (§15 notes 40–42); Steps **40–42 Done** — Phase 7 **closed** (§15 notes 43–45); Steps **43–45 Done** — Phase 8 **closed** (§15 notes 46–48); Steps **46–48 Done** — Phase 9 **closed** (§15 notes 49/53); Step **18T Done** — shared IR doc model + TSDoc + coverage CI gate (§15 note 50); Step **42T Done** — Python `.pyi` + `py.typed` + `mypy`/`pyright`/`ruff` strictness + docstring coverage (§15 note 51); Step **45T Done** — Ruby YARD + `steep==2.0.0` / `rubocop==1.88.2` strictness (§15 note 52); Step **47 Done** — generated Rust facade + blocking twins + signature-parity + Phase 0 fixture conformance + rustdoc; Steps **49–51 Done** — Phase 10 **closed**: wazero (Go) binding + full facade + live contract + subtree module release (§15 notes 54–56). **D20 / §9 Examples deliverable Done**. Step **52 Done** — Rust-only `@solvapay/core` facade (domains + helpers); eager browser WASM; React napi setup; §15 note 57. Step **53 Done** — Rust-only `@solvapay/server` facade (webhook napi/WASM, decisions, client); deleted TS bodies; new `server-superseded-ts` gate; §15 note 59. Step **54 Done** — optional C ABI scaffold (`solvapay-c` + cbindgen + C smoke); §15 note 58. Phase 6 closes when Step 39 CI is green. Phase 6G **closed**. Strict-typing retrofit track closed for TS/Python/Ruby; Rust/Go doc columns closed. **Next:** Step **55** (promote all compatibility gates) or MCP-authoring track. **Known-not-green:** `parity:check` reports 24 uncatalogued core-surface extras from the Step 52 core refactor (pre-existing; unchanged by Step 53).
...
| 49 | Scaffold wazero binding | Phase 10 | Done | — | `cargo test -p dto-gen --test go_shim_golden` (allowlisted `getMerchant`); `cargo build -p solvapay-go-wasm --target wasm32-wasip1 --profile wasm-release`; `build-wasm.sh` + committed `solvapay_core.wasm` staleness gate; `go test -race ./...` (Version + VerifyWebhook `errors.As` + GetMerchant host transport + concurrent pool + ctx-cancel teardown); CI `go-binding` job (§15 note 54) | [Phase 10](#phase-10--go-wazero--embedded-wasm) |
| 50 | Generate the Go facade + signature-parity suite (+ generated binding glue, §5.7) | Phase 10 | Done | — | Full 36-op Go guest shims + `emit_client_go` / `emit_parity_suite_go` → `client_generated.go` + `signature_parity_generated_test.go`; Phase 0 fixture conformance via `net/http` stub; godoc from IR; CI regen-drift + `@generated` + `go test -race` (§15 note 55) | [Phase 10](#phase-10--go-wazero--embedded-wasm) |
| 51 | Live contract tests + go module release wiring | Phase 10 | Done | — | `internal/livecontract` + `cmd/live-contract` + `shadow-go.yml` (dispatch-only); `publish-go.yml` subtree-split dry-run-default → `solvapay/solvapay-go`; tested `examples/go/get-merchant`; CI example + fresh-module smoke + `go-crosscompile` GOOS/GOARCH matrix; no skew guard (embedded wasm) (§15 note 56) | [Phase 10](#phase-10--go-wazero--embedded-wasm) |
| 52 | Delete superseded TS in `@solvapay/core` | Post-cutover | Done | — | Rust-only core facade (3 domains + 12 helpers); React napi setup + eager browser WASM; `test:unit:ts` aliased to rust; delegation allowlist trimmed; budgets re-recorded; §15 note 57 | [Post-cutover](#post-cutover--deletion-and-c-abi) |
| 53 | Delete superseded TS in `@solvapay/server` | Post-cutover | Done | — | Rust-only server facade; deleted webhook/paywall/retry/client TS bodies; napi optional `nowUnixSecs`; WASM fixture/shadow wire; `test:unit:ts`→rust alias; `server-superseded-ts:check`; GREEN: server 356 / contract 1193 / shadow 19 / fixture-runner 431/431 / node 25 / edge-wasm 10 / edge unit 72; §15 note 59 | [Post-cutover](#post-cutover--deletion-and-c-abi) |
| 54 | Publish the optional C ABI | Post-cutover | Done | — | `cargo test -p solvapay-c` 18; clippy `-D warnings`; `./bindings/c/ctest/run.sh` create/call/free + handle-misuse PASS; cbindgen drift + `c-binding` CI job; §15 note 58 | [Post-cutover](#post-cutover--deletion-and-c-abi) |
...
- **Flag / rollback:** edge reads `SOLVAPAY_IMPL` via guarded `globalThis.process` / `Deno.env` (never `node:process` in the edge graph). unset/`rust` → WASM; `ts` → fail-fast after Step 53 (Web Crypto helper deleted — note 59).
...
- **Deferred (resolved at Step 53 where applicable):** deleting Web Crypto ✅; Node napi fixture-clock injection ✅ (optional `nowUnixSecs` + host `Date.now`); contract harness no longer forces `SOLVAPAY_IMPL=ts` for webhooks/client. Remaining historical deferrals that moved elsewhere: full browser `client-public` API methods.
...
- **Harness (updated Step 53):** `createDefaultRegistry` binds surfaces to server wrappers under `withRustImpl`. Client fixtures inject WASM `WasmClient` for mocked `globalThis.fetch`; webhook fixtures use public facades with host `Date.now` clock injection (napi optional `nowUnixSecs`). No `SOLVAPAY_IMPL=ts` force.
...
- **Shadow facade pin (updated Step 53):** `installFacadeDriverSession` forces `SOLVAPAY_IMPL=rust` and injects WASM `WasmClient`/`FetchTransport` so fetch-based wire capture still works after the TS client body was deleted; rust driver stays on the Rust CLI. Legacy report keys remain `ts*` for compatibility.
...
- **Retrofit:** Step **42T Done** — strict typing (`.pyi` + `py.typed`, `mypy --strict` + `pyright` + `ruff`) and docstring coverage from the shared IR doc model (reuses 18T infra). **D20 example Done** — `examples/python/get-merchant` (`run` + offline `http.server` asserting `/v1/sdk/merchant`); CI in `python-binding` smoke venv.
...
- **Retrofit:** Step **45T Done** — strict typing (`steep==2.0.0 check` + `rubocop==1.88.2` against generated RBS/facade; no Sorbet) and YARD coverage from the shared IR doc model (reuses 18T infra / `doc_render`). **D20 example Done** — `examples/ruby/get_merchant` (`GetMerchant.run` + offline TCP stub asserting `/v1/sdk/merchant`); CI in `ruby-binding` full leg. (`GetMerchant` module avoids Minitest `#run` collision; TCP stub matches binding smoke tests — WEBrick is not a default gem on Ruby 3.3.)
...
### Phase 10 — Go (wazero + embedded WASM) — Closed

<!-- running per-step bullets accumulate here as each step lands -->
- Step 49 (Scaffold wazero binding): Rust guest crate `solvapay-go-wasm` at `rust/bindings/go/wasm/` (`cdylib` for `wasm32-wasip1`) compiles `solvapay-core` + `solvapay-transport` behind a guest-memory ABI (`sv_alloc` / `sv_dealloc`, packed `(ptr<<32)|len` returns) with a `solvapay_host::transport_send` host import (`WasiHostTransport: Transport`); `solvapay-transport` `wasm32` deps narrowed to `target_os = "unknown"` so WASI drops `web-sys`/`FetchTransport`; `dto-gen` gained `Toolchain::Go` emitting allowlisted guest shims (`sv_get_merchant`, `sv_verify_webhook`, shared `args.rs`) via `--go-bindings-out` (golden `go_shim_golden.rs`); Go module `github.com/solvapay/solvapay-go` embeds `solvapay_core.wasm` and runs it under wazero with a `net/http` host transport + demand-grown bounded instance pool (`Version`, `VerifyWebhook`, `Client.GetMerchant`; `solvapay.Error` via `errors.As`); ctx-cancel discards the borrowed instance. "done when" verified: `go_shim_golden`; guest WASI build; `build-wasm.sh` + committed-wasm staleness gate; `go test -race ./...` (incl. concurrent pool + cancel teardown); CI `go-binding` job (§15 note 54).
- Step 50 (Generate Go facade + signature-parity): Dropped `GO_CLIENT_ALLOWLIST`; `Toolchain::Go` emits all 36 `sv_*` client exports (`clientIgnore` / `clientAwait` / `clientSplit` via shared `client_call_body` + `pollster::block_on`) with `split_path_refs` in `args.rs`; new `emit_client_go` / `emit_parity_suite_go` → `client_generated.go` (ctx-first PascalCase methods returning `any`, godoc first-word = identifier) + `signature_parity_generated_test.go` (count/sorted-unique/compile-time refs/sync-only/frozen defaults); hand-written `GetMerchant` removed; Phase 0 client fixtures replay through the facade (`fixture_conformance_test.go` + `net/http` stub); CI regen adds `--go-client-out` / `--go-parity-out` + `@generated` paths. "done when" verified: `go_shim_golden` / `go_client_golden` / `go_parity_golden`; `go test -race ./...`; regen-drift. Deferred: `Payable`/`Gate` ergonomics + Go `decisions.rs` shims (§15 note 55).
- Step 51 (Live contract + go module release): `internal/livecontract` (SCENARIOS / ResolveArgs / Normalize / ScoreScenario) + env-gated `cmd/live-contract` writing `contract/shadow/output/go-live-report.json`; shared `internal/dispatch` reflection helper reused by fixture conformance + live binary; `shadow-go.yml` dispatch-only; `publish-go.yml` git-subtree-splits `rust/bindings/go` (dry-run default; real push/tag to `solvapay/solvapay-go` gated by `publish_to_go` / `solvapay-go-v*`); tested `examples/go/get-merchant` (`run(...)` + httptest); CI `go-binding` gains example test + fresh-module local-replace smoke; new `go-crosscompile` GOOS×GOARCH matrix (`go build`/`go vet`, `CGO_ENABLED=0`). No load-time skew guard — `//go:embed`'d wasm ships in the same module tarball as the facade. "done when" verified: `go test -race ./...`; example `go test`; cross-compile spot check; offline helpers green (§15 note 56).
- **Cross-cutting deliverables (redesign §5.6/§9/D19–D20):** generated Go signatures carry godoc from the shared IR doc model ✅ Step 50; runnable, tested example under `examples/go/` ✅ Step 51. (Go is statically typed, so no dynamic-language strictness gate applies.)

**Phase-close handoff** (filled when the last step's "done when" is verified):
- **What was done:** Steps 49–51 delivered the wazero + embedded-wasm Go binding (guest ABI + host transport + pool), the full 36-op generated facade + signature-parity + offline fixture conformance, then live-contract driver + dispatch-only shadow workflow + gated subtree-split publish to `solvapay/solvapay-go` + tested `examples/go/get-merchant` + GOOS/GOARCH cross-compile CI gate.
- **Why:** Close Phase 10 so Go matches Python/Ruby/Rust reliability posture (offline golden CI gate + dispatch-only live shadow + release-train publish) before post-cutover deletion (Steps 52–55).
- **Decisions to document:** module path `github.com/solvapay/solvapay-go`; release via git subtree split (not a monorepo `go.mod` publish); dry-run default; real push/tag gated by `publish_to_go` / `solvapay-go-v*` + `SOLVAPAY_GO_DEPLOY_TOKEN`; no skew guard (embedded wasm in the same module); platform verification = CI cross-compile + fresh-module local-replace smoke (no off-host runtime). Deferred: external `solvapay/solvapay-go` repo creation + first real tag (maintainer-gated); hosted contract env for non-dispatch live CI; `Payable`/`Gate` + Go `decisions.rs` shims.
- **Pointers:** §7.7, Phase 10, §10.3 Go example / shadow / module-release rows, §15 notes 54–56; diagrams unchanged

### Cross-cut — Per-language examples (D20 / §9) — Done

- **What was done:** Added `examples/python/get-merchant` and `examples/ruby/get_merchant` (TDD, offline mock HTTP asserting `/v1/sdk/merchant` + fields); relocated all TS demos under `examples/typescript/` (`pnpm-workspace.yaml` → `examples/typescript/*`; path refs in SDK + Mintlify docs); wired CI (`python-binding` smoke pytest, `ruby-binding` full example test, `lint-build-test` `build:examples` + example unit tests). Go/Rust examples already landed in Steps 48/51.
- **Why:** Close redesign §13 gate / D20 so every language has a CI-verifiable runnable example and the examples tree matches the per-language layout.
- **Decisions:** Uniform **get-merchant** (not facade-gate) for Python/Ruby to match Go/Rust; exercises real reqwest transport against local HTTP (`http://` base URL OK). Ruby uses `GetMerchant.run` (avoids Minitest `#run`) + TCP stub (WEBrick not default on Ruby 3.3). `build:examples` uses `pnpm -r --filter '@example/*'` (not turbo) because turbo 2.9 hard-fails on the pre-existing `@solvapay/server` ↔ `@solvapay/mcp-core` cycle.
- **Pointers:** redesign §9 "Examples", §10.3 examples row, §13 D20 gate; `examples/README.md`; `.github/workflows/ci.yml`

### Post-cutover — deletion and C ABI — In progress

<!-- running per-step bullets accumulate here as each step lands -->
- Step 52 (Delete superseded TS in `@solvapay/core`): Rust-only facades for 3 domains + 12 helper modules (`native-dispatch` / `native-core` / `native-helpers`); deleted portable TS bodies; eager `@solvapay/core/browser-wasm` + React side-effect import; React/vitest napi install; server/edge/fixture force core `resolveImpl: () => 'rust'`; `native-decisions` prefers native for core helpers under `SOLVAPAY_IMPL=ts`; retained TS = const tables + shape-only `BusinessDetailsSchema` + `isRenewalError` + §8 host surface; allowlist trimmed; CI dropped core `test:unit:ts`; budgets re-recorded — "done when" verified: core 122, server 366×2, React 1083, contract 1188×2, `delegation:check`, fixture-runner 431/431, browser symbol audit; §15 note 57
- Step 53 (Delete superseded TS in `@solvapay/server`): Rust-only Node napi + edge WASM; deleted `verifyWebhookTs`/`timingSafeEqual`, `paywall-*-ts`, `calculateDelayTs`, all 36 client `fetch` bodies; thin facades + §8 host orchestration retained; `SOLVAPAY_IMPL=ts` fail-fast (no republish-free TS rollback); napi optional webhook clock + fixture-frozen `Date.now`; client fixtures/shadow via injected WASM `FetchTransport`; `facade-driver.ts` (legacy `ts*` report keys kept); CI aliases + `server-superseded-ts` gate — "done when" verified: server 356, contract 1193, shadow 19, fixture-runner 431/431, `solvapay-node` 25, edge lib 10, edge unit 72, core 122 / mcp-core 108 / React 1083, `delegation:check` + `server-superseded-ts:check` + `manifest:check` OK; §15 note 59
- Step 53 (Delete superseded TS in `@solvapay/server`): Rust-only server facade — Node napi (`@solvapay/server-native`) + edge WASM (`@solvapay/server-wasm`) are the only execution paths; `SOLVAPAY_IMPL=ts` no longer selects a server implementation. **Removed logic:** `verifyWebhookTs` + `timingSafeEqual` + `node:crypto` (index.ts/edge.ts); `native-decisions` `tsFallback` / `CORE_HELPER_METHODS` / `calculateDelayTs`; `paywall-state-ts` / `paywall-gate-ts` / `paywall-payload-ts`; all client `fetch` bodies/URL/parse. **Rust-side:** internal napi `verify_webhook` gained optional 4th `nowUnixSecs` (public `verifyWebhook(options)` unchanged); `verifyWebhookNative` passes `Math.floor(Date.now()/1000)`; `native.ts`/`wasm.ts` postambles regenerated via `dto-gen` chrome snapshot (zero drift); fixed a latent wasm32 `SystemTime::now()` panic in `ClientShell` defaults (→ `js_sys::Date`/`Math`), rebuilt server-wasm. **Retained host TS:** adapters, fetch route handlers, MCP registration, `factory`, host `withRetry` loop (only delay selection is Rust), caches/deduplicator, all types. **No-TS rollback:** uninstalled / forced-ts throws (`server sync API not installed` / `server webhook API not installed`); rollback = republish prior facade. **Fixtures/shadow:** client wire fixtures + shadow `ts-driver` (now a facade driver) inject the real WASM `WasmClient` so mocked `globalThis.fetch` captures the `FetchTransport` wire; webhook fixtures drop `withForcedImpl('ts')` and mock `Date.now`. **New grep gate:** `server-superseded-ts:check` beside `delegation:check`; CI dropped server `test:unit:ts` / forced-ts `test:contract` / `test:unit:edge-ts` / forced-ts edge replay (aliases retained → rust/edge-wasm) — "done when" verified: server unit rust 356, contract rust 1193, `shadow:selftest` 19, fixture-runner 431/431, `solvapay-node` 25, `solvapay-wasm edge` 10, `dto-gen` golden, `binding.spec.mjs` 5, `delegation:check` / `manifest:check` / `server-superseded-ts:check`, core 122 / mcp-core 108 / react 1083; §15 note 59. **Pre-existing not-green:** `parity:check` 24 core-surface extras (Step 52 scope).
- Step 54 (Publish the optional C ABI — scaffold): new `rust/bindings/c` (`solvapay-c` cdylib+staticlib) with generation-counted opaque handles, generic `solvapay_client_call(op, args_json)` JSON envelopes, `catch_unwind` on every edge, cbindgen `include/solvapay.h` + `SOLVAPAY_ABI_VERSION`, scaffold allowlist `getMerchant` + `verifyWebhook`/`version`; C smoke + `c-binding` CI job — deferred: full 36-op `Toolchain::C` emitter column (C analog of Go step 50); §15 note 58 — "done when" verified: manual `ctest/run.sh` create/call/free + handle-misuse PASS
...
| Go module path naming (`github.com/solvapay/solvapay-go` vs vanity import) | Steps 49–51 | **Resolved (step 49/51):** module path `github.com/solvapay/solvapay-go` (`go.mod`); release via subtree-split to external `solvapay/solvapay-go` (`publish-go.yml`); vanity import deferred | SDK |
| External `solvapay/solvapay-go` repo creation + first real tag | Before first real Go publish | **Open (maintainer-gated):** create empty `solvapay/solvapay-go` + `SOLVAPAY_GO_DEPLOY_TOKEN` (contents:write); dry-run `publish-go.yml` is green without them | SDK |
| Whether the Go WASM artifact is committed in-repo or attached to release tags | Before step 49 cutover | **Resolved (step 49):** committed in-repo at `rust/bindings/go/solvapay_core.wasm` and `//go:embed`'d; rebuilt by `build-wasm.sh` with a CI staleness gate | SDK |
| WASM instance-pool sizing strategy for Go | Step 49 | **Resolved (step 49):** demand-grown, bounded pool (create-on-borrow up to a max, block at cap, return idle instances to a stack); dirty/cancelled instances discarded on ctx-cancel rather than reused | SDK |
...
| Node napi webhook fixture-clock injection + remove forced-TS server fixtures | Step 53 | **Resolved (step 53):** optional `nowUnixSecs` on napi `verifyWebhook`; harness/shadow no longer force `SOLVAPAY_IMPL=ts`; Web Crypto / client `fetch` TS bodies deleted (§15 note 59) | SDK |
| Optional C ABI full 36-op surface + `Toolchain::C` emitter column | Post–Step 54 scaffold | **Open (deferred):** Step 54 ships generic dispatch + `getMerchant` allowlist only; full surface is the C analog of Go step 50 (§15 note 58) | SDK |
...
| Hosted contract-test environment for CI shadow live runs | Post–step 25 | Open — TS `shadow.yml` + Python `shadow-python.yml` + Ruby `shadow-ruby.yml` + Rust `shadow-rust.yml` + Go `shadow-go.yml` are `workflow_dispatch`-only until a shared sandbox/contract env + secrets exist; offline golden fixtures (TS + Python Step 42 + Ruby Step 44 + Rust Step 47 + Go Step 50) remain the green CI gate | Backend + SDK |
...
| Doc-comment coverage on generated signatures (redesign §5.6/§5.1, D19) | Owned by Steps 42T/45T (+ Step 18T for TS + shared infra, Step 47 Rust, Step 50 Go) | **Done (18T + 42T + 45T + 47 + 50):** shared IR + TSDoc (18T); Python docstrings (42T, §15 note 51); Ruby YARD (45T, §15 note 52); Rust rustdoc (47); Go godoc on generated facade (50, §15 note 55) | SDK |
| Per-language examples workstream + `examples/ → examples/typescript/` relocation (redesign §9 "Examples", D20) | Steps 42/45/48/51 (per-language example); relocation PR before/with them | **Done:** uniform `get-merchant` examples for Python/Ruby/Go/Rust (offline mock HTTP / MockTransport); TS examples relocated under `examples/typescript/` (`pnpm-workspace.yaml` glob updated); CI: `python-binding` smoke pytest, `ruby-binding` full example test, `lint-build-test` `build:examples` + example unit tests, existing Go/Rust gates | SDK |
