...
52. **Delete superseded TS in `@solvapay/core`.** ✅ Landed (2026-07-24). **Rust-only, full scope:** deletes the 3 domain TS bodies (business-details / credit-display / seller-identity) **and** the 12 Phase-4/5 helper modules' logic bodies; public exports become facades over `native-core.ts` / `native-helpers.ts` → napi / eager browser WASM. `SOLVAPAY_IMPL=ts` no longer covers `@solvapay/core` logic (uninstalled or forced-ts throws `SolvaPayError('core sync API not installed')`). Browser core WASM is **eager** via `@solvapay/core/browser-wasm` (React side-effect import). Retained TS: data tables, `BusinessDetailsSchema` (shape-only Zod), all types, `isRenewalError`, §8 host surface (`SolvaPayError`, `getSolvaPayConfig`, `Env`, `version`). See §15 note 57.
...
| `@solvapay/core` pure logic            | **No `SOLVAPAY_IMPL=ts` rollback after Step 52** — uninstalled / forced-ts throws; restore requires republishing the prior facade package                                                                                                                                                                                                       |
