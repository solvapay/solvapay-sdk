...
| Third-party / exotic            | Optional C ABI                            | `rust/bindings/c` (`solvapay-c` cdylib+staticlib): cbindgen `include/solvapay.h`, generation-counted opaque handles, owned buffers + `solvapay_free_*`, `SOLVAPAY_ABI_VERSION`, `catch_unwind` panic containment, generic JSON-envelope `solvapay_client_call` (step 54 scaffold; §15 note 58)                                             |
...
- **Rust-side toolchain shims** — the napi `#[napi]`, wasm `#[wasm_bindgen]`, PyO3 (Steps 40–41), and Magnus (Steps 43–44) `args.rs` / `decisions.rs` / `payload_builders.rs` / client-dispatch modules; the `wazero`-export WASI-guest shims (`Toolchain::Go`, Step 49 scaffold — allowlisted `sv_get_merchant` / `sv_verify_webhook` + shared `args.rs` under `rust/bindings/go/wasm/src/`; full surface Step 50); the optional C ABI (`solvapay-c`, Step 54 scaffold — generic envelope dispatch + `getMerchant` allowlist + cbindgen header; full 36-op `Toolchain::C` emitter column deferred — §15 note 58).
...
| Go module release (`solvapay-go`)                    | git subtree-split `rust/bindings/go` → `solvapay/solvapay-go` + `vX.Y.Z` tag (`publish-go.yml`); dry-run default; real push/tag gated; no skew guard (embedded wasm)                                                                                                                                                                                                                                                                                                                                                                                                                                                | Step 51 ✅                                                                                                                                                                                                          |
...
| Whether the Go WASM artifact is committed in-repo or attached to release tags                                                             | Before step 49 cutover                                       | **Resolved (step 49):** committed in-repo at `rust/bindings/go/solvapay_core.wasm`, `//go:embed`'d; rebuilt by `build-wasm.sh` with a CI staleness gate                                                                                                                                                                                                                                     |
...
| Exact per-language type-check/lint toolchain + pinned versions for the dynamic-language strictness gate (§5.6, D18)                       | Steps 42T (Python), 45T (Ruby)                               | **Resolved (step 45T):** Ruby `steep==2.0.0` + `rubocop==1.88.2` (+ `rbs==4.0.3` required by steep 2) in `rust/bindings/ruby` Gemfile `:typecheck` / gemspec; CI in `ruby-binding` full matrix leg. Python earlier: `mypy==1.17.1 --strict` + `pyright==1.1.411` + `ruff==0.12.4` (§15 note 51). No Sorbet RBI.                                                                             |
...
**Note 51 — Step 42T Python strict typing + doc comments (checked 2026-07-23):** Landed the Python emitter column for D18/D19 on top of Step 18T infra. **Emitter:** `dto-gen` `emit_pyi_py` + `--py-stub-out` writes `rust/bindings/python/python/solvapay/__init__.pyi` (JSON-string boundary `args_json: str` → `str`; async + `_blocking` twins from `availability.py`; docstrings from `IrDocModel` via shared `doc_render::render_entry_doc_lines`). **Marker:** empty `py.typed` (already in maturin `include`). **Facade:** hand-written `facade.pyi` hardened — public `ApiClient` Protocol, `ParamSpec`-typed `payable`, no `Any` / `__getattr__`; companion `errors.pyi` / `results.pyi` / `_solvapay.pyi`. **Pinned toolchain** (optional-deps group `typecheck`): `mypy==1.17.1`, `pyright==1.1.411`, `ruff==0.12.4` — CI `python-binding` smoke runs them against the public stub surface after fresh wheel install. **Gates:** regen-drift + `@generated` header cover `__init__.pyi`; emitter tests assert every catalogued operation twin has a non-empty docstring (backed by `check_doc_coverage`). **Scope decision:** keep JSON-string boundary (no per-DTO TypedDicts in 42T); `examples/python/` remains a separate D20 deliverable. Sources: redesign §5.6 / D18–D19; migration map row 42T.
