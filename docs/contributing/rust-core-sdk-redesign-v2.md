...
| [`sdks/typescript/server/src/client.ts`](../../sdks/typescript/server/src/client.ts)                                                | ~1 020    | All 36 client methods + normalization quirks (§2.3)                                                            | → Rust (steps 22–24)                                                                      |
...
| [`sdks/typescript/mcp-core`](../../sdks/typescript/mcp-core) transport parts                                                        | ~2 000    | OAuth bridge, bearer, CSP, narrate, response-context                                                           | Stays TS                                                                                  |
...
| **WASM for every language**                       | Works as the Node fallback (napi-rs) and as Go's primary delivery (wazero — no better pure-Go option). As the _primary_ Python/Ruby delivery it loses: wasmtime-in-Python/Ruby adds a runtime dependency and an interop layer heavier than a native extension, startup cost is real, and GIL/GVL integration is manual. Native extensions remain the ecosystem norm for those server SDKs.                                                                                           |
...
1. Add the core fn and a `bindings:` entry (or `pnpm gen:bindings --fix` for catalog orphans).
...
| Optional C ABI | `solvapay-c` shared library (`cdylib`/`staticlib`) + committed cbindgen `include/solvapay.h`, versioned `SOLVAPAY_ABI_VERSION` (Step 54 scaffold; full 36-op emitter deferred — §15 note 58) | Third-party responsibility |
...
37. **Wire conditional exports.** `@solvapay/server` → napi-rs binding with WASI fallback, behind a runtime rollback flag (`ts` / `rust` or a package export condition) so rollback is a flag flip, not a republish.
...
52. **Delete superseded TS in `@solvapay/core`.** ✅ Landed (2026-07-24). **Rust-only, full scope:** deletes the 3 domain TS bodies (business-details / credit-display / seller-identity) **and** the 12 Phase-4/5 helper modules' logic bodies; public exports become facades over `native-core.ts` / `native-helpers.ts` → napi / eager browser WASM. the TypeScript body forced no longer covers `@solvapay/core` logic (uninstalled or forced-ts throws `SolvaPayError('core sync API not installed')`). Browser core WASM is **eager** via `@solvapay/core/browser-wasm` (React side-effect import). Retained TS: data tables, `BusinessDetailsSchema` (shape-only Zod), all types, `isRenewalError`, §8 host surface (`SolvaPayError`, `getSolvaPayConfig`, `Env`, `version`). See §15 note 57.
    ...
    **Done when:** package tests green with Rust-only logic — verified: `test:unit:rust` 122; React 1083 (napi setup); server 366×2; `test:contract` 1188 both flags; `delegation:check` OK; fixture-runner `passed=431 failed=0`; browser symbol audit + budgets re-recorded.

53. **Delete superseded TS in `@solvapay/server`.** ✅ Landed (2026-07-24). **Rust-only, full scope:** deletes both `verifyWebhook` TS bodies (`verifyWebhookTs` + `timingSafeEqual` + `node:crypto`), the paywall-state/gate/payload `*-ts` modules, the client method `fetch` bodies, and the `native-decisions` `tsFallback` / `calculateDelayTs`. Node routes sync over `@solvapay/server-native` (napi; internal `verifyWebhook` gained an optional 4th `nowUnixSecs`), edge async over `@solvapay/server-wasm`. the TypeScript body forced no longer selects a server implementation — uninstalled or forced-ts throws (`server sync API not installed` / `server webhook API not installed`). Public signatures unchanged. Retained TS: adapters, fetch route handlers, MCP registration, `factory`, the host `withRetry` loop (only delay selection is Rust), caches/deduplicator, all types. New `server-superseded-ts` grep gate guards against regressions. See §15 note 59. **Parity:** `parity:check` is binding-aware — `manifest.bindings` `names.ts` symbols (including `catalog: { kind: none }` decision helpers) are recognized without per-helper `TS_ONLY_ALLOWLIST` entries.
    _Depends on Steps 37R and 38R_: Node and edge surfaces must both be delegating before their TS bodies can be deleted.
    **Done when:** server suite green; a grep gate confirms no dead duplicated logic remains — verified: server unit rust 356; contract rust 1193; `shadow:selftest` 19; fixture-runner 431/431; `solvapay-node` 25; edge wasm lib 10; edge unit 72; `delegation:check` + `server-superseded-ts:check` + `manifest:check` OK; core 122 / mcp-core 108 / React 1083.
...
| server-superseded-ts                                 | Grep gate (`pnpm server-superseded-ts:check`): fails if any deleted `@solvapay/server` TypeScript semantic body reappears (`verifyWebhookTs`, `timingSafeEqual`, client `fetch(` bodies, `calculateDelayTs`, `paywall-*-ts` files, `tsFallback`). Runs beside `delegation:check`                                                                                                                                                                                                                                                                                                                                    | Step 53 ✅                                                                                                                                                                                                          |
...
| Python contract suite                                | Offline golden-fixture replay in `python-binding` fresh-venv pytest (535 fixtures + 36-op coverage); live backend via `shadow-python.yml` dispatch-only                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Step 42 ✅                                                                                                                                                                                                          |
...
| Ruby contract suite                                  | Offline golden-fixture replay in `ruby-binding` `rake test` (`test/contract/`, 535 fixtures + 36-op coverage); live backend via `shadow-ruby.yml` dispatch-only                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Step 44 offline ✅; Step 45 live ✅                                                                                                                                                                                 |
...
| Hosted contract-test environment for CI shadow live runs                                                                                  | Post–step 25                                                 | Offline `pnpm shadow:selftest` + Python/Ruby/Rust golden fixtures are in CI; live `pnpm shadow:run`, `shadow-python.yml`, `shadow-ruby.yml`, and `shadow-rust.yml` stay `workflow_dispatch` until a shared sandbox/contract env + secrets exist                                                                                                                                    |
...
**Note 55 — Step 50 Go facade + signature-parity (checked 2026-07-23):** Expanded Phase 10 beyond the Step 49 scaffold. **Guest shims:** dropped `GO_CLIENT_ALLOWLIST`; `emit_go_client` emits all 36 Groups A–C `sv_*` exports using shared `client_call_body` (`clientIgnore` / `clientAwait` / `clientSplit`) wrapped in `pollster::block_on`; `args.rs` gains `split_path_refs` (+ `ClientConfig` / `VerifyWebhookArgs`); DTO deserialize types imported from `solvapay_dto`. **Facade:** new `emit_client_go` → `sdks/go/client_generated.go` — ctx-first PascalCase methods, body/path args as `map[string]any` / `string`, returns `any` (arrays from list ops), godoc first-word = identifier via `render_entry_doc_lines`; hand-written `GetMerchant` removed. **Parity:** `emit_parity_suite_go` → `signature_parity_generated_test.go` (36-count, sorted-unique, compile-time `(*Client).Method` refs, sync-only matrix, frozen `defaults:`). **Fixtures:** `fixture_conformance_test.go` replays Phase 0 `contract/fixtures/client/**` through the facade against `httptest`. **CLI/CI:** `--go-client-out` / `--go-parity-out`; regen-drift + `@generated` cover both paths + wasm shims; goldens `go_shim_golden` / `go_client_golden` / `go_parity_golden`. **Deferred:** `Payable`/`Gate` ergonomics + Go `decisions.rs` shims (Step 51+). Sources: Step 47 Rust facade pattern; §15 note 54.
