...
> **As-built reference:** For the current state of the SDK, read [`architecture.md`](./architecture.md) — the evergreen "as-built" architecture. This document is the deep spec/rationale (decisions, per-step research); [`rust-migration-map.md`](./rust-migration-map.md) tracks step status.

**Status:** migration complete except step 55 (promote all compatibility gates); steps 1–54 done. Living document — every migration step started with online research against current upstream sources; findings that confirm, sharpen, or contradict a decision are written back here in the same session. Dated research notes live in §15.
...
- Public TypeScript surface: [`sdks/typescript/server/src/index.ts`](../../sdks/typescript/server/src/index.ts), [`sdks/typescript/server/src/client.ts`](../../sdks/typescript/server/src/client.ts), [`sdks/typescript/react/src/index.tsx`](../../sdks/typescript/react/src/index.tsx)
- OpenAPI type generation today: [`sdks/typescript/server/scripts/generate-types.ts`](../../sdks/typescript/server/scripts/generate-types.ts)
...
- Performance as a primary motivation. The workload is I/O-bound; the motivation is _semantic consolidation_. Performance claims must cite the budgets in §7.8.
...
The routed `SolvaPayClient` surface is 38 methods (`contract/manifest/op-surfaces.generated.md`). Grouped by domain, with the wire route each hits:
...
| Purchases                      | `listPurchases`                                                                                 | `GET /v1/sdk/purchases`                                      |
|                                | `cancelPurchase`                                                                                | `POST /v1/sdk/purchases/{ref}/cancel`                        |
|                                | `reactivatePurchase`                                                                            | `POST /v1/sdk/purchases/{ref}/reactivate`                    |
| Credits                        | `getCreditActivity`                                                                             | `GET /v1/sdk/credits/activity`                               |
...
- Framework adapters in [`sdks/typescript/server/src/adapters`](../../sdks/typescript/server/src/adapters/index.ts) (`http.ts`, `next.ts`, `mcp.ts`)
- Fetch route handlers in [`sdks/typescript/server/src/fetch`](../../sdks/typescript/server/src/fetch/index.ts)
...
Per-language **MCP-authoring adapters** — the thin `registerPayable(...)` / `ctx.respond(...)` ergonomic over each language's _own_ MCP SDK that `@solvapay/mcp` provides today — fall in the same category. They are idiomatic, hand-written framework glue, **not** core, and are **not** produced by the §5.6/§5.7 signature/binding-glue generators. Like the framework adapters above they are out of scope for steps 1–55, but — unlike an accidental omission — they are a **named future track** (see §9, "MCP-authoring adapters"). The distinction is deliberate: steps 1–55 deliver _core-surface_ parity (§2); turnkey paid-MCP _authoring_ parity in every language is the additive commitment carried by that track (gate in §13).
...
| Type strictness | Every surface ships the strictest static contract the language allows: TS `.d.ts`; Python `.pyi` + `py.typed`; Ruby RBS; Go interfaces; Rust `pub` signatures. Dynamically-typed surfaces (Python, Ruby) are CI-verified under strict type-checkers/linters against the generated output (§5.6, §10.3) |

### 2.7 Enforced, not aspirational

`pnpm parity:check` reads integrator-visible declarations: TypeScript portable exports, per-language helper modules, and the MCP packages (`python-mcp`, `ruby-mcp`, `sdks/go/mcp`, `rust-mcp`). A generic `solvapay_call` dispatcher does not count as a named sync op.

`pnpm facade-coverage:check` extracts public declarations per language. MCP package rows are scored only on MCP-section symbols; other ops are `na`. A gap with an empty reason fails the build. Browser wasm gaps cite either a capability exclusion (§7.1) or the size budget (§7.8).

The Go job tests the committed `solvapay_core.wasm`, then warns if a host rebuild differs, because `wasm32-wasip1` bytes are not stable across machines.

Shared golden fixtures run against the language surfaces. See Phase 0 (manifest + fixtures), steps 18/41/44/47/50 (parity gates), and §10.3 (CI gates). [`architecture.md`](./architecture.md) is the current description of what those gates assert.
...
| ----------------------------------------------------------------------------------------------------------------------------------- | --------- | -------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| [`sdks/typescript/core/src/business-details.ts`](../../sdks/typescript/core/src/business-details.ts) + `business-details-public.ts` | ~430      | Tax-ID validation, country/tax-type derivation, Zod schema                                                     | → Rust (step 9)                                                                           |
| [`sdks/typescript/core/src/credit-display.ts`](../../sdks/typescript/core/src/credit-display.ts)                                    | ~50       | Zero-decimal currency handling, credit → minor-unit conversion                                                 | → Rust (step 10)                                                                          |
| [`sdks/typescript/core/src/seller-identity.ts`](../../sdks/typescript/core/src/seller-identity.ts)                                  | ~90       | Seller tax-identifier display resolution                                                                       | → Rust (step 10)                                                                          |
| [`sdks/typescript/core/src/index.ts`](../../sdks/typescript/core/src/index.ts)                                                      | ~155      | `SolvaPayError` (`status`, `code`), `Env` Zod schema, `getSolvaPayConfig`                                      | Error → Rust (step 17); config facade stays TS                                            |
| [`sdks/typescript/server/src/client.ts`](../../sdks/typescript/server/src/client.ts)                                                | ~1 020    | All 38 routed client methods (`op-surfaces.generated.md`) + normalization quirks (§2.3)                        | → Rust (steps 22–24)                                                                      |
| [`sdks/typescript/server/src/utils.ts`](../../sdks/typescript/server/src/utils.ts)                                                  | ~320      | `withRetry` (3 backoff strategies), `createRequestDeduplicator` (Workers-safe lazy interval)                   | Retry policy → Rust (step 11); deduplicator stays host-side (timers/maps are per-runtime) |
| [`sdks/typescript/server/src/index.ts`](../../sdks/typescript/server/src/index.ts) `verifyWebhook`                                  | ~50       | Node-sync HMAC verification                                                                                    | → Rust (step 12)                                                                          |
| [`sdks/typescript/server/src/edge.ts`](../../sdks/typescript/server/src/edge.ts) `verifyWebhook`                                    | ~60       | Async Web Crypto duplicate of the same logic                                                                   | Deleted by the same step 12 (one Rust impl, two facades)                                  |
| [`sdks/typescript/server/src/paywall-state.ts`](../../sdks/typescript/server/src/paywall-state.ts)                                  | ~155      | `classifyPaywallState`, gate/nudge copy builders                                                               | → Rust (step 13)                                                                          |
| [`sdks/typescript/server/src/paywall-gate.ts`](../../sdks/typescript/server/src/paywall-gate.ts)                                    | ~100      | `buildPaywallGate`, PAYG-topup reclassification                                                                | → Rust (step 14)                                                                          |
| [`sdks/typescript/server/src/paywall.ts`](../../sdks/typescript/server/src/paywall.ts)                                              | ~1 160    | `SolvaPayPaywall` (caches, customer resolution, decide/protect), `PaywallError`, `paywallErrorToClientPayload` | Decision core → Rust (steps 32–33); cache/plumbing stays TS facade                        |
| [`sdks/typescript/server/src/factory.ts`](../../sdks/typescript/server/src/factory.ts)                                              | ~1 230    | `createSolvaPay`, `payable` adapters, `gate()`                                                                 | Stays TS; Python/Ruby/Go/Rust get idiomatic equivalents (§2.4)                            |
| [`sdks/typescript/server/src/helpers/*`](../../sdks/typescript/server/src/helpers/index.ts)                                         | ~1 900    | 24 framework-agnostic route cores (`*Core`), `handleRouteError`, balance-poll schedules                        | → Rust (steps 26–31); `Request` parsing stays thin TS                                     |
| [`sdks/typescript/server/src/types/generated.ts`](../../sdks/typescript/server/src/types/generated.ts)                              | generated | OpenAPI DTOs via `openapi-typescript`                                                                          | Replaced by generated Rust DTOs + generated TS decls (steps 15, 18)                       |
| [`sdks/typescript/server/src/types/client.ts`](../../sdks/typescript/server/src/types/client.ts)                                    | ~550      | Hand-maintained SDK overlays (§6.3)                                                                            | Encoded in manifest + generator (step 16)                                                 |
| [`sdks/typescript/mcp-core`](../../sdks/typescript/mcp-core) pure parts                                                             | ~450      | `paywallToolResult`, `response-envelope`, `tool-names`, `descriptors`                                          | → Rust (steps 34–35)                                                                      |
| [`sdks/typescript/mcp-core`](../../sdks/typescript/mcp-core) transport parts                                                        | ~2 000    | OAuth bridge, bearer, response-context. Narration and CSP merge are Rust ops (`mcpNarrate`, `mcpMergeCsp`)     | OAuth/bearer stay TS; narration and CSP merge are Rust                                    |
| [`sdks/typescript/react`](../../sdks/typescript/react), `@solvapay/next`, adapters, fetch handlers, auth, cli                       | large     | Product/framework surface                                                                                      | Never moves (§8)                                                                          |
...
flowchart TB
  ...
  subgraph bindings ["Specialized bindings"]
    NAPI["napi-rs<br/>Node native prebuilds"]
...
2. **No timers in core.** `withRetry` sleeps and `createRequestDeduplicator` runs a cleanup interval today. The Rust retry engine computes _schedules_ (pure); the binding owns the actual sleep (tokio timer, JS `setTimeout` via the facade, GVL-released `sleep` in Ruby). The deduplicator stays host-side entirely.
...
│  ├─ src/paywall_state.rs    # (as-built; not src/paywall/)
│  ├─ src/paywall_gate.rs
│  ├─ src/paywall_decision.rs
│  ├─ src/paywall_payload.rs
...
| `solvapay-transport` | `Transport` trait, `reqwest`/rustls impl (native), Fetch impl (wasm32), client shell (auth headers, idempotency, retry wiring), all 38 routed method implementations                                              | Depends on core + dto; async but runtime-agnostic (§7.4)                                                                                                                                                                                                 |
...
| ------------------------------- | ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Node TypeScript                 | `napi-rs` v3                              | Native prebuilds as optional-dependency packages (eight `sdks/node-native` targets). There is no WASI fallback; an unmatched platform fails to load. Edge and browser use `@solvapay/server-wasm`                                                                                                                                          |
| Browser / edge / Workers / Deno | `wasm-bindgen` + `wasm32-unknown-unknown` | Capability-separated build (§7.1); host Fetch transport via `@solvapay/server-wasm`. Separate from the Node napi prebuilds                                                                                                                                                                                                                 |
...
| Third-party / exotic            | Optional C ABI                            | `sdks/capi` (`solvapay-c` cdylib+staticlib): cbindgen `include/solvapay.h`, generation-counted opaque handles, owned buffers + `solvapay_free_*`, `SOLVAPAY_ABI_VERSION`, `catch_unwind` panic containment, generic JSON-envelope `solvapay_client_call` (step 54 scaffold; §15 note 58)                                                   |

### 4.6 Why not the alternatives

| Approach                                          | Why weaker for this target set                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Single raw C ABI for all first-party wrappers** | Forces every language through the least common denominator: manual memory management, no idiomatic async (Promises/awaitables/GVL-aware blocking all need hand-built shims), duplicated safety wrappers per language, and a worse DX than napi-rs/PyO3/Magnus which generate those shims. Each first-party binding through C ABI roughly doubles its maintenance cost to save one core artifact. Keep the C ABI optional for _third parties_, where we don't own the wrapper anyway. |
...
| **WASM for every language**                       | Go's primary delivery is wazero. Node does not use a WASM fallback: it ships native prebuilds and fails to load when none match. As the _primary_ Python/Ruby delivery WASM loses: wasmtime-in-Python/Ruby adds a runtime dependency and an interop layer heavier than a native extension, startup cost is real, and GIL/GVL integration is manual. Native extensions remain the ecosystem norm for those server SDKs.                                                               |
...
operations:
  checkLimits:
    ...
    names:
      { ts: checkLimits, py: check_limits, rb: check_limits, go: CheckLimits, rust: check_limits }
    ...
    docs: # authored in manifest (Step 18T / §15 note 50); OpenAPI op description fallback when absent
      summary: "Check remaining usage/spend limits for a customer against a product's plan."
      params:
        customerRef: 'Customer to evaluate. Accepts internal ref or external ref.'
        product: 'Product whose plan limits are checked.'
    errors:
...
facade:
  createSolvaPay:
    ...
    docs: { summary: 'Create a configured SolvaPay client / high-level facade.' }
  gate:
    names: { ts: 'payable.gate', py: 'sp.gate', rb: 'sp.gate', go: 'sp.Gate', rust: 'sp.gate' }
    docs:
      {
...
| Response-shape polymorphism                 | `getCustomer`/`listProducts`/`listPlans`/`cancelPurchase` accept multiple backend shapes (§2.3)                                                            | Manifest normalization rules + fixtures for _each_ accepted shape                                                                                                              |
...
- Parameter list (required/optional, defaults)
- A language-neutral **doc model** per entry point: a summary plus per-parameter and return descriptions (§5.1 `docs:`). **Authored today in the contract manifest `docs:` block** (Step 18T / §15 note 50); OpenAPI operation `description` linkage is a forward-compatible follow-on. Each emitter renders the IR model into the idiomatic doc-comment form (see "Doc-comment generation" below)
...
| Python       | `.pyi` stubs + `py.typed` marker + facade module (strictness-checked, §5.6) |
| Ruby         | RBS signatures (strictness-checked, §5.6) + facade                          |
...
Every generated file carries a `generated — do not edit` header. Hand edits to generated files are a CI failure. Every emitted public signature also carries a language-native doc comment (TSDoc / Python docstring / YARD / godoc / rustdoc) from the shared IR doc model — see "Doc-comment generation".
...
| Generated signature-parity suites | Manifest change updates catalog, facades, _and_ parity tests in one PR                                                                                                                                                                                                             |
| Golden signature snapshots        | Per-language signature snapshots reviewed as ordinary diffs                                                                                                                                                                                                                        |
| Type-strictness verification      | Generated dynamically-typed surfaces pass strict type-checkers + linters (Python: `mypy --strict` + `pyright` strict + `ruff`; Ruby: `steep check` + `rubocop`) against the emitted output                                                                                         |
| Doc-comment coverage              | Every catalogued public entry point emits a non-empty language-native doc comment on all five surfaces; generated and drift-checked (shared infra + TS ✅ Step 18T / §15 note 50; Python ✅ Step 42T / §15 note 51; Ruby ✅ Step 45T / §15 note 52; Rust ✅ Step 47; remaining 50) |

#### Add-a-method workflow

**Runbook:** [`sdk-codegen.md`](./sdk-codegen.md) (Workflow A).

1. Refresh the OpenAPI snapshot if the wire shape changed (`pnpm snapshot:openapi --from-url`).
2. Scaffold or edit the contract manifest (`pnpm gen:scaffold operation …` or hand-edit `sdk-contract.yaml`).
3. Fill curated fields (prose docs / name overrides / bindings review); `pnpm gen:bindings --fix` if reconciliation is missing a linker.
4. Run `pnpm gen` (IR → all emitters via `tools/codegen/gen.ts`).
5. All five language signatures, facade stubs, signature-parity suites, and binding glue update in a single PR.
6. CI fails if regeneration was skipped or drifted (`pnpm gen:check`).

#### Doc-comment generation

Every generated public signature — on all five surfaces — carries a language-native doc comment rendered from the one IR doc model (above), so a developer hovering any SDK symbol in their editor sees the **same description** regardless of language. Descriptions are authored in the contract manifest `docs:` block (Step 18T / §15 note 50). For routed operations, when `docs:` is absent, dto-gen falls back to the OpenAPI operation `description` (then `summary`); manifest `docs:` still overrides.

| Surface    | Rendered form                                               |
| ---------- | ----------------------------------------------------------- |
| TypeScript | TSDoc (`/** … */` with `@param`) on `.d.ts` declarations    |
| Python     | docstrings + `.pyi` stub comments                           |
| Ruby       | YARD (`# @param` / `# @return`) above RBS-backed signatures |
| Go         | godoc comments (sentence starts with the identifier name)   |
| Rust       | rustdoc (`///`) on `pub` signatures — renders on docs.rs    |

Doc comments are part of the generated output: they are drift-checked like signatures (regenerate → zero diff) and hand edits to them fail the generated-header gate. Coverage is enforced — see the reliability gate below.

#### Strictness for dynamically-typed surfaces

Statically-typed surfaces (TypeScript, Go, Rust) get their strictness from the compiler. The dynamically-typed surfaces (Python, Ruby) must earn the equivalent guarantee explicitly: the generated surface is the **strictest static contract the language allows**, regenerated on every SDK update and verified in CI so it can never silently loosen.

- **Python.** Each emitter run ships `.pyi` stubs alongside the facade module plus a `py.typed` marker (PEP 561) so consumers' type-checkers see the annotations. The generated + facade code is verified under `mypy --strict` **and** `pyright` (strict mode), with `ruff` linting the facade. No `Any` leaks in the public surface; overloads express the async/blocking split.
- **Ruby.** Each emitter run ships RBS signatures (already emitted, see the emitters table above) verified with `steep check`, plus `rubocop` on the facade. Optional Sorbet RBI generation is a possible follow-on for Sorbet users, **not** required for the gate.

These checks run against the _generated_ output (not hand-written code) so any drift toward looser types is a regeneration failure. The exact tool versions are pinned per §13.

### 5.7 Binding-glue generation (native ↔ envelope ↔ core)

§5.6 generates the public facade **signatures** (`.d.ts` / `.pyi` / RBS / Go / Rust) and the signature-parity suites. §5.7 generates the **binding-glue layer**: the per-toolchain FFI shims that marshal arguments across the boundary and reconstruct results/errors around the existing JSON-envelope-over-string ABI (`{"ok":true,"value":…}` | `{"ok":false,"error":<SdkError>}`, §15 note 32). This is the layer today hand-mirrored across [`sdks/node-native/src/`](../../sdks/node-native/src/decisions.rs) and [`sdks/wasm/src/`](../../sdks/wasm/src/decisions.rs) — `args`, `decisions`, `payload_builders`, the client dispatch (`native_client.rs` / `wasm_client.rs`), and the native-side `native.ts` / `wasm.ts` dispatch + envelope reconstructor. Adding a core fn currently means a hand-written `#[napi(js_name=…)]` and a byte-for-byte-parallel `#[wasm_bindgen(js_name=…)]` wrapper (extract each JSON arg with `require_string` / `optional_f64`, call core, `to_value`, wrap in `run_envelope_sync`); §5.7 makes that a manifest edit.
...
`dto-gen` gains a `BindingSymbol` per core symbol, lowered into `Ir.binding_symbols` from `#[solvapay_export]`. YAML `bindings:` is retired (`manifest-schema` rejects it) and is not the parity input. `parity:check` reads the generated `binding-symbols.snapshot.json`. Each descriptor carries:
...
| IR boundary type                    | Rust extract (napi/wasm)           | Rust serialize                      | TS reconstruct                                                                                               | Python (PyO3)                                     | Ruby (Magnus)                                          | Go (later)          |
| ----------------------------------- | ---------------------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------- | ------------------------------------------------------ | ------------------- |
| `string` (required)                 | `require_string(key)`              | `to_value`                          | `String(v)`                                                                                                  | `&str` / `String` decode                          | `String` (Step 44)                                     | `string`            |
| `string?` (optional)                | `optional_string(key)`             | skip-absent                         | `v ?? undefined`                                                                                             | `Option<&str>` / `Optional[str]`                  | `nil`-coalesce (Step 44)                               | `*string`           |
| `f64` / number                      | `require_f64` / `optional_f64`     | `serialize_whole_f64` (whole → int) | `Number(v)`                                                                                                  | `f64` / `float` / `int`                           | `Float` / `Integer` (Step 44)                          | `float64` / `int64` |
| `bool`                              | `require_bool`                     | `to_value`                          | `Boolean(v)`                                                                                                 | `bool`                                            | `true`/`false` (Step 44)                               | `bool`              |
| raw `Value` passthrough             | `require_value`                    | verbatim                            | verbatim                                                                                                     | dict (Step 41)                                    | `Hash` (Step 44)                                       | `json.RawMessage`   |
| host-injected (`nowMs`, clock, RNG) | read from injected arg, not caller | n/a                                 | supplied by host adapter                                                                                     | host wall clock (`verify_webhook`)                | host adapter / `_verify_webhook_at` (Step 45)          | host adapter        |
| envelope error                      | `err_envelope(<SdkError>)`         | envelope JSON                       | `Api`→`SolvaPayError`, `Paywall`→`PaywallError` (`structuredContent`), `Webhook`/`Transport`→`SolvaPayError` | `SolvaPayError` (+ `code` attr) / envelope string | `SolvaPay::Error` / `SolvaPay::PaywallError` (Step 44) | `errors.As`         |

#### Emitters

New dto-gen emitters produce, all carrying the `generated — do not edit` header (hand edits are a CI failure, §5.6):

- **Rust-side toolchain shims** — the napi `#[napi]`, wasm `#[wasm_bindgen]`, PyO3 (Steps 40–41), and Magnus (Steps 43–44) `args.rs` / `decisions.rs` / `payload_builders.rs` / client-dispatch modules; the `wazero`-export WASI-guest shims (`Toolchain::Go`, Step 49 scaffold — allowlisted `sv_get_merchant` / `sv_verify_webhook` + shared `args.rs` under `sdks/go/wasm/src/`; full surface Step 50); the optional C ABI (`solvapay-c` — generated `Toolchain::C` dispatch: 38 routed client methods plus 7 MCP composites, 45 arms in `sdks/capi/src/dispatch.rs`).
- **Native-side marshalling/reconstruction** — the TS `native.ts` / `wasm.ts` dispatch, Python `_native.py` (Step 41), and Ruby `_native.rb` (Step 44) + envelope → language-native error reconstructors; the Go equivalent (Step 49) is the wazero host plumbing in the Go module's `internal/runtime` (guest-memory marshalling + `net/http` `transport_send` + `solvapay.Error` via `errors.As`), with the generated facade signatures landing in Step 50.
...
| Hand-edit header detection              | The generated-header grep (§5.6) covers `args`/`decisions`/`payload_builders`/client-dispatch + `native.ts` / `wasm.ts` + Python/Ruby native marshalling + Ruby public client/RBS/parity |
| Binding-boundary catalog reconciliation | Every non-§8 core symbol has exactly one `BindingSymbol` and vice-versa                                                                                                                  |
| Retrofit byte-identical proof           | Regenerated node + wasm shims are byte-identical to the committed 37R/38R hand-written output; both-selection-mode suites stay green on the generated glue                               |

#### Add-a-core-symbol workflow

**Runbook:** [`sdk-codegen.md`](./sdk-codegen.md) (Workflow B).

1. Annotate the core fn with `#[solvapay_export]` (`pnpm gen:bindings` prints targets for catalog orphans; it does not write YAML `bindings:`).
2. Run `pnpm gen` (IR → §5.6 signatures **and** §5.7 glue).
3. Every toolchain shim + native marshalling updates in one PR.
4. A skipped or drifted regen fails CI (`pnpm gen:check`, same class as §5.6).

---

## 6. Behavioral contracts to freeze (module-level detail)

This section pins the _semantics_ Phase 0 fixtures must capture. It exists so a session translating a module doesn't have to reverse-engineer intent from TS.
...
`SolvaPayError` carries optional `status` (HTTP) and free-form `code`. Existing thrown messages (per-method prefixes, §2.3) are frozen in the manifest as templates. The Rust `SdkError` (§4.4) adds _stable_ codes for webhook errors and transport errors without changing any existing message string. New codes may be added; existing strings may not change until a major.

Cross-language plug-in rule: wrappers do not re-encode domain failures. They map `SdkError` once at the FFI/facade boundary into the host exception type, then rethrow / reject. Integrators in every language see the same stable `code` vocabulary and the same message templates; only the exception _class_ name and language idioms differ.

### 6.5 MCP contracts (pure parts only)

From [`sdks/typescript/mcp-core`](../../sdks/typescript/mcp-core): `MCP_TOOL_NAMES` (12 canonical names, e.g. `create_payment_intent`, `upgrade`, `manage_account`, `topup`), `paywallToolResult` (deliberately `isError: false` — a paywall is a user-actionable gate, not a tool failure; narration in `content[0].text`, gate on `structuredContent`), and `response-envelope` / `descriptors`. The OAuth bridge and bearer handling stay TypeScript. CSP merge and narration are Rust ops (`mcpMergeCsp`, `mcpNarrate`); `narrate.ts` only wraps `mcpNarrate`.
...
| wasm-bindgen            | No runtime — futures ride the host microtask queue via `wasm-bindgen-futures`; Fetch is the transport _and_ the timer source                                                                                                                                                                                |
...
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| npm (`@solvapay/server`) | Per-target native packages as `optionalDependencies` (eight `sdks/node-native` targets: darwin-x64/arm64, linux-gnu/musl × x64/arm64, win32-x64/arm64) + `wasm-bindgen` package `@solvapay/server-wasm` (edge + browser profiles) wired through export conditions. No napi-rs WASI package | Node fails to load when no native prebuild matches. There is no WASI fallback and no `NAPI_RS_FORCE_WASI` job. Edge defaults to `@solvapay/server-wasm` (Step 38R); Node stays on `@solvapay/server-native` (Step 37R); the browser public-safe subset is opt-in via `@solvapay/core/browser-wasm` |
...
| RubyGems | Platform gems built on native CI hosts via `rake native gem` (linux x64/arm64, darwin x64/arm64, mingw) + source gem requiring a Rust toolchain; `rb-sys-dock` cross-compile deferred until dock images align with Magnus 0.7 / Ruby 3.3 (§15 notes 46/48) | Source-gem compile as documented fallback; no silent stub |
...
| Optional C ABI | `solvapay-c` shared library (`cdylib`/`staticlib`) + committed cbindgen `include/solvapay.h`, versioned `SOLVAPAY_ABI_VERSION`. Generated dispatch: 38 routed methods + 7 MCP composites (45 arms) | Third-party responsibility |
...
| ------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- |
| Browser WASM, gzipped                      | Baseline recorded in [`sdks/wasm/budgets.json`](../../sdks/wasm/budgets.json) at Step 38; **re-recorded at Step 38R** (6531→63633 B gzip) when the browser profile gained the public-safe pure-logic subset + serde envelope machinery; **Step 52** makes browser core WASM **eager** (React/`@solvapay/core/browser-wasm` installs at import — no portable TS fallback). Gzip size unchanged (63633). Byte regression > 10% needs approval + baseline update rationale                         | Phase 6+ / Step 52  |
| WASM cold start (instantiate + first call) | Browser: fresh process/module instantiation + first `wasmVersion`, reported as **p20** of post-warmup samples (lower tail — cold start is bounded below by real work and unbounded above by shared-runner contention, so the median drifts while p20 stays stable). Edge: same methodology for frozen valid `verifyWebhook` — also CI-enforced to catch blowups; browser remains the §7.8 headline metric. Cold-start regression tolerance is **50%** (bytes stay at 10%). Same `budgets.json`. | Phase 6+            |
...
| Binary coverage                            | Full platform matrix green on main — enforced by clean-install CI (`node clean install (native, …)` × 24: 8 targets × Node 22/24/26) after publish-shaped `npm install` of packed tarballs. No WASI clean-install job                                                                                                                                                                                                                                                                           | Phase 6+            |
| Unsupported platform                       | Node load fails when no native prebuild matches. There is no WASI fallback. Edge and browser use `@solvapay/server-wasm`                                                                                                                                                                                                                                                                                                                                                                        | Phase 6+            |

Measurement methodology: `sdks/wasm/scripts/measure-wasm.mjs` — deterministic raw + gzip level-9 byte counts; multiple isolated child processes; discard the first warmup samples then report cold-start **p20** + MAD (diagnostic); pin Node/OS/toolchain metadata. Bytes tolerate 10% regression; cold start tolerates 50%. Normal CI uses `--check` (cannot rewrite budgets); recording requires explicit `--record` from a CI linux host. Do not make unqualified performance claims anywhere in SDK docs; cite these budgets.
...
| MCP SDK registration glue ([`register-virtual-tools-mcp.ts`](../../sdks/typescript/server/src/register-virtual-tools-mcp.ts)), `@solvapay/mcp-core` transport parts (OAuth bridge, bearer)  | JS SDK types and Node/fetch middleware. Narration and CSP merge are Rust ops (`mcpNarrate`, `mcpMergeCsp`)                                                                                   | Payload builders move (steps 34–35); registration and OAuth/bearer middleware stay TS                                                                                                                                                                                                                                                                       |
...
Goal: make current TS behavior _executable as data_ before a single line of Rust exists. Every later phase's exit gate replays these fixtures.
...
1. **OpenAPI snapshot + regen script.** Check in the filtered `/v1/sdk/*` snapshot plus a script that re-fetches, re-filters (same exclusion of `/v1/sdk/agents`, same prune + placeholder logic as [`generate-types.ts`](../../sdks/typescript/server/scripts/generate-types.ts)), and diffs.
   _Scope:_ snapshot file, `tools/codegen/snapshot-openapi.ts`, CI job.
...
2. **SDK contract manifest.** Write the manifest (§5.1) as the single canonical public-API catalog: operation names, per-language method names, normalization rules, retry/idempotency semantics, error codes + message templates, sync/async availability, and the `createSolvaPay` / `payable` / `protect` / `gate` facade entry points. Include a schema and a validation script.
   _Gotcha:_ the catalog must cover all 38 routed client methods (§2.3 — including `updateProduct` and `getUserInfo`, which are easy to miss because integration docs rarely mention them), every top-level export in [`index.ts`](../../sdks/typescript/server/src/index.ts), and the `@solvapay/core` helpers.
...
7. **Client request/response fixtures.** For every one of the 36 `SolvaPayClient` methods: at least one success and one error fixture, plus one fixture per accepted response shape for the polymorphic methods (§2.3: `getCustomer` ×3 shapes, `listProducts` ×2, `listPlans` ×2 + price precedence, `cancelPurchase`/`reactivatePurchase` nested/flat).
   _Gotcha:_ seed the RNG so auto-generated idempotency keys are deterministic; assert the `Idempotency-Key` header value in `wire` expectations.
...
17. **Error model.** Implement `SdkError` (§4.4) as the single cross-language error surface; map `SolvaPayError` / `PaywallError` construction paths; freeze message templates in the manifest; document the one conversion layer each binding must use (§6.4).
    _Gotcha:_ no parallel error enums in transport/bindings — transport failures become `SdkError::Transport`, never a second public error type.
...
18T. **Doc-comment generation — shared infra + TS (retrofit).** ✅ **Done** (2026-07-23; §15 note 50). Step 18 shipped the generated `.d.ts` without real doc comments; this retrofit landed the cross-surface doc-comment machinery (§5.6 "Doc-comment generation", D19) with TypeScript as the reference surface. IR doc model + surface-agnostic `check_doc_coverage` + CI gate in `dto-gen`; descriptions authored once in manifest `docs:` (never per-language; OpenAPI operation-description linkage deferred — note 50); TSDoc (`/** … */` with `@param` / `@returns`) on `client.generated.d.ts`. Shared infra reused by Python (42T), Ruby (45T), Rust (47), and Go (50) — those steps add only their emitter column.
**Done when:** the doc-comment coverage gate is green for TypeScript; regeneration idempotence and committed-output drift hold for the doc comments (regenerate → zero diff), and hand edits to the generated doc comments fail the `@generated` header gate. **Verified:** coverage CI step + unit tests; regen idempotence; hand-edit/drift/`@generated` bite; TS language-service hover payload; `pnpm --filter @solvapay/server test:types`.
...
20. **WASM Fetch transport.** Implement the Fetch-backed transport behind the same trait via `wasm-bindgen` / `web-sys`, `!Send` futures.
    _Gotcha:_ Workers' Fetch lacks some browser request options; keep the transport surface to what both provide (method/url/headers/body — no streaming request bodies in v1).
    **Done when:** the same round-trip passes under `wasm32-unknown-unknown` (wasm-pack test or Workers miniflare harness). ✅ Landed — Node `wasm-bindgen-test` harness + `tools/conformance/wasm-fixture-server.mjs` (per-fixture `/__case/<i>` mounts); see §15 note 14.
...
24. **Client methods, group C — the rest.** `checkLimits`, `trackUsage`, `trackUsageBulk`, `cancelPurchase`, `reactivatePurchase` (nested-purchase extraction), products (`get`/`list`/`create`/`update`/`delete`/`clone`, `bootstrapMcpProduct`, `configureMcpPlans`), plans (`list` with price precedence, `create`/`update`/`delete`), `getPaymentMethod`, auto-recharge trio.
    **Done when:** their fixtures pass; a coverage script confirms all 38 routed methods are implemented.
...
25. **Shadow-mode harness.** Run TS and Rust clients side by side against the live backend contract-test environment; assert deep equality on normalized responses (see sequence diagram §11.4).
    _Gotcha:_ normalize non-deterministic fields (timestamps, generated refs) via the manifest before comparing; log any divergence with the full wire exchange.
...
34. **MCP payload builders.** Translate the pure builders from [`sdks/typescript/mcp-core`](../../sdks/typescript/mcp-core): `paywallToolResult` (preserving the deliberate `isError: false`, §6.5) and `response-envelope`.
...
35. **MCP names + descriptors.** Translate [`tool-names.ts`](../../sdks/typescript/mcp-core/src/tool-names.ts) (the 12-name `MCP_TOOL_NAMES` table — single source of truth stays single) and the pure parts of [`descriptors.ts`](../../sdks/typescript/mcp-core/src/descriptors.ts).
    ...
    **Step 35 note:** `solvapay-core::mcp::{tool_names,descriptors}` + TS pure extract `descriptor-metadata.ts` rewired into `buildSolvaPayDescriptors` / `buildSolvaPayPrompts`. Handlers / zod / fs / crypto stay host. CSP merge and narration are Rust ops. Live fixture census (`contract/fixtures/census.generated.json`): parsed 787, executed 782, delegated 5. See migration-map Step 35 decisions + §15 note 27.

---

### Phase 6 — Node binding cutover, then edge/browser WASM

> **Post-Step-54:** the migration rollback flag and its resolver / impl-union type were removed. Rollback is republishing the prior package, not a flag flip. The step text below is historical.

The first phase where Rust serves production traffic.

```mermaid
flowchart LR
  CORE6["Rust core"] --> NAPI6["napi-rs native prebuilds<br/>no WASI fallback"]
  CORE6 --> WASM6["wasm-bindgen<br/>@solvapay/server-wasm<br/>edge + browser profiles"]
  NAPI6 --> EXP["@solvapay/server + @solvapay/core<br/>Node: napi · Edge: WASM · Browser: public-safe WASM<br/>full-surface delegation after 37R + 38R<br/>runtime rollback-flag per surface"]
  WASM6 --> EXP
...
36. **Scaffold napi-rs.** Binding package with prebuilds for the §7.7 platform matrix using napi-rs v3's per-target optional-dependency layout. The live targets are the eight native triples in `sdks/node-native`. There is no WASI fallback package. Add the explicit pre-publish artifact gate (every target directory exists and contains exactly the expected `.node` — the napi CLI warns-and-continues on missing artifacts, so CI must hard-fail instead).
    **Done when:** `require` works on every native matrix target in CI.

37. **Wire conditional exports.** `@solvapay/server` → napi-rs native binding (no WASI fallback). The runtime rollback flag from this step was removed after Step 54; rollback is a republish.
    **Done when:** the existing server test suite is green on the binding, and green again with the flag forced back to TS.
    **Step 37 note:** Cut over Node sync `verifyWebhook` only (full-surface napi later). Runtime flag the runtime rollback flag (`ts` / `rust` / unset→prefer rust with silent TS fallback). Loader is sync `createRequire` in `packages/server/src/webhook-native.ts` (never imported by `edge.ts`). `@solvapay/server` optionalDependency on `@solvapay/server-native` via `workspace:*`; CI gate `node-binding-conformance`. See migration-map Step 37 decisions. **The narrow scope of this step is superseded by Step 37R** (full-surface Node napi cutover, below) — the flag semantics, loader pattern, and CI gate carry forward; only the "verifyWebhook only" scope is redone.
...
38. **Edge/browser WASM cutover.** Cut `edge.ts` consumers over to the `wasm-bindgen` build (capability-separated: the browser artifact has no secret-key symbols, §7.1). Record the size and cold-start baselines that become the §7.8 budgets.
    _Gotcha:_ `@solvapay/mcp-core` under Deno resolves `@solvapay/server` to the edge build — its test suite is part of this step's gate.
    **Done when:** edge export tests + mcp-core Deno tests pass and WASM size/cold-start budgets are recorded and met.
    **Step 38 note:** Narrow cutover — only async edge `verifyWebhook` → `solvapay_core::verify_webhook` via `@solvapay/server-wasm` (wasm-bindgen `web` target + runtime wrappers). Node stays on Step 37 napi. Browser profile exports `wasmVersion` only (symbol audit). Budgets in `sdks/wasm/budgets.json`. See migration-map Step 38 decisions + §15 note 29.

39. **Clean-install smoke tests.** Fresh `npm install` + one native call across the platform matrix (glibc, musl, macOS x64/arm64, Windows; Node 22/24/26) as a permanent CI gate. There is no WASI-fallback install path.
    **Done when:** the gate runs green on main.
    **Step 39 note:** Consumer-path gate (not in-tree). `node-binding-artifacts` packs an immutable clean-install bundle (`sdks/node-native/scripts/prepare-clean-install-packages.mjs` → publish-shaped `.tgz` + `manifest.json`). Runtime jobs `node clean install (native, <target>, Node <major>)` (24: 8 targets × Node 22/24/26) download that bundle, `npm install` into an empty temp project, and call public `@solvapay/server`. There is no WASI job. Harness: `sdks/node-native/scripts/clean-install-smoke.mjs`. CI remains PR-triggered (`pull_request` to `main`/`dev`).

37R. **Full-surface Node napi cutover (supersedes Step 37's narrow scope).** Every public method of `@solvapay/server` and `@solvapay/core` becomes a delegation shim over `@solvapay/server-native`: async methods via the napi `tokio_rt` shared runtime, HTTP through the reqwest/rustls transport in `solvapay-transport`, config/error conversion at the binding boundary. runtime rollback-flag semantics extend per-surface (each cut-over surface honors `ts` / `rust` / unset-prefer-rust independently, read per-call as in Step 37). §8 exclusions stay untouched — React, framework adapters, factory ergonomics, deduplicator/caches, MCP registration/middleware glue keep their TS logic. Binding mechanism stays napi-rs per D1 (no C ABI for first-party). Sequenced after Step 39's CI matrix is green and before Phase 7, so Python starts from a proven full-surface binding pattern (§15 notes 31–32).
Confirmed session-sized sub-steps (Step 37R patch plan / §15 note 32): - **37R-a — Binding foundation + client Group A.** New napi `NativeClient` class (constructor: `apiKey`, `apiBaseUrl`) wrapping Rust `SolvaPayClient` over `ReqwestTransport`; async methods for the 10 Group A ops; generalized loader/dispatch module `packages/server/src/native.ts` (supersedes `webhook-native.ts` pattern); `SdkError`→JSON-envelope error mapping in `sdks/node-native/src/error.rs`. Touched: `sdks/node-native/src/{lib.rs,error.rs}`, regenerated `index.d.ts`, `packages/server/src/native.ts` (new), `packages/server/src/client.ts` (per-method dispatch), server unit tests. RED: force the Rust binding, inject a fake `NativeClient` via `setNativeClientForTests`; the 10 Group A methods fail (TS-only) until each dispatches. Done when: `test:unit:rust` + `test:unit:ts` green (≥329 each) with the Rust binding forced and again with the TypeScript body forced (both selection modes existed at the time); Group A client fixtures pass both flags; `node-binding-conformance` green. - **37R-b — Client Groups B + C (26 methods).** ✅ Landed (2026-07-21). Mechanical extension of 37R-a for the remaining typed methods (payment/checkout/process, products/plans/purchases/usage/limits/auto-recharge), reusing the Rust `SolvaPayClient` Group B/C impls and normalization already landed in `solvapay-transport`. RED: same fake-client injection; B/C methods fail until dispatched. Done when: full 36-method client fixture corpus green both flags; suites green both flags. See §15 note 34. - **37R-c — Helper decision cores + paywall + retry.** ✅ Landed (2026-07-21). Delegate the `@solvapay/core` Phase-4 extracts (`customer-sync`, `payment`, `checkout`, `purchase`, `renewal`, `usage`, `limits`, `plans`, `error`, `product`, `paywall-decision`) and the `@solvapay/server` paywall/retry surfaces (`buildPaywallGate`, `buildGateMessage`, `buildNudgeMessage`, `classifyPaywallState`, `paywallErrorToClientPayload`, `withRetry`) — all **sync** napi calls except that `withRetry` keeps host timers and delegates only `RetryPolicy::next_delay` per attempt. Server route-helper `*Core` functions stay TS _orchestration_ shims (auth / `ensureCustomer` / HTTP / `console`) whose _decisions_ already live in delegated `@solvapay/core` extracts — catalogued as "host orchestration, decisions delegated," not as non-delegating logic. RED: force `rust`, stub the sync binding functions absent; helper unit + `helper-*` / `paywall/*` / `retry-schedule` fixtures fail until each shim delegates. Done when: those fixtures + unit suites green both flags; `withRetry` delay-schedule fixtures identical both flags. See §15 note 35. - **37R-d — `@solvapay/core` pure logic + MCP builders.** ✅ Landed (2026-07-21). Sync napi delegation for `business-details`, `credit-display`, `seller-identity` (`@solvapay/core`) and the MCP payload builders (`McpAdapter.formatGate`, `paywallToolResult`, tool-names/descriptors in `@solvapay/mcp-core`) via per-package install-gated wrappers (`native-core.ts` / `native-mcp.ts`) over `payload_builders.rs`. RED: force `rust`, stub binding fns; `business-details`/`credit-display`/`seller-identity` + `mcp/*` fixtures fail until delegated. Done when: those fixtures green both flags; React package tests run **unmodified**. See §15 note 36. - **37R-e — Conformance + gates.** ✅ Landed (2026-07-21). No new delegation; wired the closing gates: full server + core + mcp-core suites + `test:contract` green with the Rust binding forced and again with `ts`; ``shadow:selftest` (deleted)` IDENTICAL (TS driver pins the TypeScript body forced for fetch wire capture); clean-install smoke extended (`verifyWebhook`+`buildPaywallGate`+ host-native`getCustomer`stub);`node-binding-delegation` grep gate (`pnpm delegation:check`+ CI job); React tests unmodified. See §15 note 37.
**Done when:** server + core test suites are green with the Rust binding forced forced and again with`ts`forced, and the`node-binding-delegation` grep inventory shows every non-§8 export delegating to the binding.

38R. **Full-surface edge WASM cutover (supersedes Step 38's narrow scope).** ✅ Landed (2026-07-21). Edge mirror of 37R: every non-§8 export of the `@solvapay/server` edge surface (+ install-gated `@solvapay/core` / `@solvapay/mcp-core`) delegates to `@solvapay/server-wasm`, with per-surface runtime rollback-flag (`ts` forces TypeScript; `rust`/unset prefers WASM with **no silent fallback** on edge). Async client methods route through a wasm-bindgen `WasmClient` over `FetchTransport`; sync decision/paywall/core/MCP surfaces use `initSync` + one JSON-envelope boundary shared with the napi path. The browser profile additionally gains the public-safe pure-logic subset with async warm-up + sync TS fallback (§7.1 / §7.8). Node stays on the 37R napi path; the two graphs never cross-import (`node:module` / `@solvapay/server-native` / `./native` never enter the edge graph; the browser bundle never ships `WasmClient` / `verifyWebhook` / MCP symbols).
Sub-steps (§15 note 38): - **38R-a — WASM foundation + Group A + `wasm.ts`.** `sdks/wasm`: `Cargo.toml` gains `serde` / `solvapay-dto` / `solvapay-transport` / `wasm-bindgen-futures` (the `edge` feature drives `webhook-verify` + the transport client); `error.rs` gains the envelope helpers (`ok_envelope` / `err_envelope` / `internal_error_envelope` / `parse_args_json` / `run_envelope_sync` / async `run_envelope`) mirroring the node binding (webhook-throw path + `js_sys` moved under `#[cfg(feature = "edge")]`); `args.rs` copied/adapted (no napi); `wasm_client.rs` wraps `Rc<SolvaPayClient>` over `FetchTransport` and exposes all 38 routed Groups A–C methods as async JSON-envelope fns (`#![cfg(all(feature = "edge", target_arch = "wasm32"))]`). TS: `packages/server/src/wasm.ts` generalizes `webhook-wasm.ts` (`loadWasmBinding`, `getWasmClient`, `callWasm`, `callWasmSync`, `ensureWasmReadySync`, `resolveEdgeImpl`, envelope reconstruct, test seams); `client.ts` `dispatchClient` splits edge→WASM / Node→napi via runtime check + dynamic import; runtime wrappers (`node`/`workerd`/`deno`/`web`) `export *` + `ready()`/`ensureReadySync()`. - **38R-b — Groups B + C.** Covered by the same `WasmClient` (all 36) + the `client-wasm-dispatch` unit suite exercising B/C exactly as `client-native-dispatch` does for napi. - **38R-c — Sync decisions/paywall/retry via `initSync`.** `decisions.rs` adapted node→wasm (`#[wasm_bindgen(js_name=…)]`, edge-only); `ensureWasmReadySync` (Node edge `readFileSync`+`initSync`; workerd/Deno `initSync`); `edge.ts` installs `installNativeDecisionApi({ callNativeSync: callWasmSync, the implementation selector: resolveEdgeImpl })`; `native-decisions.ts` `dispatchSync` re-gated on install (dropped the `process.versions.node` check). - **38R-d — core + mcp-core install on edge.** `edge.ts` also installs `installNativeCoreApi` + `McpAdapter.formatGate` and publishes `Symbol.for('solvapay.nativeSyncApi')` for mcp-core; `payload_builders.rs` exports the sync builders (public-safe subset dual-profile; MCP subset edge-only); `native-core.ts` / `native-mcp.ts` `shouldAttempt` re-gated on install. Deno smoke stays green; React unmodified. - **38R-e — Browser profile public-safe logic.** Browser feature exports business-details / credit-display / seller-identity sync fns only (no webhook / no `WasmClient`); `check-browser-symbols.mjs` allowlist extended; opt-in `@solvapay/core/browser-wasm` `warmBrowserCoreWasm()` async-warms + installs WASM as the core sync dispatch (main entry stays TS so React is unaffected); browser budget re-recorded with rationale. - **38R-f — Gates.** `DELEGATION_MARKERS` gains `callWasm` / `callWasmSync` / `verifyWebhookWasm`; both-flags server/core/mcp-core suites + delegation check + symbol audit + budgets + Deno edge smoke + clean-install orchestrator unit green. - **38R-g — Docs.** This section + §7.1 / §7.8 / §13 gate row / §15 note 38 / Phase 6 Mermaid + migration-map Step 38R.
**Done when:** the edge export suites are green with the Rust binding forced and again with `ts`, the browser symbol audit + WASM budgets pass, the `node-binding-delegation` inventory recognizes edge markers, and the Deno edge smoke exercises the async webhook plus sync decision/paywall/MCP dispatch end-to-end.
...
39G-a. **Binding-boundary IR + manifest `bindings:` section.** Add the `bindings:` section + Zod schema to the contract manifest; lower it into `Ir.binding_symbols` in [`tools/codegen/dto-gen/src/ir.rs`](../../tools/codegen/dto-gen/src/ir.rs) (§5.7 descriptor: core fn path, per-toolchain export names, ordered JSON-args with boundary types + required/optional + host-injected flags, `split_path_refs` rules, return shape, sync/async, envelope mode). Enumerate today's hand-written symbols (the `decisions.rs` / `payload_builders.rs` / client-dispatch fns across node + wasm). Add the catalog-reconciliation cross-check (every non-§8 core symbol ↔ exactly one `BindingSymbol`).
...
_Gotcha:_ the wasm crate has no `webhook-verify` feature (the `edge` feature drives it) and is `panic=abort` (no `catch_unwind`); the emitter must reproduce the `#[cfg]` gating exactly (§15 note 38) or the diff fails.
**Done when:** regenerated node + wasm shims diff-clean against the committed hand-written output, both-selection-mode server/core/mcp-core suites stay green on the generated shims, and the hand-edit header gate covers the shim files.
...
40. **Scaffold PyO3/maturin.** Package with the tokio runtime via `pyo3-async-runtimes`, GIL-release plumbing for the sync facade, `abi3` wheel config, thread-safe module declaration for free-threaded CPython (§15 note 2). The Rust-side PyO3 binding shims are **generated from the §5.7 emitter** (add the PyO3 toolchain column to the binding-boundary matrix + the PyO3 emitter), not hand-written — this step scaffolds the package/runtime plumbing the generated shims plug into.
    **Done when:** wheels build for the §7.7 matrix and a hello-world call round-trips async _and_ sync.
...
42. **Live contract tests + publish.** Run the backend contract suite from Python; wire PyPI publishing into the release train with version stamping (§7.7). Ship a runnable, tested example under `examples/python/`.
    **Done when:** contract tests green, the wheel installs cleanly into a fresh venv on each matrix platform, and a runnable, tested example exists under `examples/python/`.
    **Example (D20):** ✅ `examples/python/get-merchant` — `run(client)` + offline `http.server` test asserting `/v1/sdk/merchant`; CI in `python-binding` smoke venv.

42T. **Python strict typing + doc comments (retrofit).** ✅ **Done** (2026-07-23; §15 note 51). Steps 40–42 predated the dynamic-language strictness gate (§5.6, D18) and the doc-comment coverage gate (D19). Landed `emit_pyi_py` → committed `__init__.pyi` (JSON-string boundary, async/`_blocking` twins, IR docstrings) + `py.typed`; hardened idiomatic `facade.pyi` (`ApiClient` Protocol, `ParamSpec` `payable`, no public `Any`); pinned `mypy==1.17.1 --strict` + `pyright==1.1.411` (strict) + `ruff==0.12.4` in the `python-binding` smoke job against the public stub surface; CI regen-drift + `@generated` cover `__init__.pyi`.
**Done when:** the dynamic-language type-strictness gate and the doc-comment coverage gate are both green in the `python-binding` job, run against the generated output.
...
45. **Live contract tests + publish.** Run the backend contract suite from Ruby; wire RubyGems publishing into the release train. Ship a runnable, tested example under `examples/ruby/`.
    **Done when:** contract tests green, the gem installs cleanly (`gem install` on each matrix platform, plus one source-compile check), and a runnable, tested example exists under `examples/ruby/`.
    **Example (D20):** ✅ `examples/ruby/get_merchant` — `GetMerchant.run(client)` + offline TCP stub test asserting `/v1/sdk/merchant`; CI in `ruby-binding` full leg.

45T. **Ruby strict typing + doc comments (retrofit).** ✅ **Done** (2026-07-23; §15 note 52). Steps 43–45 predated the dynamic-language strictness gate (§5.6, D18) and the doc-comment coverage gate (D19). Landed YARD (`# @param` / `# @return`) on generated `client.rb` / `helpers.generated.rb` from shared `IrDocModel` via `doc_render::render_entry_doc_lines`; extended generated RBS for `NativeDispatch` + facade internals so `steep==2.0.0 check` is green; pinned `rubocop==1.88.2` (+ `rbs==4.0.3`) in the `ruby-binding` full matrix leg. Optional Sorbet RBI stays a possible follow-on, **not** required for the gate.
**Done when:** the type-strictness gate and the doc-comment coverage gate are both green in the `ruby-binding` job, run against the generated output.
...
46. **Scaffold the `solvapay` facade crate.** Carve idiomatic public API out of `solvapay-transport` + `solvapay-core`: re-exports, `Client::new`, `gate` / payable-style ergonomics (§2.4), optional `blocking` feature. No new semantic logic — ergonomics only. Workspace layout from §4.3; MSRV pinned; docs.rs metadata.
    _Gotcha (updated Step 48):_ consumers depend on the public `solvapay` facade; `solvapay-dto` / `solvapay-core` / `solvapay-transport` are also published (version-locked path deps) so the facade can resolve from crates.io. Version all four with the release train (§7.7, §15 notes 5/53).
    **Done when:** crate builds, a hello-world async call round-trips against a mock transport, and `blocking` feature compiles.

47. **Generate Rust facade signatures + signature-parity suite.** ✅ **Done** (2026-07-23). Emit `pub` signatures from the IR (§5.6); add the Rust **signature-parity test suite** (§2.8) — same assertion classes as TS/Python/Ruby — plus fixture conformance against Phase 0 goldens. Also emit rustdoc (`///`) doc comments on the generated `pub` signatures from the shared IR doc model (Step 18T infra), so hover/docs.rs shows the same descriptions as every other surface (§5.6 "Doc-comment generation", D19). Rust is statically typed, so no dynamic-language strictness gate applies (D18).
    **Done when:** shared fixture conformance passes, the per-language parity check confirms every catalogued entry point, the Rust signature-parity suite is green and structurally equivalent to the TS/Python/Ruby suites, and the doc-comment coverage gate is green for the Rust surface (rustdoc renders on docs.rs).

48. **crates.io publish + docs.rs + live contract tests.** ✅ **Done** (2026-07-23; §15 note 53). Wire publishing into the release train; run the backend contract suite from the Rust facade; verify docs.rs builds. Ship a runnable, tested example under `examples/rust/`.
    **Done when:** contract tests green, `cargo add solvapay` installs cleanly on a fresh project, docs.rs renders the public API, and a runnable, tested example exists under `examples/rust/`. ✅ Landed — publish graph + `publish-rust.yml` / `shadow-rust.yml` / `live-contract` + tested example; first real crates.io upload still maintainer-gated (name reservation + Trusted Publisher).
...
49. **Scaffold wazero binding.** Build the core for `wasm32-wasip1`; embed via `//go:embed`; load with wazero (pure Go, zero cgo). Implement the `Transport` trait contract as a wazero host function backed by `net/http`. Instance pool for concurrent calls (one WASM instance is single-threaded). Map `ctx` cancellation to instance teardown; map `SdkError` → `solvapay.Error` retrievable via `errors.As` (§15 note 4).
    _Gotcha:_ do **not** use cgo — consumer cross-compilation and go-module distribution forbid it (§4.6). Resolve whether the `.wasm` artifact is committed in-repo or attached to release tags before this step's cutover (§13).
    ...
    **Done (2026-07-23):** scaffold landed — `solvapay-go-wasm` WASI guest crate (guest-memory ABI + `solvapay_host::transport_send` import), `dto-gen` `Toolchain::Go` emitting allowlisted `sv_get_merchant` / `sv_verify_webhook` shims via `--go-bindings-out` (golden-gated), and the `github.com/solvapay/solvapay-go` module (embedded `solvapay_core.wasm`, `net/http` host transport, demand-grown bounded instance pool, ctx-cancel teardown). Hello-world surface `Version` / `VerifyWebhook` (`errors.As`) / `Client.GetMerchant` green under `go test -race` incl. concurrent-pool + cancel-teardown; CI `go-binding` job wired (§15 note 54).

50. **Generate the Go facade + signature-parity suite.** Sync-first surface from the IR (§5.6): exported PascalCase names, `ctx context.Context` first, option structs, `Payable`/`Gate` ergonomics (§2.4). The Go binding glue (wazero-export shims + host-side marshalling/reconstruction) is generated alongside the facade signatures from the §5.7 emitter. Add the Go **signature-parity test suite** (§2.8) — same assertion classes as the other surfaces. Also emit godoc doc comments (sentence starting with the identifier name) on the generated exported symbols from the shared IR doc model (Step 18T infra), so hover shows the same descriptions as every other surface (§5.6 "Doc-comment generation", D19). Go is statically typed, so no dynamic-language strictness gate applies (D18).
    **Done when:** shared fixture conformance passes, the per-language parity check confirms every catalogued entry point, the Go signature-parity suite is green and structurally equivalent to the other four suites, and the doc-comment coverage gate is green for the Go surface.
    **Done (2026-07-23):** routed-client guest shims (38 methods, `Toolchain::Go`, no allowlist) + `emit_client_go` / `emit_parity_suite_go` → `client_generated.go` / `signature_parity_generated_test.go`; Phase 0 fixture conformance through the Go facade; godoc from IR; CI regen-drift + `@generated` + `go test -race` green. `Payable`/`Gate` ergonomics deferred (§15 note 55).

51. **Live contract tests + go module release wiring.** Run the backend contract suite from Go; wire tagged module releases into the release train (§7.7). Ship a runnable, tested example under `examples/go/`.
    **Done when:** contract tests green, `go get` installs cleanly into a fresh module on each supported GOOS/GOARCH, and a runnable, tested example exists under `examples/go/`.
    **Done (2026-07-23):** `internal/livecontract` + `cmd/live-contract` + `shadow-go.yml` (dispatch-only); `publish-go.yml` subtree-split dry-run-default → `solvapay/solvapay-go`; tested `examples/go/get-merchant`; CI example + fresh-module smoke + `go-crosscompile` matrix; no skew guard (embedded wasm) (§15 note 56).

---

### Examples — per-language, runnable, tested

Every supported language ships at least one **runnable, tested** example demonstrating the idiomatic SDK surface (client + facade/gating). Examples are **hand-written** — the same category as the `@solvapay/react` package and the MCP-authoring adapters (§8, §9 "MCP-authoring adapters") — **NOT** emitted by the §5.6/§5.7 generators.

The `examples/` directory restructures into per-language subtrees:

```
examples/
  typescript/   # all current TS / Deno / Next.js examples relocate here
  python/
  ruby/
  go/
  rust/
...
- **Relocation:** ✅ **Landed** — every former top-level TS example now lives under `examples/typescript/` (`pnpm-workspace.yaml` glob `examples/typescript/*`; docs/deploy path refs updated).
- **Per language:** `examples/<language>/` holds at least one runnable example plus a test that exercises it (against a mock transport or the offline fixture backend, so examples stay CI-verifiable without live credentials). Uniform family: `get-merchant` for Python / Ruby / Go / Rust (local mock HTTP or injected transport).
- **CI:** a "per-language examples build + test" gate builds and runs each language's example (§10.3). ✅ Wired — Python example pytest in `python-binding` smoke; Ruby example test in `ruby-binding` full; TS `pnpm build:examples` + example unit tests in `lint-build-test`; Go/Rust example gates unchanged.

### After cutover — deletion and C ABI (one deletion step per area, in this order)

52. **Delete superseded TS in `@solvapay/core`.** ✅ Landed (2026-07-24). **Rust-only, full scope:** deletes the 3 domain TS bodies (business-details / credit-display / seller-identity) **and** the 12 Phase-4/5 helper modules' logic bodies; public exports become facades over `native-core.ts` / `native-helpers.ts` → napi / eager browser WASM. the TypeScript body forced no longer covers `@solvapay/core` logic (uninstalled or forced-ts throws `SolvaPayError('core sync API not installed')`). Browser core WASM is **eager** via `@solvapay/core/browser-wasm` (React side-effect import). Retained TS: data tables, `BusinessDetailsSchema` (shape-only Zod), all types, `isRenewalError`, §8 host surface (`SolvaPayError`, `getSolvaPayConfig`, `Env`, `version`). See §15 note 57.
    _Depends on Step 37R_ (and 38R for edge consumers): deletion assumes every export already delegates to the binding — the narrow Step 37/38 cutovers alone do not wire that.
    **Done when:** package tests green with Rust-only logic — verified: `test:unit:rust` 122; React 1083 (napi setup); server 366×2; `test:contract` 1188 both flags; `delegation:check` OK; live fixture census parsed 787, executed 782, delegated 5; browser symbol audit + budgets re-recorded.

53. **Delete superseded TS in `@solvapay/server`.** ✅ Landed (2026-07-24). **Rust-only, full scope:** deletes both `verifyWebhook` TS bodies (`verifyWebhookTs` + `timingSafeEqual` + `node:crypto`), the paywall-state/gate/payload `*-ts` modules, the client method `fetch` bodies, and the `native-decisions` `tsFallback` / `calculateDelayTs`. Node routes sync over `@solvapay/server-native` (napi; internal `verifyWebhook` gained an optional 4th `nowUnixSecs`), edge async over `@solvapay/server-wasm`. the TypeScript body forced no longer selects a server implementation — uninstalled or forced-ts throws (`server sync API not installed` / `server webhook API not installed`). Public signatures unchanged. Retained TS: adapters, fetch route handlers, MCP registration, `factory`, the host `withRetry` loop (only delay selection is Rust), caches/deduplicator, all types. The `server-superseded-ts` grep gate from this step is retired. See §15 note 59. **Parity:** `parity:check` reads the generated binding-symbols snapshot produced from `#[solvapay_export]`. YAML `bindings:` is retired and is not the parity driver.
    _Depends on Steps 37R and 38R_: Node and edge surfaces must both be delegating before their TS bodies can be deleted.
    **Done when:** server suite green; dead duplicated logic stays deleted — verified at landing: server unit rust 356; contract rust 1193; `solvapay-node` 25; edge wasm lib 10; edge unit 72; `delegation:check` + `manifest:check` OK; core 122 / mcp-core 108 / React 1083. `shadow:selftest` and `server-superseded-ts` are not live gates. Live fixture census: parsed 787, executed 782, delegated 5.
...
54. **Publish the optional C ABI.** cbindgen headers, opaque handles, `solvapay_free_*` functions, `SOLVAPAY_ABI_VERSION`, panic containment per §7.6. The third-party C surface reuses the **§5.7 envelope emitter** — the same JSON-envelope marshalling generated for the first-party toolchains (add the C toolchain column + cbindgen emitter), so the C ABI is not a separately hand-written boundary.
    ...
    **Done (2026-07-24 — scaffold):** Landed `sdks/capi` (`solvapay-c`, `publish = false`, `cdylib`+`staticlib`) with generation-counted opaque handles, generic `solvapay_client_call(op, args_json)` returning the §5.7 JSON envelope, embedded tokio current-thread runtime, `catch_unwind` on every FFI edge, cbindgen-committed `include/solvapay.h` + `SOLVAPAY_ABI_VERSION=1`, scaffold allowlist `getMerchant` + sync `solvapay_verify_webhook` / `solvapay_version`. CI: `c-binding` job (build, header drift/`@generated`, `cargo test`/`clippy`, `ctest/run.sh`). The `Toolchain::C` dispatch is generated: 38 routed methods plus 7 MCP composites (45 arms in `sdks/capi/src/dispatch.rs`). See §15 note 58.
...
55. **Promote all compatibility gates.** API diff, cross-surface parity/coverage, homogeneous signature-parity suites for all five surfaces (§2.8), signature-generation reliability gates (§5.6), fixture conformance for all bindings + surfaces, no-unwrap Clippy/deny, size budgets, clean-install matrices, fuzzing (webhook parser, FFI JSON boundaries, C ABI handle misuse) — all required checks on main.
    ...
    **55-c (2026-08-21):** in-repo dry-run path landed (`pnpm release:dryrun`, `dry_run` on `publish.yml` / `publish-preview.yml`). "Required on main" stays open until a maintainer runs `apply-branch-protection.mjs --apply`. See §15 note 62.
...
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `@solvapay/server` conditional exports | **No the TypeScript body forced rollback after Step 53** — paywall/retry/client/webhook semantics are Rust-only; uninstalled / forced-`ts` throws (`server sync API not installed` / `server webhook API not installed`). Node still selects `@solvapay/server-native`, edge `@solvapay/server-wasm`; rollback is republishing the prior facade package |
| `verifyWebhook` Node-sync / edge-async | Thin Rust-only facades keep both public signatures (Node sync over napi, edge async over WASM); the `node:crypto` / `timingSafeEqual` TS bodies were deleted at Step 53. Retained TS is adapter/facade wiring only, covered by `verify-webhook-impl` / `verify-webhook-edge-impl` unit tests                                                            |
| Fetch-runtime validation               | Adapter/route tests remain TS; the client itself is a thin Rust-only dispatch (napi/WASM) with no TS `fetch` body to point back at after Step 53                                                                                                                                                                                                        |
| Deno / Workers                         | WASM build pinning; size/cold-start gate rejects a broken WASM before publish                                                                                                                                                                                                                                                                           |
| React behavior                         | After Step 52: installs napi in tests + eager browser WASM at runtime; React never binds to the C ABI                                                                                                                                                                                                                                                   |
| `@solvapay/core` pure logic            | **No the TypeScript body forced rollback after Step 52** — uninstalled / forced-ts throws; restore requires republishing the prior facade package                                                                                                                                                                                                       |
...
| Dynamic-language type-strictness                     | Generated Python passes `mypy --strict` + `pyright` strict + `ruff`; generated Ruby passes `steep check` + `rubocop` — run against emitted output (§5.6)                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Step 42T ✅ (Python, §15 note 51); 45T ✅ (Ruby, §15 note 52)                                                                                                                                                       |
| Doc-comment coverage                                 | Every catalogued public entry point carries a non-empty language-native doc comment (TSDoc/docstring/YARD/godoc/rustdoc) from the shared IR doc model; generated and drift-checked (§5.6)                                                                                                                                                                                                                                                                                                                                                                                                                                     | Step 18T ✅ (TS + shared infra, §15 note 50); 42T ✅ (Python, §15 note 51); 45T ✅ (Ruby, §15 note 52); 47 ✅ (Rust); 50 (Go)                                                                                       |
| Per-language examples build + test                   | Each language's runnable example under `examples/<lang>/` builds and its test passes (mock/offline backend) (§9 "Examples")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Steps 42 ✅ (Python), 45 ✅ (Ruby), 48 ✅ (Rust), 51 ✅ (Go); TS `examples/typescript/` + `build:examples` ✅                                                                                                       |
| Shared fixture conformance                           | All bindings + all five surfaces replay the golden set                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Step 8 onward; Python offline suite ✅ Step 42; Ruby offline suite ✅ Step 44                                                                                                                                       |
...
| Shadow mode                                          | Live-backend identity check: the npm `@solvapay/server` facade binding (WASM `FetchTransport`) vs the Rust CLI (shadow-invoker) (`shadow:run` (deleted); offline `shadow:selftest` (deleted) in CI). Before Step 53 this was TS-vs-Rust; the facade client is now Rust-only, so the driver injects the real `@solvapay/server-wasm` `WasmClient`. Live CI gate is manual-dispatch until a hosted contract-test environment exists. Python live mirror: `shadow-python.yml` (Step 42). Ruby live mirror: `shadow-ruby.yml` (Step 45). Rust live mirror: `shadow-rust.yml` (Step 48). Go live mirror: `shadow-go.yml` (Step 51) | Step 25; Python ✅ Step 42; Ruby ✅ Step 45; Rust ✅ Step 48; Go ✅ Step 51                                                                                                                                         |
| Clean-install smoke tests                            | Platform matrix (8 native targets × Node 22/24/26)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Step 39                                                                                                                                                                                                             |
...
| server-superseded-ts                                 | Historical Step 53 grep gate (retired in PR 8 with the migration-guard family)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Step 53 ✅; retired PR 8                                                                                                                                                                                            |
| Platform build matrices                              | napi-rs / wheels / gems / WASM / go module / crates.io                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Steps 36, 40, 43 ✅, 46, 49 ✅ (`go-binding`: WASI guest rebuild + committed-wasm staleness + `go test -race`, §15 note 54); Step 51 ✅ (`go-crosscompile` GOOS×GOARCH build+vet + fresh-module smoke, §15 note 56) |
| Pre-publish artifact gate                            | Every expected `.node`/`.wasm`/wheel/gem present (napi CLI won't hard-fail for us)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Step 36; Python `check-wheels.py` ✅ Step 40/42; Ruby `check-gems.rb` ✅ Step 43                                                                                                                                    |
| Python contract suite                                | Offline golden-fixture replay in `python-binding` fresh-venv pytest (census parsed 787, executed 782, delegated 5; 38 routed client ops); live backend via `shadow-python.yml` dispatch-only                                                                                                                                                                                                                                                                                                                                                                                                                                  | Step 42 ✅                                                                                                                                                                                                          |
| PyPI Trusted Publishing                              | maturin abi3 matrix + OIDC (`publish-python.yml`); TestPyPI default; real PyPI gated; §7.7 version stamp + load-time skew guard                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Step 42 ✅                                                                                                                                                                                                          |
| Ruby contract suite                                  | Offline golden-fixture replay in `ruby-binding` `rake test` (`test/contract/`; census parsed 787, executed 782, delegated 5; 38 routed client ops); live backend via `shadow-ruby.yml` dispatch-only                                                                                                                                                                                                                                                                                                                                                                                                                          | Step 44 offline ✅; Step 45 live ✅                                                                                                                                                                                 |
| RubyGems publish                                     | Platform-gem matrix + source gem + gated RubyGems publish (`publish-ruby.yml` OIDC; dry-run = local `gem install`); §7.7 version stamp + load-time skew guard                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Step 45 ✅                                                                                                                                                                                                          |
| Rust live contract suite                             | Offline Phase 0 fixture conformance via facade `Client` (Step 47); live backend via `shadow-rust.yml` dispatch-only + `tools/live-contract`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Step 47 offline ✅; Step 48 live ✅                                                                                                                                                                                 |
| crates.io Trusted Publishing                         | Topo publish dto→core→transport→facade (`publish-rust.yml` OIDC); dry-run default; real publish gated; §7.7 version stamp                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Step 48 ✅                                                                                                                                                                                                          |
| Go live contract suite                               | Offline Phase 0 fixture conformance via facade `Client` (Step 50); live backend via `shadow-go.yml` dispatch-only + `cmd/live-contract`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Step 50 offline ✅; Step 51 live ✅                                                                                                                                                                                 |
| Go module release (`solvapay-go`)                    | git subtree-split `sdks/go` → `solvapay/solvapay-go` + `vX.Y.Z` tag (`publish-go.yml`); dry-run default; real push/tag gated; no skew guard (embedded wasm)                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Step 51 ✅                                                                                                                                                                                                          |
...
| Binding-glue regen (drift / idempotence / hand-edit) | Generated §5.7 shims + native glue in sync, deterministic, and header-protected against hand edits                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Phase 6G (39G-b/c ✅); forward-applied to Python (40–41) + Ruby (43–44)                                                                                                                                             |
...
- Signature-parity suites (§2.8) are the same _type_ of test in every surface language — locking the portable function signatures, not only behavioral fixtures — and are generated from the IR (§5.6).
...
> **Phase 3 note:** Until the napi binding exists (step 36), the Rust side is invoked via a CLI subprocess (`tools/conformance/shadow-invoker`) rather than “via binding”. The sequence is otherwise unchanged.
...
| D9  | Browser WASM is `wasm-bindgen`. Node napi ships native targets only; there is no WASI fallback. An unmatched platform fails to load                                                                                                                                                                            | §4.5                                                                                                   |
...
| D17 | The per-core-symbol binding-boundary descriptor is derived from `#[solvapay_export]`. YAML `bindings:` is retired. IR-driven §5.7 emitters produce every toolchain shim (napi/wasm/PyO3/Magnus/wazero/C) + native-side marshalling. `parity:check` reads the generated binding-symbols snapshot                | §5.7, §5.6, Phase 6G                                                                                   |
| D18 | Dynamically-typed surfaces ship the strictest generated static contract the language allows (Python `.pyi` + `py.typed`; Ruby RBS) and are CI-verified with strict type-checkers + linters (Python `mypy --strict`/`pyright`/`ruff`; Ruby `steep`/`rubocop`; optional Sorbet RBI) against the generated output | §2.6, §5.6, §10.3; Steps 42T, 45T                                                                      |
| D19 | Every generated public signature across all five surfaces carries a language-native doc comment (TSDoc / Python docstring / YARD / godoc / rustdoc) rendered from one shared IR doc model, so LSP hover shows identical descriptions in every language; coverage is a generated, drift-checked CI gate         | §5.1, §5.6, §10.3; Step 18T ✅ (shared infra + TS, §15 note 50); 42T ✅; 45T ✅; 47 ✅ (Rust); 50 (Go) |
| D20 | Each supported language ships at least one runnable, tested example under `examples/<language>/`; examples are hand-written (not generated), built + tested in CI; the current TS examples relocate to `examples/typescript/`                                                                                  | §8, §9 "Examples", §10.3                                                                               |
...
| ----------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Exact WASM size / cold-start numeric budgets                                                                                              | Step 38 baseline; re-recorded step 38R                       | **Resolved (step 38; re-recorded 38R):** see `sdks/wasm/budgets.json` — after the full-surface cutover browser gzip **63633** bytes / cold ~**13.3 ms** (public-safe subset + serde; lazy + opt-in) and edge diagnostic gzip **298838** / ~**16.4 ms** (full transport client + all sync envelopes; lazy); both carry rationale notes; regression >10% needs approval + `--record` |
| Final npm optional-dependency layout + package names for prebuilds                                                                        | Steps 36–37                                                  | **Resolved (step 37):** `@solvapay/server-native` + per-target `@solvapay/server-native-<platform>` for eight native triples (no `-wasm32-wasi` package); `@solvapay/server` optionalDependency `workspace:*` on `@solvapay/server-native`                                                                                                                                         |
| Python package name on PyPI (`solvapay` vs scoped) and minimum CPython (abi3 floor)                                                       | Steps 40–42                                                  | **Resolved (step 40):** PyPI name `solvapay` (unscoped; matches crates.io facade); `abi3-py39` floor (CPython 3.9+)                                                                                                                                                                                                                                                                |
| Ruby gem name + versioning scheme; source-gem toolchain floor                                                                             | Steps 43–45                                                  | **Resolved (step 43/45):** gem + Magnus crate name `solvapay`; Ruby ≥3.0; `SOLVAPAY_RELEASE_VERSION` / `SOLVAPAY_CORE_SHA` stamps; load-time skew guard in `lib/solvapay.rb`                                                                                                                                                                                                       |
| rb-sys-dock cross-compile vs native-host gem builds                                                                                       | Step 45                                                      | **Open (deferred):** `rbsys/*:0.9.128` images expose Ruby 4.0 host + incomplete 3.3 toolchains; Magnus 0.7 fails inside dock. CI/publish use native-host `rake native gem` until dock/Magnus align                                                                                                                                                                                 |
| Go module path naming (`github.com/solvapay/solvapay-go` vs vanity import)                                                                | Steps 49–51                                                  | **Resolved (step 49/51):** module path `github.com/solvapay/solvapay-go` (`go.mod`); release via subtree-split to external `solvapay/solvapay-go` (`publish-go.yml`); vanity import deferred to first tagged release. Maintainer must still create the empty external repo + deploy token before the first real push                                                               |
| Whether the Go WASM artifact is committed in-repo or attached to release tags                                                             | Before step 49 cutover                                       | **Resolved (step 49):** committed in-repo at `sdks/go/solvapay_core.wasm`, `//go:embed`'d; rebuilt by `build-wasm.sh` with a CI staleness gate                                                                                                                                                                                                                                     |
| WASM instance-pool sizing strategy for Go (fixed pool vs demand; max concurrent instances)                                                | Step 49                                                      | **Resolved (step 49):** demand-grown bounded pool (create-on-borrow up to max, block at cap, idle instances returned to a stack); dirty/cancelled instances discarded on ctx-cancel                                                                                                                                                                                                |
| crates.io name reservation for `solvapay` (and whether internal crates are published)                                                     | Before step 46 / Step 48                                     | **Resolved (step 48):** publish all four crates (dto/core/transport/facade) with version+path deps. Maintainer must still reserve the four names before the first real upload                                                                                                                                                                                                      |
...
| ~~37R per-surface runtime rollback-flag semantics~~                                                                                       | **Resolved (37R patch plan)**                                | Per-call `the per-surface implementation selector` in generalized `native.ts`; `ts` / `rust` / unset→prefer-rust-silent-TS-fallback (see §15 note 32)                                                                                                                                                                                                                              |
...
| Free-threaded CPython: declare `gil_used = false` from day one, or after an audit?                                                        | Step 40                                                      | **Resolved (step 40):** keep PyO3 0.28 default thread-safe module (no `gil_used = true` opt-out); lock-light core audit recorded in §15 note 43                                                                                                                                                                                                                                    |
| Fuzz corpus seed strategy (webhook payloads, malformed signatures, FFI JSON)                                                              | Step 55-a                                                    | **Resolved (55-a / §15 note 60):** committed length-prefixed seeds in `internal/fuzz/corpus/<target>/` from Phase 0 webhook + client fixtures via `seed-corpus`; stable replay is the required gate; nightly `cargo fuzz` is dispatch-only. Crashes become extra corpus files.                                                                                                     |
| Whether UniFFI is ever used for a _sixth_ language later                                                                                  | Only if a new language can't use a specialized binding       | §4.6                                                                                                                                                                                                                                                                                                                                                                               |
...
| Whether a later proc-macro layer auto-derives binding-boundary descriptors from the Rust core signatures                                  | Resolved                                                     | Descriptors come from `#[solvapay_export]`. YAML `bindings:` is retired (`manifest-schema` rejects it) and is not the parity driver                                                                                                                                                                                                                                                |
| Hosted contract-test environment for CI shadow live runs                                                                                  | Post–step 25                                                 | Offline `shadow:selftest` (deleted) + Python/Ruby/Rust golden fixtures are in CI; live `shadow:run` (deleted), `shadow-python.yml`, `shadow-ruby.yml`, and `shadow-rust.yml` stay `workflow_dispatch` until a shared sandbox/contract env + secrets exist                                                                                                                          |
| PyPI Trusted Publishing (OIDC) environment + TestPyPI project                                                                             | Step 42                                                      | **Resolved (step 42):** `.github/workflows/publish-python.yml` uses `pypa/gh-action-pypi-publish` with `id-token: write`; TestPyPI is the default dry-run; real PyPI gated by `publish_to_pypi=true` / `solvapay-python-v*` tag. Maintainer must still register the GitHub environment on PyPI/TestPyPI once.                                                                      |
| RubyGems publish workflow + credentials env                                                                                               | Step 45                                                      | **Resolved (step 45):** `.github/workflows/publish-ruby.yml` uses `rubygems/configure-rubygems-credentials` with `id-token: write`; dry-run default is local `gem install` (no TestPyPI equivalent); real `gem push` gated by `publish_to_rubygems=true` / `solvapay-ruby-v*` tag. Maintainer must still register the Trusted Publisher on RubyGems.org once.                      |
...
| Exact per-language type-check/lint toolchain + pinned versions for the dynamic-language strictness gate (§5.6, D18)                       | Steps 42T (Python), 45T (Ruby)                               | **Resolved (step 45T):** Ruby `steep==2.0.0` + `rubocop==1.88.2` (+ `rbs==4.0.3` required by steep 2) in `sdks/ruby` Gemfile `:typecheck` / gemspec; CI in `ruby-binding` full matrix leg. Python earlier: `mypy==1.17.1 --strict` + `pyright==1.1.411` + `ruff==0.12.4` (§15 note 51). No Sorbet RBI.                                                                             |
| Doc-comment coverage on generated signatures (§5.6 / D19)                                                                                 | Steps 42T/45T/47/50 (shared infra ✅ 18T)                    | **Partial (18T + 42T + 45T + 47 Done for TS/Python/Ruby/Rust):** IR `IrDocModel` + `check_doc_coverage` + CI gate + TSDoc (18T); Python docstrings (42T); Ruby YARD (45T); Rust rustdoc on generated facade (`emit_client_rs`, Step 47). Still open: Go emitter column (50)                                                                                                        |
| `examples/` directory layout + per-language example CI wiring, and the `examples/ → examples/typescript/` relocation (§9 "Examples", D20) | When the relocation PR lands (before/with Steps 42/45/48/51) | **Resolved:** TS examples under `examples/typescript/` (`pnpm-workspace.yaml` → `examples/typescript/*`); Python/Ruby/Go/Rust `get-merchant` examples offline-tested; CI gates in `lint-build-test` / `python-binding` / `ruby-binding` / `rust` / `go-binding`                                                                                                                    |
...
4. Implement only that step's scope. The per-step _Scope_ and _Gotcha_ notes are the session brief; §6 has the behavioral contract for whatever module you're translating.
...
**Note 61 — Step 55-b required-checks manifest (checked 2026-08-21):** In-repo enumeration of every expanded `ci.yml` check name so "all required checks on main" is machine-checked rather than Settings-only prose. **Layout:** `contract/required-checks.yaml` (schemaVersion 1, 62 entries) + `scripts/lib/required-checks.ts` (matrix expander for `include:`-only, cartesian axes, and nested `${{ matrix.settings.target }}` / `${{ matrix.target.id }}`) + `scripts/check-required-checks.ts` (`pnpm checks:required` → `required-checks: OK` / `N issue(s)`). Four issue kinds: `missing-required`, `stale-manifest`, `matrix-drift`, `invalid-reason`. Allowed reason codes: `c-abi-smoke-only`, `go-client-suite-only`, `dispatch-only`. **Deferrals (visible, not silent):** `c-binding` stays required but records `c-abi-smoke-only` (`smoke.c` is single-op; generated dispatch is 38 routed methods plus 7 MCP composites); `go-binding` records `go-client-suite-only` (`suite == "client"` filter). **Apply:** `scripts/apply-branch-protection.mjs` prints `PUT repos/:owner/:repo/branches/main/protection/required_status_checks` with `strict: true` + the 62 contexts; `--apply` is maintainer-opt-in and was not run. Wired into `lint-build-test` beside `parity:check` and `.husky/pre-push`. Step 39 flipped Done (27 clean-install names catalogued). Stale "24 uncatalogued extras" / "535 fixtures" claims corrected (550). Step 55 "required on main" stays open until a maintainer applies protection. 55-c landed as note 62.

**Note 60 — Step 55-a fuzzing (checked 2026-08-21):** Compatibility-gate fuzzing without putting nightly/libFuzzer on the required path. **Layout:** `internal/fuzz/` is a detached Cargo workspace (`[workspace]` in `internal/fuzz/Cargo.toml`, not a member of `Cargo.toml`) so `cargo test --workspace`, clippy, and `check-publish-graph.sh` stay untouched. Required check = stable corpus replay (`solvapay-core` `tests/fuzz_corpus_replay.rs` over `webhook_verify` + `envelope_args`; `solvapay-c` replay over `envelope_args` via real `dispatch` + `c_handle`). Nightly run = `.github/workflows/fuzz.yml` `workflow_dispatch` only, toolchain `nightly-2026-08-20`, `-max_total_time` per target, crash artifacts uploaded. **Oracles:** `check_webhook_invariants` (no panic, five `WebhookErrorCode`s, deterministic, `Ok` ≡ `serde_json::from_str(body)`); `check_envelope_invariants_with` (parseable `{ok:true,value}` xor `{ok:false,error:SdkError}`, unknown op → Transport); model-based `HandleOp` sequence against the real `extern "C"` ABI with a shadow live-handle map (`SolvapayStatus::Panic` never observed). Unit tests inject deliberately-wrong stubs so the oracles are proven to _report_ violations. **Seeds:** `internal/fuzz/seed-corpus` writes length-prefixed corpus bytes from Phase 0 webhook fixtures + client fixture args. Corpus is committed. `check-no-unwrap.sh` continues to scan `fuzz/` — targets use `if let` / `abort()` on oracle failure. **Refactor:** six-way `ok_envelope`/`err_envelope`/`parse_args_json` duplication collapsed into `solvapay_core::envelope`; wasm/Go keep a non-unwinding `run_envelope_sync` over `run_envelope_result` (`panic = abort`). **C ABI:** malformed `client_new` JSON is `NullArgument` (parse_args_json Transport was incorrectly mapped to Panic); smoke.c covers double-free, null-handle call, null args to `client_new`. Local webhook fuzz 15s / ~1.27M execs, no crashes. Resolves §13 "Fuzz corpus seed strategy". Next: 55-b required-checks manifest.

**Note 59 — Step 53 delete superseded TS in `@solvapay/server` (checked 2026-07-24):** Rust-only server facade — Node napi + edge WASM are the only execution paths; the TypeScript body forced no longer selects a server implementation. **Webhook:** public `verifyWebhook(options)` unchanged (Node sync over `verifyWebhookNative`, edge async over `verifyWebhookWasm`); deleted `verifyWebhookTs` + `timingSafeEqual` + the `node:crypto` import from `index.ts`/`edge.ts`. Forced-`ts` / unresolved binding throws `SolvaPayError('server webhook API not installed')`. **napi clock:** `verify_webhook` gained an optional 4th `now_unix_secs?: i64` (Some→use it, None→`SystemTime::now()`); `verifyWebhookNative` passes `Math.floor(Date.now()/1000)`, so fixture-frozen clocks reach Rust because the harness mocks `Date.now` (`patchClock`) — no `withForcedImpl('ts')` on webhook fixtures anymore. Rust lib gains ±300s boundary + fixture-clock unit tests (25 in `solvapay-node`); `binding.spec.mjs` gains injected-clock accept/reject (5). **Decisions:** `native-decisions.ts` refactored like core `native-dispatch.ts` — no `tsFallback`, no `*Ts` imports, no `CORE_HELPER_METHODS` special case, no `calculateDelayTs`; throws `SolvaPayError('server sync API not installed')` when uninstalled or `the implementation selector not selecting rust`. `PaywallState` moved to `types/paywall.ts`; `paywall-state.ts`/`paywall-gate.ts` are re-export facades; deleted `paywall-state-ts.ts` / `paywall-gate-ts.ts` / `paywall-payload-ts.ts`. `utils.ts` keeps the host `withRetry` loop (only delay selection is Rust). **Client:** `client.ts` drops `tsFallback` + all `fetch` bodies/URL assembly/response parsing — each method is args-assembly + `dispatchClient` (Node napi / edge WASM); fail-fast on forced-`ts` / missing binding. **Fixtures/shadow:** client wire fixtures inject the real `@solvapay/server-wasm` `WasmClient` and run under `withRustImpl` so the mocked `globalThis.fetch` captures the WASM `FetchTransport` wire (napi `reqwest` is not `fetch`-mockable). Fixed a latent wasm32 panic: `ClientShell` default clock/RNG used `SystemTime::now()`, which aborts on `wasm32-unknown-unknown` → gated to `js_sys::Date::now()` / `Math::random()`; rebuilt `@solvapay/server-wasm`. Shadow `ts-driver.ts` renamed to `facade-driver.ts` (`installFacadeDriverSession` / `FACADE_SHADOW_OPERATION_NAMES`) over the injected WASM client; `orchestrator`/`compare` keep the legacy `ts*` report keys — now the facade side — with compatibility comments. **Characterization:** deleted `create-customer` / `client-topup` / `multi-currency-plans` unit suites (pure deleted-TS wire shape); trimmed `client-error` (apiKey construction guard), `credits-usage` + `bootstrap-mcp` (delegation + type-compile only). **CI:** dropped server `test:unit:ts`, forced-`ts` `test:contract`, `test:unit:edge-ts`, and the forced-`ts` edge webhook replay; aliased `test:unit:ts`→`test:unit:rust` and `test:unit:edge-ts`→`test:unit:edge-wasm`; added `server-superseded-ts` gate beside `delegation:check`. **Generated:** `native.ts` / `wasm.ts` postambles emitted via `dto-gen` chrome snapshot (`tools/codegen/dto-gen/assets/native-ts-emit.snapshot.json`) — regenerated, zero drift (`dto-gen` golden green). **Retained §8 TS:** adapters, fetch route handlers, MCP registration, `factory`, host `withRetry` loop, all types. **New grep gate:** `scripts/lib/superseded-server-ts-check.ts` + `scripts/check-superseded-server-ts.ts` (5 self-tests). **GREEN:** server unit rust 356 (26 files); contract rust 1193 (13 files); `shadow:selftest` 19; fixture census parsed 787, executed 782, delegated 5 (104 skipped-unbound); `cargo test -p solvapay-node` 25; `solvapay-wasm --features edge --lib` 10; `dto-gen` golden (zero drift); `binding.spec.mjs` 5; `delegation:check` / `manifest:check` / `server-superseded-ts:check` OK; server `test:types` + `build` OK; core 122 / mcp-core 108 / react 1083. **Parity follow-up:** `parity:check` now consults `manifest.bindings` `names.ts` directly, so cross-language decision helpers (`classifyCancelError`, `decidePaywallOutcome`, …) no longer need per-helper `TS_ONLY_ALLOWLIST` entries; genuine TS-only seams (`callNativeSync`, WASM test helpers, adapters, `*Core` route helpers) remain allowlisted. Next: Step 54 (already landed as Note 58 C ABI) / Step 55.

**Note 58 — Step 54 optional C ABI scaffold (checked 2026-07-24):** Native (not WASM) third-party C surface at `sdks/capi` (`solvapay-c`). **Shape:** opaque generation-counted handles packed into `*mut SolvapayClient` (free bumps generation → checked `InvalidHandle`, never UB); single generic dispatch `solvapay_client_call(handle, op, args_json) -> char*` returning the universal §5.7 JSON envelope (`{"ok":true,"value":…}` \| `{"ok":false,"error":<SdkError>}`); callee-allocated strings freed via `solvapay_free_string`; `catch_unwind` on every `extern "C"` edge (§7.6). **Runtime:** embedded tokio current-thread `block_on` (facade `blocking` precedent) over `ReqwestTransport` + `ClientShell` + `SolvaPayClient`. **Scaffold allowlist:** `getMerchant` (+ `solvapay_verify_webhook` / `solvapay_version` / `solvapay_abi_version`); unknown ops → Transport envelope. **Header:** cbindgen `include/solvapay.h` committed + drift-checked in `c-binding` CI (not dto-gen `GENERATED_PATHS` — `build.rs` emits it). Generated `Toolchain::C` dispatch covers 38 routed methods plus 7 MCP composites (`sdks/capi/src/dispatch.rs`). Additive / `publish = false` — no npm/Py/Ruby/Go/crates.io surface change. GREEN locally: `cargo test -p solvapay-c` (18), clippy `-D warnings`, `./bindings/c/ctest/run.sh` PASS (create/call/free + use-after-free + garbage handle). Manual smoke confirmed — map row **Done**.

**Post-Step-54 cleanup (checked 2026-07-24):** Deleted the migration rollback flag entirely — `SOLVAPAY_IMPL`, `resolveImpl`, and `SolvaPayImpl` are gone from runtime `@solvapay/server` / `@solvapay/core` (Rust-only since Steps 52/53). Follow-up sweep removed remaining env pins from CI, checkout-demo smoke, shadow harness, clean-install smokes, and workerd fixtures; The `server-superseded-ts` gate is retired. Kept offline `parity:check`, which reads the generated binding-symbols snapshot from `#[solvapay_export]`, not YAML `bindings:`, signature-parity generated suites, and the shadow harness (WASM client injection only).

**Note 57 — Step 52 delete superseded TS in `@solvapay/core` (checked 2026-07-24):** Rust-only core facade, full scope (3 domains + 12 helper modules). **Design:** `native-dispatch.ts` shared install/dispatch; `native-core.ts` / `native-helpers.ts` facades call `dispatchSync` with **no TS fallback** — uninstalled or `the implementation selector not selecting rust` throws `SolvaPayError('core sync API not installed')`. Server/edge/fixture installs force `an install that always selected rust` for core so ambient the TypeScript body forced still reaches napi/WASM for core helpers; `native-decisions` preferred native for the 36 core-helper methods even under `ts` (paywall/retry TS bodies were deleted later in Step 53 — see note 59). **Browser:** `@solvapay/core/browser-wasm` eagerly warms + installs on import; React side-effect imports it; vitest stubs the entry and installs napi. **Retained TS:** country/tax const tables, shape-only `BusinessDetailsSchema`, all types, `isRenewalError` (host guard, no Rust binding), §8 `SolvaPayError`/`getSolvaPayConfig`/`Env`/`version`. **Allowlist:** removed ~36 `portable-ts-fallback` core rows; `isRenewalError` → `type-guard-or-const`. **CI:** dropped core `test:unit:ts` dual-flag (aliased to rust); React conformance notes napi setup. **RED evidence:** `native-core.unit.test.ts` / `native-helpers.unit.test.ts` assert throw-when-uninstalled + sentinel-when-installed; browser-wasm tests assert no TS fallback. GREEN: core 122, server 366×2, React 1083, contract 1188×2, `delegation:check`, fixture census parsed 787, executed 782, delegated 5, browser symbol audit + budgets re-record. Next: Step 53 (server TS deletion) — ✅ landed as note 59.

**Note 56 — Step 51 Go live contract + subtree publish + example + cross-compile (checked 2026-07-23):** Closed Phase 10. **Live:** `sdks/go/internal/livecontract` ports Python/Ruby/Rust helpers (`SCENARIOS` / `ResolveArgs` / `Normalize` / `ScoreScenario` / `ExtractRef`) and `cmd/live-contract` drives `*solvapay.Client` via shared `internal/dispatch` reflection (also reused by `fixture_conformance_test.go`); writes `contract/shadow/output/go-live-report.json`. **Shadow:** `.github/workflows/shadow-go.yml` is `workflow_dispatch`-only until a hosted contract env exists. **Publish:** `.github/workflows/publish-go.yml` rebuilds + staleness-checks the embedded wasm, `go build`/`go vet`s the monorepo module, `git subtree split --prefix sdks/go`, verifies the split module builds, dry-run default; real push/tag to `github.com/solvapay/solvapay-go` gated by `publish_to_go` / `solvapay-go-v*` + `SOLVAPAY_GO_DEPLOY_TOKEN`. **No skew guard:** `//go:embed`'d `solvapay_core.wasm` ships inside the same module tarball as the facade (one git tag = one module+wasm) — same reasoning Note 53 used to skip a Rust skew guard. **Example:** `examples/go/get-merchant` extracts testable `run(...)` + `httptest` coverage (local `replace` for CI; consumers `go get github.com/solvapay/solvapay-go`). **CI:** `go-binding` gains example test + fresh-module local-replace smoke; new `go-crosscompile` matrix `{linux,darwin,windows}×{amd64,arm64}` (`CGO_ENABLED=0` `go build`/`go vet`). **Deferred:** maintainer creation of empty `solvapay/solvapay-go` + first real tag; hosted contract env for non-dispatch live CI; `Payable`/`Gate` + Go `decisions.rs` shims. Sources: §7.7; Steps 42/45/48; §15 notes 53–55.

**Note 55 — Step 50 Go facade + signature-parity (checked 2026-07-23):** Expanded Phase 10 beyond the Step 49 scaffold. **Guest shims:** dropped `GO_CLIENT_ALLOWLIST`; `emit_go_client` emits all 38 routed Groups A–C `sv_*` exports using shared `client_call_body` (`clientIgnore` / `clientAwait` / `clientSplit`) wrapped in `pollster::block_on`; `args.rs` gains `split_path_refs` (+ `ClientConfig` / `VerifyWebhookArgs`); DTO deserialize types imported from `solvapay_dto`. **Facade:** new `emit_client_go` → `sdks/go/client_generated.go` — ctx-first PascalCase methods, body/path args as `map[string]any` / `string`, returns `any` (arrays from list ops), godoc first-word = identifier via `render_entry_doc_lines`; hand-written `GetMerchant` removed. **Parity:** `emit_parity_suite_go` → `signature_parity_generated_test.go` (36-count, sorted-unique, compile-time `(*Client).Method` refs, sync-only matrix, frozen `defaults:`). **Fixtures:** `fixture_conformance_test.go` replays Phase 0 `contract/fixtures/client/**` through the facade against `httptest`. **CLI/CI:** `--go-client-out` / `--go-parity-out`; regen-drift + `@generated` cover both paths + wasm shims; goldens `go_shim_golden` / `go_client_golden` / `go_parity_golden`. **Deferred:** `Payable`/`Gate` ergonomics + Go `decisions.rs` shims (Step 51+). Sources: Step 47 Rust facade pattern; §15 note 54.

**Note 54 — Step 49 wazero (Go) binding scaffold (checked 2026-07-23):** Opened Phase 10. **Guest crate:** `sdks/go/wasm` (`solvapay-go-wasm`, `crate-type = ["cdylib"]`, `wasm32-wasip1`, workspace member; `wasm32-wasip1` added to `rust-toolchain.toml`) compiles `solvapay-core` (`server` feature) + `solvapay-transport` behind a guest-memory ABI — `sv_alloc` / `sv_dealloc`, exports return packed `u64` `(ptr<<32)|len`; `sv_version` / `sv_verify_webhook` (sync envelope) / `sv_get_merchant` (async via `pollster::block_on`); `sv_client_new` configures a `thread_local` client from `{"apiKey","apiBaseUrl"}`. **Host transport:** `#[link(wasm_import_module = "solvapay_host")] transport_send(req_ptr,req_len)->u64`; `WasiHostTransport: Transport` serializes `HttpRequest` as `{method, url, headers:[[name,value]], body}` JSON, guest allocates the response via `sv_alloc`. **Transport crate WASI fix:** narrowed `js-sys`/`wasm-bindgen*`/`web-sys` + `fetch_transport`/`FetchTransport` cfg from `target_arch = "wasm32"` to `all(target_arch = "wasm32", target_os = "unknown")` so `wasm32-wasip1` (`target_os = "wasi"`) no longer pulls `web-sys`; `SharedTransport` stays `Arc<dyn Transport>` for all wasm32. **Emitter:** `dto-gen` gained `Toolchain::Go` (lang `go`, empty attr macro, `Rc` clone-ty like Wasm, `pick_doc = &sym.doc`) emitting `#[no_mangle] pub unsafe extern "C" fn sv_<fn>(args_ptr,args_len)->u64` client shims (CLIENT allowlist = `getMerchant` for the scaffold) + `sv_verify_webhook` + shared `args.rs`; Go chrome embedded as Rust string constants (not the JSON snapshot) since the FFI pattern is distinct; `--go-bindings-out` → `write_go_shim` writes `args.rs` / `client.rs` / `webhook.rs` + `rustfmt`; golden `go_shim_golden.rs`. **Go module:** `github.com/solvapay/solvapay-go` (`go.mod`, `require github.com/tetratelabs/wazero`) `//go:embed`s the committed `solvapay_core.wasm`; `internal/runtime` compiles once and keeps a **demand-grown bounded instance pool** (semaphore cap + idle stack; one instance is single-threaded); host `transport_send` does `net/http`; `Version` / `VerifyWebhook` / `Client.GetMerchant`; `solvapay.Error` mirrors `SdkError` for `errors.As`; ctx-cancel discards the borrowed (dirty) instance rather than returning it. **Artifact:** `build-wasm.sh` builds `--profile wasm-release`, `wasm-opt -Oz` when present else commits the cargo artifact verbatim; target dir resolved via `cargo metadata`. **CI:** `go-binding` job (rustup + `wasm32-wasip1`, Go 1.23, rebuild + committed-wasm staleness gate, gofmt/vet, `go test -race ./...`); regen adds `--go-bindings-out bindings/go/wasm/src` with the three shims in the drift + `@generated` gates. Confirms §15 note 4 / D14 (no cgo). Sources: §15 note 4; [wazero](https://wazero.io/), [tetratelabs/wazero](https://github.com/tetratelabs/wazero).

**Note 53 — Step 48 crates.io publish + docs.rs + live contract (checked 2026-07-23):** Closed Phase 9. **Publish graph:** flipped `solvapay-dto` → `solvapay-core` → `solvapay-transport` → `solvapay` to `publish = true` with `version` + `path` deps and shared `[workspace.package]` metadata (`repository` / `keywords` / `categories` / READMEs / `[package.metadata.docs.rs]`). Decision: publish all four crates (no skew guard — single compiled crate, unlike Python/Ruby facade↔native). **Live:** `tools/conformance/live-contract` (lib + bin, `publish = false`) ports Python/Ruby helpers (`SCENARIOS` / `resolve_args` / `normalize` / `score_scenario`) and drives `solvapay::blocking::BlockingClient`; writes `contract/shadow/output/rust-live-report.json`. **Shadow:** `.github/workflows/shadow-rust.yml` is `workflow_dispatch`-only until a hosted contract env exists. **Publish:** `.github/workflows/publish-rust.yml` stamps `SOLVAPAY_RELEASE_VERSION` / `SOLVAPAY_CORE_SHA`, dry-run default, real publish via OIDC (`rust-lang/crates-io-auth-action`) gated by `publish_to_crates_io` / `solvapay-rust-v*` (topo order + index wait). **Example:** `examples/rust/get-merchant` extracts testable `run(...)` + mock-transport integration test. **CI:** graph check + `cargo publish -p solvapay-dto --dry-run`, nightly `RUSTDOCFLAGS=--cfg docsrs cargo +nightly doc -p solvapay --all-features` (stable cannot enable `feature(doc_cfg)`), example build+test, `cargo test -p live-contract` via workspace. **Deferred:** maintainer name reservation + Trusted Publisher registration before first real upload; `cargo add solvapay` / docs.rs render after that. Sources: §7.7; Python/Ruby Steps 42/45; §15 note 5.

**Note 52 — Step 45T Ruby strict typing + doc comments (checked 2026-07-23):** Landed the Ruby emitter column for D18/D19 on top of Step 18T infra. **Emitter:** `emit_client_rb` `render_yard` writes YARD (`# summary` / `# @param` / `# @return`, mapping shared `@returns` → YARD `@return`) above every generated `def` in `client.rb` + `helpers.generated.rb`; hand-authored YARD on `facade.rb` / `helpers.rb` for `create` / `gate` / `payable` / `protect` / `verify_webhook` / `with_retry`. **RBS:** `emit_rbs_rb` extended with `NativeDispatch`, facade/private surfaces, binding-arg helper params, and result constructors so `steep check` is green against committed `sig/solvapay.rbs` (no Sorbet RBI). **Pinned toolchain** (Gemfile `:typecheck` + gemspec): `steep==2.0.0`, `rubocop==1.88.2`, `rbs==4.0.3` (steep 2 requires rbs ~> 4). CI `ruby-binding` full matrix leg runs `bundle exec steep check` + `bundle exec rubocop lib` after `rbs validate`. **Gates:** `ruby_doc_coverage` integration test + regen-drift / `@generated` cover YARD + RBS. Sources: redesign §5.6 / D18–D19; migration map row 45T.

**Note 51 — Step 42T Python strict typing + doc comments (checked 2026-07-23):** Landed the Python emitter column for D18/D19 on top of Step 18T infra. **Emitter:** `dto-gen` `emit_pyi_py` + `--py-stub-out` writes `sdks/python/python/solvapay/__init__.pyi` (JSON-string boundary `args_json: str` → `str`; async + `_blocking` twins from `availability.py`; docstrings from `IrDocModel` via shared `doc_render::render_entry_doc_lines`). **Marker:** empty `py.typed` (already in maturin `include`). **Facade:** hand-written `facade.pyi` hardened — public `ApiClient` Protocol, `ParamSpec`-typed `payable`, no `Any` / `__getattr__`; companion `errors.pyi` / `results.pyi` / `_solvapay.pyi`. **Pinned toolchain** (optional-deps group `typecheck`): `mypy==1.17.1`, `pyright==1.1.411`, `ruff==0.12.4` — CI `python-binding` smoke runs them against the public stub surface after fresh wheel install. **Gates:** regen-drift + `@generated` header cover `__init__.pyi`; emitter tests assert every catalogued operation twin has a non-empty docstring (backed by `check_doc_coverage`). **Scope decision:** keep JSON-string boundary (no per-DTO TypedDicts in 42T); `examples/python/` remains a separate D20 deliverable. Sources: redesign §5.6 / D18–D19; migration map row 42T.

**Note 50 — Step 18T doc-comment generation / manifest `docs:` as authored source (checked 2026-07-23):** Landed the shared IR doc model (`IrDocModel` on `IrEntryPoint`) + surface-agnostic `check_doc_coverage` (wired into `generate_from_snapshot` after `lower_catalog`) + TSDoc emission on `client.generated.d.ts`. **Authored source decision:** entry-point docs are authored only in the contract manifest `docs:` block (`summary` / `params.<name>` / optional `returns`) — not yet linked from OpenAPI operation `description`. Rationale: `dto-gen` `parse_openapi` still captures only method/path/operationId/body refs (no operation description), and catalog ids (`checkLimits`) do not map 1:1 to OpenAPI `operationId` (`LimitsSdkController_…`). DTO field/type docs already flow from OpenAPI via `doc_of`. Per-parameter docs: manifest `docs.params.<name>` wins over inline `params[].doc` when both exist. A later step can add an OpenAPI-operation-description fallback by linking routes without changing the IR shape. CI: named "doc-comment coverage gate" (`cargo test -p dto-gen doc_coverage`) plus transitive regen-drift / `@generated` enforcement. Sources: redesign §5.1 / §5.6 / D19; `contract/manifest/sdk-contract.yaml` `docs:` catalog.

**Note 1 — napi-rs (checked 2026-07; Node WASI fallback removed):** v3 (announced 2025-07) is current. Prebuilds ship as per-target npm packages under `optionalDependencies`. Upstream napi-rs can load a WASI artifact; `@solvapay/server-native` does not. `sdks/node-native` targets are the eight native triples, and an unmatched platform fails to load. Browser and edge use `@solvapay/server-wasm` (wasm-bindgen), which confirms D9. Cross-compilation is first-class (`--use-napi-cross`, cargo-zigbuild, cargo-xwin). Upstream CI covers Node 22/24/26. The napi CLI _warns and continues_ when a target artifact is missing at publish time — hence the hard pre-publish artifact gate in step 36. Sources: [napi.rs v3 announcement](https://napi.rs/blog/announce-v3), [release guide](https://napi.rs/docs/deep-dive/release).
...
**Note 3 — Magnus / rb-sys (checked 2026-07; revisited Step 43 2026-07-23):** Magnus (high-level) over rb-sys (raw C-API bindings) remains the recommended stack; gems build via the `rb_sys` gem + `rake-compiler` with `bundle gem --ext=rust` scaffolding, `crate-type = ["cdylib"]`, `#[magnus::init]` entry point. Precompiled platform gems _intended_ via `rb-sys-dock` (Docker); production precedent includes wasmtime-rb and blake3-rb. **Step 43 finding:** current `rbsys/*:0.9.128` dock images ship a Ruby 4.0 host + incomplete 3.3 toolchains that break Magnus 0.7 — CI therefore builds platform gems on native runners (`rake native gem`) and defers dock cross-compile to Step 45. Crate package name must be `solvapay` (not `solvapay-ruby`) to match the ExtensionTask artifact / `Init_solvapay`. Ruby ≥3.0 floor. GVL release for blocking calls goes through Magnus `without_gvl`. Sources: [magnus](https://github.com/matsadler/magnus), [rb-sys](https://github.com/oxidize-rb/rb-sys), [oxidize-rb docs](https://oxidize-rb.org/docs/); §15 note 46.
...
**Note 14 — Step 20 WASM Fetch transport (checked 2026-07-17):** Pinned the wasm-bindgen release train **0.2.126** end-to-end: `wasm-bindgen 0.2.126`, `js-sys`/`web-sys 0.3.103`, `wasm-bindgen-futures 0.4.76`, `wasm-bindgen-test 0.3.76`, and `wasm-bindgen-cli 0.2.126` (CLI must match the crate pin exactly — `.cargo/config.toml` sets `runner = "wasm-bindgen-test-runner"`). Harness choice: **Node-based `wasm-bindgen-test`** (lighter than miniflare; Workers-specific validation stays step 38). `FetchTransport` resolves `fetch` from `js_sys::global()` (not `web_sys::window()`) so the same path works in browsers, Workers, and Node ≥18. Request surface is method/url/headers/body only — body as `Uint8Array`, no streaming (Workers `RequestInit` parity). Error mapping: request-build failures (bad URL/headers) → `SdkError::Transport { retryable: false }`; fetch rejection (network/DNS/refused) → `retryable: true`; non-OK HTTP stays `HttpResponse`. Fixture delivery: `tools/conformance/wasm-fixture-server.mjs` serves `GET /__fixtures` plus per-fixture mounts at `/__case/<index><wire.path>` — required because ~34 wire request shapes are shared across multiple response variants (client normalization cases); a single global matcher first-hits the wrong response. Wrapper: `tools/conformance/test-wasm-transport.sh`. Sources: [wasm-bindgen 0.2.126](https://crates.io/crates/wasm-bindgen/0.2.126), [web-sys fetch example](https://rustwasm.github.io/docs/wasm-bindgen/examples/fetch.html), [wasm-bindgen-test usage](https://rustwasm.github.io/docs/wasm-bindgen/wasm-bindgen-test/usage.html), [Workers limits](https://developers.cloudflare.com/workers/platform/limits/).
...
**Note 27 — Step 35 MCP names + descriptors (checked 2026-07-17):** No new crate deps / toolchain pins under the step-8 freeze — serde/serde_json only. Pure cores: `MCP_TOOL_NAMES` (12), `TOOL_FOR_VIEW`/`VIEW_FOR_TOOL`, `derive_icons`, `build_tool_descriptor_metadata`, `build_prompt_descriptor_metadata`, `build_prompt_user_message`, `validate_public_base_url`. TS extract `packages/mcp-core/src/descriptor-metadata.ts` rewired into `descriptors.ts` (handlers/schemas/fs/crypto stay host; CSP merge and narration are Rust ops). Decision impact: registration order is contract (intent → transport → activate_plan); skip-absent `title`/`icons`/annotation flags; `deriveIcons` undefined → fixture `null`; frozen `publicBaseUrl` message stays a local const (no manifest `errors.mcp` key). Characterization extended on `descriptors.unit.test.ts` before extract. GREEN: +20 fixtures under `mcp/{tool-names,derive-icons,descriptors,prompts}`. Live census: parsed 787, executed 782, delegated 5. Sources: step-32 TS-extract rewire pattern; step-34 `mcp/` fixture layout; no upstream research beyond confirming no new deps needed.

**Note 49 — Step 46 Rust public facade scaffold (checked 2026-07-23):** Opened Phase 9 with `sdks/rust` — async-first public crate over `solvapay-transport` + `solvapay-core` (no new semantic logic). **Surface:** `Config` (`SOLVAPAY_SECRET_KEY` default, 10s limits TTL), `Client::new` (native `ReqwestTransport`) / `Client::with_transport`, delegated `get_merchant`, §2.4 `gate` / `payable` / `Allow::track_success|track_fail` with tokio-backed limits cache + customer single-flight mirroring Ruby/Python facades; `decide_paywall_outcome` / `evaluate_*_limits` stay core. **Feature:** `blocking` enables `solvapay::blocking::Client` (current-thread `block_on`, native-only). **Workspace naming:** Magnus cdylib workspace package renamed to `solvapay-ruby` with `[lib] name = "solvapay"` so the facade keeps crates.io name `solvapay`; CI Ruby smoke uses `cargo test -p solvapay-ruby --lib`. **Gates:** `cargo test -p solvapay` + `--features blocking`; explicit CI step (workspace builds do not enable optional features); `publish = false` until Step 48 resolves internal-crate publish vs path-deps (§15 note 5). **Deferred:** Step 47 generated 35 remaining client methods + signature-parity; Step 48 docs.rs / live contract / publish train. Sources: §2.4 Rust sketch; §4.3 layout; Ruby `facade.rb` host plumbing; transport `MockTransport` pattern.

**Note 48 — Step 45 Ruby live contract + RubyGems publish (checked 2026-07-23):** Closed Phase 8. **Skew guard:** load-time `SolvaPay._check_version_skew` in `lib/solvapay.rb` compares gem `VERSION` to native `version` and raises `SolvaPayError(code: "version_skew")` on mismatch; covered by `test/version_skew_test.rb`. **Live:** stdlib `scripts/live_contract.rb` (`SolvaPay::LiveContract` pure helpers + guarded `main`) ports the Python driver — same scenario order, `requires:stripe|active_purchase` skips, volatile normalize, structured-error scoring — invoking the public keyword `Client` (no `_blocking` twins). Offline helper coverage in `test/live_contract_test.rb`. **Shadow:** `.github/workflows/shadow-ruby.yml` is `workflow_dispatch`-only (mirrors `shadow-python.yml`) until a hosted contract env exists. **Publish:** `.github/workflows/publish-ruby.yml` — native-host §7.7 platform matrix + source gem, stamps `lib/solvapay/version.rb` + `SOLVAPAY_RELEASE_VERSION`/`SOLVAPAY_CORE_SHA`, `check-gems.rb`, dry-run default = local `gem install` (RubyGems has no TestPyPI), real `gem push` via OIDC (`rubygems/configure-rubygems-credentials`) gated by `publish_to_rubygems=true` / `solvapay-ruby-v*` tag. **Deferred:** rb-sys-dock cross-compile (`rbsys 0.9.128` / Magnus 0.7 mismatch); keep `scripts/dock-build.sh` and native-host `rake native gem`. Sources: §7.7; Python Step 42 / Note 45; §10.3 Ruby contract/RubyGems rows.

**Note 47 — Step 44 Ruby facade + binding glue (checked 2026-07-23):** Grew `Toolchain::Ruby` from the Step 43 hello-world to the full §5.7 surface. **Magnus shims:** `emit_ruby_client` emits all 38 routed Groups A–C methods as sync GVL-releasing wrappers (`without_gvl` + `block_on`); decisions (42) + payload builders (23) emit over `run_envelope_sync`; shared `args.rs` + generated `register.rs` wires every free fn into private `SolvaPay::Native`. **Native marshalling:** `emit_native_rb.rs` + `--native-rb-out` produce `_native.rb` (`CLIENT_METHODS`/`SYNC_METHODS` from IR `names.rb`) as the only JSON-envelope parser and `SdkError` reconstructor. `SolvaPayError` is canonical; `SolvaPay::Error` remains an alias for native raises. **Public facade:** generated keyword-only `client.rb` + `helpers.generated.rb` + `sig/solvapay.rbs`; hand-written `SolvaPay.create` / `payable(...).protect` / `gate`; Ruby owns blocking retry sleep, a true Mutex/ConditionVariable customer single-flight (no mutex held during HTTP), 60s successful customer cache, and 10s limits cache. **Parity:** `emit_parity_suite_rb` + `--rb-parity-out` produces `signature_parity_generated_test.rb` with real IR defaults, exact `Method#parameters`, blocking-only availability, and no `_blocking`/async twins. **Offline conformance:** `sdks/ruby/test/contract/` replays all 535 shared fixtures through deterministic clock/RNG seams and a local TCP stub; the separate coverage guard requires success and error fixtures for all 36 operations. **Gates:** CI regen-drift + `@generated` cover all five Magnus files, `_native.rb`, public wrappers/helpers, RBS, and parity; platform legs compile/smoke while one host runs full Minitest + RBS. GREEN: dto-gen 44 unit tests plus all goldens/idempotence; `solvapay` 14 unit tests; Ruby 557 tests / 2,000 assertions; `rbs validate`; focused Python 51 tests; TS manifest/type parity. Live contract, RubyGems publish, skew guard, and dock revisit landed in Step 45 (note 48). Sources: §15 notes 3/46; Python Step 41/42 patterns; §2.4 / §5.6 / §5.7.

**Note 46 — Step 43 Magnus/rb-sys scaffold (checked 2026-07-23):** Scaffolded `sdks/ruby` gem (`solvapay`, Ruby ≥3.0) + Magnus cdylib crate **named `solvapay`** (must match `rb_sys` ExtensionTask / `Init_solvapay` artifact; plan wording "solvapay-ruby" refers to the binding path). **Runtime:** binding-owned multi-thread tokio; blocking calls release the GVL via Magnus `without_gvl`. **Hello-world surface:** `version()` / `native_build_info()`, sync `verify_webhook` (`catch_unwind` → `SolvaPay::Error` with `code`), generated allowlisted `client.rs` `get_merchant`. **Emitter:** `Toolchain::Ruby` + `artifacts.ruby` chrome in `binding-emit.snapshot.json`; `--ruby-bindings-out` writes scaffold shims (full 102-symbol emission landed in Step 44). **CI:** `ruby-binding` matrix on native hosts (x86_64/aarch64 linux, x86_64/arm64 darwin, x64-mingw-ucrt) + source-gem compile smoke + `check-gems.rb` hard-fail artifact gate; regen-drift covers generated paths. **Dock deferral:** current `rbsys/*:0.9.128` images expose Ruby 4.0 host + incomplete 3.3 toolchains; Magnus 0.7 fails inside dock — `scripts/dock-build.sh` kept for Step 45 revisit. GREEN: `cargo test -p solvapay --lib`; smoke minitest; `ruby_shim_golden`. Sources: §15 note 3; oxidize-rb `setup-ruby-and-rust`; Magnus 0.7 / rb-sys 0.9.

**Note 44 — Step 41 Python facade + binding glue (checked 2026-07-23):** Grew `Toolchain::Python` from the Step 40 hello-world to the full §5.7 surface. **PyO3 shims:** `emit_python_client` emits all 38 routed Groups A–C methods as `future_into_py` + `{name}_blocking` twins; decisions (42) + payload builders (23) emit as `#[pyfunction(name = "<snake>")]` over `run_envelope_sync`; shared `args.rs` + generated `register.rs` wires every free fn into `_solvapay`. **Native marshalling:** `emit_native_py.rs` + `--native-py-out` produce `_native.py` (`ClientMethod`/`SyncMethod` Literals from IR `names.py`, `unwrap_envelope` / `reconstruct_envelope_error` mapping Api→`SolvaPayError`, Paywall→`PaywallError(structured_content)`, Webhook/Transport→`SolvaPayError`, plus `call_native` / `call_native_blocking` / `call_native_sync`). **Facade:** hand-written `create_solvapay` / `@sp.payable` / `sp.gate` (+ `gate_blocking`) with host-side 60s customer dedup + 10s limits cache; gate copy from generated decision envelopes. **Parity:** `emit_parity_suite_py` + `--py-parity-out` → `signature_parity_generated_test.py` (defaults 2/500/300/10_000, error mapping, client presence + blocking twins). **Gates:** CI regen-drift + `@generated` cover `args`/`decisions`/`payload_builders`/`client`/`register`/`_native.py`/parity suite; `python-binding` job runs `cargo test -p solvapay-python --lib` + fresh-venv pytest. GREEN: dto-gen goldens (`python_shim_golden`, `native_py_golden`, `python_parity_golden`); solvapay-python lib tests; facade/envelope pytest. Sources: §15 notes 2/43; 39G-b/c emitter patterns; §2.4 / §5.6 / §5.7.

**Note 45 — Step 42 Python contract suite + PyPI publish (checked 2026-07-23):** Closed Phase 7. **Offline suite:** `tests/contract/` (fixture_loader / stub_backend / dispatch / host_adapters) replays all `contract/fixtures/**/*.json` against the Python binding — client wire via stdlib `ThreadingHTTPServer` + `SolvaPayClient._for_fixtures` (clock/RNG injection for auto idempotency), sync via `call_native_sync` (inject `nowMs` from `input.clock` for `buildCreateCustomerParams`), webhook via `_verify_webhook_at`, host adapters for `withRetry` / `pollBalanceUntilIncreased` / delay constants / `_resolve_authenticated_user` / `_construct_sdk_error`. Coverage guard ports the 38 routed-op success+error check. Live fixture census: parsed 787, executed 782, delegated 5. **Live:** `scripts/live_contract.py` (stdlib) + `.github/workflows/shadow-python.yml` (`workflow_dispatch` only; mirrors `shadow.yml`). **Publish:** `.github/workflows/publish-python.yml` — maturin §7.7 matrix + sdist + `check-wheels.py` + `pypa/gh-action-pypi-publish` OIDC; TestPyPI default; real PyPI gated. Version stamp via `SOLVAPAY_RELEASE_VERSION` / `SOLVAPAY_CORE_SHA` (`version()` / `native_build_info()`); load-time skew guard in `__init__.py`. Sources: §7.7; fixture-runner + TS harness; PyPI Trusted Publishing docs.

**Note 43 — Step 40 PyO3/maturin scaffold (checked 2026-07-23):** Scaffolded `sdks/python` (`solvapay-python` cdylib + maturin `solvapay` PyPI package, `abi3-py39`). **Runtime:** binding-owned multi-thread tokio via `pyo3-async-runtimes` 0.28; `future_into_py` for async client methods; blocking twins use `runtime.block_on` inside `Python::detach`. **Hello-world surface:** `version()`, sync `verify_webhook` (`catch_unwind` → `SolvaPayError` with `code`), generated `client.rs` with `get_merchant` async + `get_merchant_blocking`. **Emitter:** `Toolchain::Python` + `artifacts.python` chrome in `binding-emit.snapshot.json`; `--python-bindings-out` writes only the allowlisted hello-world `client.rs` (full 102-symbol emission deferred to Step 41); golden `python_shim_golden.rs` proves byte-identical below `@generated`. **Free-threaded:** default thread-safe module (no `gil_used = true`); core remains lock-light (no binding-owned globals beyond the once-init tokio runtime). **CI:** `python-binding` matrix (manylinux x64/arm64, musllinux, macOS universal2, Windows x64) + fresh-venv pytest + `check-wheels.py` hard-fail artifact gate; regen-drift + `@generated` header cover `client.rs`. GREEN: `cargo test -p solvapay-python` 7; pytest smoke 6; `cargo test -p dto-gen --test python_shim_golden`. Sources: §15 note 2; PyO3 0.28.3 / maturin 1.8+ / pyo3-async-runtimes 0.28.0.

**Note 42 — Step 39G-c native-side TS marshalling emitters + retrofit proof (checked 2026-07-23):** Landed the TypeScript half of §5.7 emission (`native.ts` / `wasm.ts`). **IR → unions:** `dto-gen` `emit_bindings_ts.rs` builds `NativeClientMethod`/`WasmClientMethod` from `client` symbols and `NativeSyncMethod`/`WasmSyncMethod` from `decisions` then `payloadBuilders` (MCP section split injects the two group comments), ordered by `emitOrder`. Loader/cache/the implementation selector/`unwrapEnvelope`/`reconstructEnvelopeError`/`callNative*`/`callWasm*`/`verifyWebhook*` chrome is verbatim from `assets/native-ts-emit.snapshot.json` (refreshed by `scripts/extract-native-ts-emit.mjs`); node vs wasm comment variants (`Step 37R-d — …` vs bare `@solvapay/core`) stay in chrome. **CLI:** `--native-ts-out` / `--wasm-ts-out` (single files). **Byte-identical proof:** regenerating the two files yields only the intentional `@generated by dto-gen — do not edit.` header block; golden `native_ts_golden.rs` asserts body equality after stripping leading JSDocs. **emitOrder alignment:** six `payloadBuilders` credit-display / seller-identity symbols had TS-union order inverted vs Rust shim `emitOrder` — reordered those `emitOrder` values to the public TS order and regenerated `payload_builders.rs` (mechanical reorder only; 39G-b golden still green). **Gates:** CI regen-drift + `@generated` header gate cover `native.ts` / `wasm.ts`. GREEN: `cargo test -p dto-gen` (incl. both goldens); `cargo test -p solvapay-node` 22/22; server 366×2, core 117×2, mcp-core 108×2; `pnpm delegation:check` OK (inventory unchanged). Sources: committed 37R/38R TS glue; §15 note 41.
