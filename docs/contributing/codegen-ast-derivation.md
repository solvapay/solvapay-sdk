...
- **`syn` blindness — the backstop is built.** `internal/boundary-asserts`
  emits fn-pointer coercions for every sync export (including `&'static str`,
  slices, `Vec`, `Duration`, and inherent methods) and per-DTO
  `assert_boundary` for async client methods. A mis-read type fails
  `cargo check -p boundary-asserts`.
