...
- **Anything the model or a text-only user must act on goes in `content[0].text`.** Plan name, remaining included usage, credit balance, reset/renewal date, the one recovery intent tool, and a https URL. `structuredContent` still carries the `BootstrapPayload` / gate for the widget and for programmatic consumers. It is a bonus for the model, never the only copy.
...
- **Text-only hosts must still inform, upgrade, and discover.** A gated or account call without an iframe has to (1) state current limits, (2) name exactly one recovery tool plus a https URL, (3) point at `docs://solvapay/overview.md` for capabilities and `manage_account` for the signed-in customer's plan / balance / payment method.
