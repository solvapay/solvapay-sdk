...
**Inline density.** Claude's host guidance (not the spec) caps an inline card at 2 actions and 4–5 data points, with no drill-ins. The host can expand to fullscreen to disclose the rest. Do not re-import fullscreen density into the inline card.
...
- **Text-only hosts must still inform, upgrade, and discover.** A gated call without an iframe has to (1) state current limits, (2) name `` `account` `` with the right `view` (or `` `activate_plan` `` when a `planRef` is known), (3) include a https URL. Keep `docs://solvapay/overview.md` registered for capability discovery — do not put it in recovery copy. The docs site pages are provider implementation guides, not customer recovery.
