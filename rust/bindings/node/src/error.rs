//! Binding error surface: webhook JS throws + JSON-envelope helpers (Step 37R-a).
//!
...
///
/// Used by sync decision envelopes and async client prep boundaries (§7.6).
...
///
/// Catches panics (§7.6) and maps [`SdkError`] to an error envelope. Success
/// values serialize into `{"ok":true,"value":…}`.
...
/// `{"ok":true,"value":…}`. Callers should wrap sync prep work in
/// [`std::panic::catch_unwind`] when needed (§7.6).
///
/// Host-native only — WASI builds omit [`crate::native_client`].
