...
const describePaymentIntegration = 
  (USE_REAL_BACKEND && SOLVAPAY_SECRET_KEY && STRIPE_TEST_SECRET_KEY) 
    ? describe 
    : describe.skip;
...
describePaymentIntegration('Payment Integration - End-to-End Stripe Checkout Flow', () => {
  ...
  let usageBasedPlan: { reference: string; name: string; pricePerUnit?: number; type?: string; freeUnits?: string } | null = null;
  ...
  beforeAll(async () => {
    if (!SOLVAPAY_SECRET_KEY || !STRIPE_TEST_SECRET_KEY) {
      ...
      console.log('   See packages/server/README.md "Payment Integration Tests" for setup instructions\n');
      return;
    }
    ...
    try {
      ...
      if (plans.length === 0) {
        throw new Error(`No plans found for agent ${defaultAgent.reference}. Please create at least one plan.`);
      }
      defaultPlan = plans[0];
      ...
      try {
        ...
        await createTestPaymentIntent(apiClient, defaultAgent.reference, planToTest.reference, testCustomer);
  ...
  describe('Credit Management - Free Units & Deduction', () => {
    ...
    it('should deduct exactly 1 credit per trackUsage call', async () => {
      ...
      console.log(`✅ trackUsage verified: 1 credit deducted (${creditsBeforeUsage} → ${creditsAfterUsage})`);
    }, 15000);
  });
  ...
  describe('E2E Flow - Payment → Webhook → Credits → Usage', () => {
    it('should complete full cycle: payment intent → confirmation → webhook processing → credit grant → protected function access', async () => {
      ...
      if (!ENABLE_WEBHOOK_TESTS) {
        ...
        console.log('   2. Forward webhooks: stripe listen --forward-to localhost:3001/webhooks/stripe');
      ...
      console.log(`✅ Payment confirmed: ${confirmed.amount / 100} ${confirmed.currency.toUpperCase()}`);
      ...
      console.log(`✅ Webhook processed: +${creditsAdded} units (${initialRemaining} → ${updatedLimits.remaining})`);
