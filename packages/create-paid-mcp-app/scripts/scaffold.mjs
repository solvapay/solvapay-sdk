...
const BASE_TEMPLATE_DIR = resolve(HERE, '..', 'templates', '_base')
const OPENAPI_OVERLAY_DIR = resolve(HERE, '..', 'templates', 'from-openapi')
  ...
  try {
    ...
    throw new Error(
      `Template directory missing: ${templateDir}. ` +
        `The packaged templates/_base/ tree did not ship with this build.`,
    )
  }
  ...
  try {
    ...
    throw new Error(
      `Template marker file missing: ${marker}. ` +
        `templates/_base/ is in placeholder state — investigate the published create-paid-mcp-app tarball.`,
    )
  }
}
...
async function writeDotEnv(target, selections) {
  ...
  const lines = [
    '# Generated by create-paid-mcp-app scaffold.',
