...
describe('usePurchaseStatus', () => {
  ...
  describe('Edge cases and integration', () => {
    ...
    it('should handle multiple cancelled purchases with different start dates', () => {
      ...
      const purchases = [sub1, sub2, sub3]
      vi.mocked(usePurchaseModule.usePurchase).mockReturnValue({
        purchases,
        loading: false,
        hasPlan: vi.fn(),
  ...
  describe('isPaidPurchase helper (internal)', () => {
    it('should correctly identify paid purchases', () => {
      const paidSub = createPurchase({ amount: 1000 })
      const freeSub = createPurchase({ amount: 0 })
      const undefinedSub = createPurchase({ amount: undefined })
      const purchases = [paidSub, freeSub, undefinedSub]
