...
export function usePurchaseStatus(): PurchaseStatusReturn {
  ...
  const isPaidPurchase = useCallback((sub: PurchaseInfo): boolean => {
  ...
  const purchaseData = useMemo(() => {
    const cancelledPaidPurchases = purchases.filter(sub => {
      // Look for purchases with cancelledAt set and status === 'active'
      return sub.status === 'active' && sub.cancelledAt && isPaidPurchase(sub)
    })
