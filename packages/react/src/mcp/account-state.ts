...
export function resolveAccountState(input: AccountStateInput): AccountState {
  if (input.loading) return 'G'

  const limits = input.limits
  if (limits?.activationRequired) return 'H'
  if (isCancelledNotExpired(input.purchase, input.now)) return 'J'
  if (limits?.overage) return 'I'
  if (limits?.needsTopUp) return 'D'

  const shape = input.planShape ?? resolvePlanShape(input.purchase?.planSnapshot)
  if (!input.purchase && !shape) return 'A'
  ...
  if (input.purchase) return 'C'
  return 'A'
}

function isAtFiniteCap(limits: AccountLimitsLike | null | undefined): boolean {
 ...
 */
export function allowanceMeterUnit(
