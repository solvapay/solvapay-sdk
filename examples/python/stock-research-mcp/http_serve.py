...
def build_http_app(
    ...
    *,
    ...
    async def health(_request: Request) -> JSONResponse:
        return JSONResponse({"status": "ok", "server": "stock-research-mcp"})
    ...
    mcp_app = server.streamable_http_app(
        ...
        host=bind_host,
        # MCPJam and other hosts probe GET /mcp for SSE; a stateful session
        # waits forever on that GET. Stateless matches the TypeScript servers.
        # JSON responses (not SSE) so initialize completes in one HTTP body —
        # the SSE path returns 200 with an empty stream and MCPJam hangs.
        json_response=True,
        stateless_http=True,
