#!/usr/bin/env bash
# Reserved-domain ngrok for Python stock-research on appmcp.your-subdomain.
# If the platform agent already has that hostname online, reuse it and bind
...
stop_stock_research_on_port() {
  ...
  for pid in $pids; do
    ...
    if [[ "$cmd" != *stock-research-mcp/main.py* ]]; then
      continue
    fi
    echo "Stopping leftover stock-research-mcp (pid ${pid}) on :${port}"
...
cleanup() {
  ...
  stop_stock_research_on_port "${MCP_PORT:-3030}"
  if [[ -n "${NGROK_PID:-}" ]]; then
...
PUBLIC_URL="${STOCK_RESEARCH_NGROK_URL:-${MCP_PUBLIC_BASE_URL:-}}"
if [[ -z "$PUBLIC_URL" ]]; then
  echo "Missing STOCK_RESEARCH_NGROK_URL (or MCP_PUBLIC_BASE_URL)." >&2
  echo "Set it in examples/python/stock-research-mcp/.env to your reserved ngrok origin, for example:" >&2
  echo "  STOCK_RESEARCH_NGROK_URL=https://appmcp.your-subdomain.ngrok.app" >&2
  exit 1
fi
...
if TUNNEL_PORT="$(tunnel_local_port "$PUBLIC_URL")"; then
  ...
  if [[ -n "${MCP_PORT:-}" && "$MCP_PORT" != "$TUNNEL_PORT" ]]; then
    ...
    echo "stock-research-mcp must listen on ${TUNNEL_PORT} to serve that hostname." >&2
    exit 1
  fi
  ...
  if ! command -v ngrok >/dev/null 2>&1; then
    echo "ngrok is required to expose stock-research-mcp. Install it with: brew install ngrok" >&2
    exit 1
  fi

  NGROK_CONFIG="${STOCK_RESEARCH_NGROK_CONFIG:-}"
  ...
  if [[ -z "${NGROK_CONFIG:-}" || ! -f "$NGROK_CONFIG" ]]; then
    ...
    echo "Copy platform/ngrok.yml.example to platform/ngrok.yml, or set STOCK_RESEARCH_NGROK_CONFIG." >&2
    echo "For local-only HTTP (no tunnel), use: pnpm mcp:stock-research" >&2
    exit 1
  fi
fi

echo "Starting stock-research-mcp on http://localhost:${MCP_PORT}"
echo "Public tunnel origin: ${MCP_PUBLIC_BASE_URL}"
echo "MCP endpoint: ${MCP_PUBLIC_BASE_URL}/mcp"

stop_stock_research_on_port "${MCP_PORT}"

uv run --project "$PYTHON_MCP" --extra dev --with httpx --with uvicorn \
...
while true; do
  if ! kill -0 "$SERVER_PID" 2>/dev/null; then
    echo "stock-research-mcp exited unexpectedly." >&2
