...
3. **Environment Variables**
   - Run `npx solvapay init` in `examples/typescript/checkout-demo/` to create a gitignored `.env`, **or**
...
cd examples/typescript/checkout-demo
...
`examples/typescript/chat-checkout-demo` (`scripts/deploy.mjs`, gitignored env files, one-time Worker secrets).
...
2. **Install** — from the **monorepo root**, run `pnpm install` (CI uses `pnpm install --frozen-lockfile`; commit any `pnpm-lock.yaml` changes when `examples/typescript/checkout-demo/package.json` changes).
...
5. **SolvaPay** — a secret key and product ref for the merchant you want the demo to use (`npx solvapay init` in `examples/typescript/checkout-demo/` writes these to gitignored `.env`).
