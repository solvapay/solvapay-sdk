#!/usr/bin/env bash
# Reserved-domain ngrok for Go weather-mcp.
...
stop_weather_mcp_on_port() {
  ...
  for pid in $pids; do
    ...
    if [[ "$cmd" != *weather-mcp* && "$cmd" != *"--mode http"* && "$cmd" != *stock-research-mcp* ]]; then
      continue
    fi
    ...
    if [[ -n "$ppid" && "$ppid" != "1" ]]; then
      ...
      if [[ "$pcmd" == *go\ run* || "$pcmd" == *uv\ run* ]]; then
...
cleanup() {
  ...
  stop_weather_mcp_on_port "${MCP_PORT:-3030}"
  if [[ -n "${NGROK_PID:-}" ]]; then
...
PUBLIC_URL="${WEATHER_MCP_NGROK_URL:-${MCP_PUBLIC_BASE_URL:-}}"
if [[ -z "$PUBLIC_URL" ]]; then
  echo "Missing WEATHER_MCP_NGROK_URL (or MCP_PUBLIC_BASE_URL)." >&2
  echo "Set it in examples/go/weather-mcp/.env to your reserved ngrok origin, for example:" >&2
  echo "  WEATHER_MCP_NGROK_URL=https://appmcp.your-subdomain.ngrok.app" >&2
  exit 1
fi
...
if TUNNEL_PORT="$(tunnel_local_port "$PUBLIC_URL")"; then
  ...
  if [[ -n "${MCP_PORT:-}" && "$MCP_PORT" != "$TUNNEL_PORT" ]]; then
    ...
    echo "weather-mcp must listen on ${TUNNEL_PORT} to serve that hostname." >&2
    exit 1
  fi
  ...
  if ! command -v ngrok >/dev/null 2>&1; then
    echo "ngrok is required to expose weather-mcp. Install it with: brew install ngrok" >&2
    exit 1
  fi

  NGROK_CONFIG="${WEATHER_MCP_NGROK_CONFIG:-}"
  ...
  if [[ -z "${NGROK_CONFIG:-}" || ! -f "$NGROK_CONFIG" ]]; then
    ...
    echo "Copy platform/ngrok.yml.example to platform/ngrok.yml, or set WEATHER_MCP_NGROK_CONFIG." >&2
    echo "For local-only HTTP (no tunnel), use: ./scripts/http.sh" >&2
    exit 1
  fi
fi

echo "Starting weather-mcp on http://localhost:${MCP_PORT}"
echo "Public tunnel origin: ${MCP_PUBLIC_BASE_URL}"
echo "MCP endpoint: ${MCP_PUBLIC_BASE_URL}/mcp"

stop_weather_mcp_on_port "${MCP_PORT}"
...
while true; do
  if ! kill -0 "$SERVER_PID" 2>/dev/null; then
    echo "weather-mcp exited unexpectedly." >&2
