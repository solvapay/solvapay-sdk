#!/usr/bin/env bash
# Reserved-domain ngrok for Ruby bitcoin-analytics-mcp.
...
stop_mcp_example_on_port() {
  ...
  for pid in $pids; do
    ...
    if [[ -n "$ppid" && "$ppid" != "1" ]]; then
      ...
      if [[ "$pcmd" == *go\ run* || "$pcmd" == *uv\ run* || "$pcmd" == *ruby* ]]; then
...
if TUNNEL_PORT="$(tunnel_local_port "$PUBLIC_URL")"; then
  ...
  if [[ -n "${MCP_PORT:-}" && "$MCP_PORT" != "$TUNNEL_PORT" ]]; then
    ...
    echo "bitcoin-analytics-mcp must listen on ${TUNNEL_PORT} to serve that hostname." >&2
    exit 1
  fi
  ...
  if ! command -v ngrok >/dev/null 2>&1; then
    echo "ngrok is required to expose bitcoin-analytics-mcp. Install it with: brew install ngrok" >&2
    exit 1
  fi

  NGROK_CONFIG="${BITCOIN_ANALYTICS_NGROK_CONFIG:-}"
  ...
  if [[ -z "${NGROK_CONFIG:-}" || ! -f "$NGROK_CONFIG" ]]; then
    ...
    echo "Copy platform/ngrok.yml.example to platform/ngrok.yml, or set BITCOIN_ANALYTICS_NGROK_CONFIG." >&2
    echo "For local-only HTTP (no tunnel), use: pnpm mcp:bitcoin-analytics" >&2
    exit 1
  fi
fi

echo "Starting bitcoin-analytics-mcp on http://localhost:${MCP_PORT}"
...
while true; do
  if ! kill -0 "$SERVER_PID" 2>/dev/null; then
    echo "bitcoin-analytics-mcp exited unexpectedly." >&2
