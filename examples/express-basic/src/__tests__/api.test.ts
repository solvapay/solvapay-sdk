...
describe('Express Basic API - Paywall Tests', () => {
  ...
  describe('Paywall Free Tier', () => {
    ...
    it('should block operations after free tier limit (6th operation)', async () => {
      ...
      const response = await request(app).post('/tasks').set('x-customer-ref', customerRef).send({
        title: 'Blocked Task',
      })
    ...
    it('should provide fresh free tier for different users', async () => {
      ...
      const user1Response = await request(app).post('/tasks').set('x-customer-ref', user1).send({
      ...
      const user2Response = await request(app).post('/tasks').set('x-customer-ref', user2).send({
        title: 'User2 Task',
      })
    ...
    it('should enforce limit consistently across all operations using same plan', async () => {
      ...
      const listResponse = await request(app).get('/tasks').set('x-customer-ref', customerRef)
