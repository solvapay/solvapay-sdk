...
export class StubSolvaPayClient implements SolvaPayClient {
  ...
  constructor(options: StubClientOptions = {}) {
    ...
    this.log(`   Storage: ${this.useFileStorage ? 'File-based (' + this.dataDir + ')' : 'In-memory only'}`);
  ...
  private async initializeStorage(): Promise<void> {
    ...
    try {
      ...
      await fs.writeFile(
        this.customerFile,
        JSON.stringify(this.inMemoryCustomers, null, 2)
      );
  ...
  private async withFileLock<T>(fn: () => Promise<T>): Promise<T> {
    ...
    let release: () => void;
    this.fileLock = new Promise(resolve => { release = resolve; });
  ...
  async checkLimits(params: {
    ...
    if (!usage || this.isNewDay(usage.lastReset, now)) {
      ...
      this.log(`🆕 Reset daily counter for ${params.customerRef}, remaining: ${this.freeTierLimit - 1}`);
      return {
        withinLimits: true,
  ...
  async trackUsage(params: {
    ...
    this.log(`   Outcome: ${params.outcome}, Duration: ${params.actionDuration}ms, Action: ${params.action || 'unknown'}`);
  }
  ...
  async addCredits(customerRef: string, credits: number): Promise<void> {
    ...
    await this.saveCustomerData(customerData);
    this.log(`💰 Added ${credits} credits to ${customerRef}, total: ${customerData[customerRef].credits}`);
  }
