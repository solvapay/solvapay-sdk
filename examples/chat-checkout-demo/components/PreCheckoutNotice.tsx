...
import { formatPrice, useLocale, usePlans, type Plan } from '@solvapay/react'
...
interface PreCheckoutNoticeProps {
  ...
  productRef: string
  /**
   * The 402 paywall payload that surfaced this notice. `activation_required`
   * gates carry plan summaries on `paywallContent.plans`, which we use as a
   * fallback when the live `usePlans` fetch hasn't landed yet (so the
   * bullets render immediately even on first paint).
   */
...
/**
 * Inline pre-checkout notice strip. Restores the educational moment
 * between hitting a 402 and opening the full embedded checkout: a
 * scenario-tailored heading, 1-2 plan-derived bullets, and a single
 * CTA. Click flips `<InlineCheckout>` from `stage: 'notice'` to
 * `stage: 'checkout'`, mounting `<PaywallNotice.EmbeddedCheckout>`.
 *
 * Demo-only — the SDK's `<PaywallNotice>` doesn't gate `EmbeddedCheckout`
 * behind a click since that's a consumer UX choice.
 */
export const PreCheckoutNotice: React.FC<PreCheckoutNoticeProps> = ({
  currentScenario,
  productRef,
  paywallContent,
  onUnlock,
  ...
  // paid-product gates (`payment_required`) don't, so fall back to the
  // product's plans directly. The shared `plansCache` keyed by `productRef`
  // means this is usually a no-op extra fetch.
  const { plans: livePlans } = usePlans({ productRef: productRef || undefined })
  const plans = useMemo(() => {
    if (livePlans.length > 0) return livePlans
    if (paywallContent && 'plans' in paywallContent && paywallContent.plans) {
      return paywallContent.plans.map(planFromGate)
    }
    return []
  }, [livePlans, paywallContent])

  const paidPlan = useMemo(
    () => plans.find(p => p.requiresPayment !== false && (p.price ?? 0) > 0),
    [plans],
  )
