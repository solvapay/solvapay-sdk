  ...
  try {
    ...
    if (!serverUrl) {
      return NextResponse.json(
        { 
          error: 'Server misconfiguration: Missing PUBLIC_URL environment variable. This prevents serving a valid OpenAPI specification with placeholder URLs.' 
        }, 
        { status: 500 }
      );
    }
    ...
    if (specString.includes('your-domain') || specString.includes('your-subdomain')) {
      return NextResponse.json(
        { 
          error: 'Invalid OpenAPI specification: Contains placeholder URLs. Please regenerate with proper environment variables.' 
        }, 
        { status: 500 }
      );
    }
