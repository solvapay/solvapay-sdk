...
export default function HomePage() {
  ...
  const { cancelledSubscription, shouldShowCancelledNotice, formatDate } = useSubscriptionStatus()
  ...
  const handleManageSubscription = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}))
        const errorMessage =
          errorData.error || errorData.details || 'Failed to create management token'
        throw new Error(errorMessage)
      }
  ...
  return (
    <div className="px-4 sm:px-0">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-4">SolvaPay Custom GPT Actions Demo</h1>
        <p className="text-lg text-gray-600 mb-6">
          A Next.js frontend demonstrating SolvaPay&apos;s paywall-protected API endpoints designed
          for OpenAI Custom GPT Actions integration.
        </p>
        ...
        {isLoading ? (
          ...
          <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg">
            <p className="text-sm text-green-800">
              You&apos;re on the{' '}
              <span className="font-semibold">{activePaidSubscription?.planName}</span> plan
            </p>
          </div>
          ...
          <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-sm text-blue-800">
              You&apos;re on the{' '}
              <span className="font-semibold">{mostRecentActiveSubscription?.planName}</span> plan
            </p>
          </div>
        ) : shouldShowCancelledNotice && cancelledSubscription ? (
          <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
            <p className="text-sm font-medium text-amber-900">
              Your <span className="font-semibold">{cancelledSubscription.planName}</span>{' '}
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex items-center space-x-2">
              <div
                className={`w-3 h-3 rounded-full ${apiStatus.tasks ? 'bg-green-500' : 'bg-red-500'}`}
              ></div>
              <span className="text-sm font-medium">Tasks API</span>
            </div>
            <div className="flex items-center space-x-2">
              <div
                className={`w-3 h-3 rounded-full ${apiStatus.oauth ? 'bg-green-500' : 'bg-red-500'}`}
              ></div>
      ...
      <div className="bg-white shadow rounded-lg p-6">
        ...
        <p className="text-gray-600 mb-4">Test the API endpoints directly from this interface:</p>
